...
static void NoDbgRegParms updateFormatOfShortenedObjectto(sqInt objOop, sqInt indexableSize);
	...
	if ((delta <= 8 /* allocationUnit */)
	 && (isOldObject(objOop))) {
		...
		if (numSlots >= (numSlotsMask())) {
			...
			flag("endianness");
			longAtput(GIV(freeStart), (((sqInt)((usqInt)((numSlotsMask())) << (numSlotsFullShift())))) + numSlots);
			longAtput(newObj, headerForSlotsformatclassIndex(numSlotsMask(), format, classIndex));
		}
		else {
