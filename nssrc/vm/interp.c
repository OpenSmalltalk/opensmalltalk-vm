...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
 ...
 && (((((usqInt) (longAt(argumentArray))) >> 8) & 15) == 2)))) {
	...
	if (GIV(argumentCount) == 3) {
		longAtput(GIV(stackPointer) - (3 * BytesPerWord), longAt(GIV(stackPointer) - (2 * BytesPerWord)));
		/* begin pop: */
		GIV(stackPointer) -= 3 * BytesPerWord;
	}
	else {
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if ((rcvr & 1)) {
		/* begin pop:thenPush: */
		longAtput(sp = GIV(stackPointer) - ((1 - 1) * BytesPerWord), longAt((GIV(specialObjectsOop) + BaseHeaderSize) + (TrueObject << ShiftForWord)));
		GIV(stackPointer) = sp;
	}
	else {
		hdr = longAt(rcvr);
