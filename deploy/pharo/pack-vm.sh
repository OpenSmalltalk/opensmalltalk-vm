...
case "${ARCH}" in
	...
	linux32ARMv6)
		...
		;;
	win32x86)
		do_rename_vm "win" "i386" "${ROOT_DIR}/build.${ARCH}/${FLAVOR}/build/vm" "Pharo.exe PharoConsole.exe *.dll"
		;;
	win64x64)
		do_rename_vm "win" "x86_64" "${SUFFIX}" "${ROOT_DIR}/build.${ARCH}/${FLAVOR}/build/vm" "Pharo.exe PharoConsole.exe *.dll"
		;;
	*) 
