/* Automatically generated from Squeak on 11 January 2010 9:57:42 pm 
   by VMMaker 3.11.12
 */
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"ClipboardExtendedPlugin 11 January 2010 (i)"
#else
	"ClipboardExtendedPlugin 11 January 2010 (e)"
#endif
;
...
EXPORT(sqInt) ioAddClipboardData(void) {
	...
	clipboardAddress = interpreterProxy->positive64BitValueOf(clipboard);
...
EXPORT(sqInt) ioCreateClipboard(void) {
	...
	clipboardAddress = interpreterProxy->positive64BitIntegerFor(sqCreateClipboard());
	if (interpreterProxy->failed()) {
		return null;
	}
