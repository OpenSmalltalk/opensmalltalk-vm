...
l2:	/* end stackIntegerValue: */;
	...
	/* begin success: */
	successBoolean = (fetchClassOf(copiedValues)) == (fetchPointerofObject(ClassArray, foo->specialObjectsOop));
	if (!(successBoolean)) {
		if (!foo->primFailCode) {
			foo->primFailCode = 1;
		}
...
register struct foo * foo = &fum;
	...
	foo->primitiveIndex = (primBits & 511) + (((usqInt) primBits) >> 19);
	/* begin success: */
	successBoolean = (foo->argumentCount - 1) == (argumentCountOf(foo->newMethod));
	if (!(successBoolean)) {
		if (!foo->primFailCode) {
			foo->primFailCode = 1;
		}
...
l1:	/* end fetchClassOf: */;
	...
	if (((foo->newMethod & 1) == 0) && (((((usqInt) (longAt(foo->newMethod))) >> 8) & 15) >= 12)) {
		/* begin success: */
		successBoolean = (argumentCountOf(foo->newMethod)) == foo->argumentCount;
		if (!(successBoolean)) {
			if (!foo->primFailCode) {
				foo->primFailCode = 1;
			}
...
register struct foo * foo = &fum;
	...
	if (((foo->newMethod & 1) == 0) && (((((usqInt) (longAt(foo->newMethod))) >> 8) & 15) >= 12)) {
		/* begin success: */
		successBoolean1 = (argumentCountOf(foo->newMethod)) == foo->argumentCount;
		if (!(successBoolean1)) {
			if (!foo->primFailCode) {
				foo->primFailCode = 1;
			}
...
register struct foo * foo = &fum;
	...
	for (i4 = 1; i4 <= foo->jmpDepth; i4 += 1) {
		oop = foo->suspendedCallbacks[i4];
		if (!((oop & 1))) {
			markAndTrace(oop);
		}
		oop = foo->suspendedMethods[i4];
		if (!((oop & 1))) {
			markAndTrace(oop);
		}
	}
