 ...
 * 
 * Last edited: 2002-10-26 14:33:09 by piumarta on emilia.inria.fr
 * 
 * NOTE:
...
int sqCopyFilesizetosize(char *srcName, int srcNameSize, char *dstName, int dstNameSize)
{
  ...
  sqFilenameFromString(from, srcName, srcNameSize);
  sqFilenameFromString(to, dstName, dstNameSize);
  return copy(from, to);
}
