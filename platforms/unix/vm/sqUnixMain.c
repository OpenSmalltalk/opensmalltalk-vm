 ...
 *
 * Last edited: 2003-03-03 03:55:24 by piumarta on emilia.inria.fr
 */
  ...
  for (;;)
    {
      FILE *f= 0;
      struct stat sb;
      if ((  (-1 == stat(shortImageName, &sb)))
	  || ( 0 == (f= fopen(shortImageName, "r"))))
	{
      ...
      {
	int fd= open(shortImageName, O_RDONLY);
	if (fd < 0) abort();
	printf("fstat(%d) => %d\n", fd, fstat(fd, &sb));
      }
