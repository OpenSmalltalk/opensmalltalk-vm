...
void sendSelection(XSelectionRequestEvent *requestEv)
{
  ...
  /* refuse the selection if the target type isn't XA_STRING[_UTF8] */
  if ((requestEv->target == XA_STRING) || (requestEv->target == XA_STRING_UTF8))
    {
      int len= strlen(stPrimarySelection);
      char *buf= (char *)malloc(len * 2);
      int n;
#    ifdef DEBUG_SELECTION
      printf("sendSelection: len=%d, sel=%s\n", len, stPrimarySelection);
#    endif
      if (requestEv->target == XA_STRING_UTF8)
	n= sq2uxUTF8(stPrimarySelection, len, buf, len * 2, 1);
      else
	n= sq2uxText(stPrimarySelection, len, buf, len * 2, 1);
#    ifdef DEBUG_SELECTION
      printf("sendSelection: n=%d, buf=%s\n", n, buf);
#    endif
      ...
      free(buf);
      notifyEv.property= (requestEv->property == None) ? requestEv->target : requestEv->property;
    }
...
char *getSelection(void)
{
  ...
  if (textEncodingUTF8)
    XConvertSelection(stDisplay, XA_PRIMARY, XA_STRING_UTF8, None, stWindow, CurrentTime);
  else
    XConvertSelection(stDisplay, XA_PRIMARY, XA_STRING, None, stWindow, CurrentTime);
...
#       endif
  ...
  /* get the value of the selection from the containing property */
  {
    ...
    if (bytesAfter > 0)
      XBell(stDisplay, 0);
    data[min(nitems, MAX_SELECTION_SIZE - 1)]= '\0';
  }
  /* convert the encoding if necessary */
  {
    int bytes= strlen(data);
    if (bytes && allocateSelectionBuffer(bytes * 2))
      {
	if (textEncodingUTF8)
	  ux2sqUTF8(data, bytes, stPrimarySelection, bytes * 2, 1);
	else
...
static int display_clipboardReadIntoAt(int count, int byteArrayIndex, int startIndex)
{
  ...
  if (!isConnectedToXServer)
    return 0;
  clipSize= min(count, strlen(getSelection()));
...
static int x2sqKey(XKeyEvent *xevt)
{
  ...
  int charCode= buf[0];
#ifdef DEBUG_EVENTS
  printf("convert keycode %d=%02x -> %d=%02x (keysym %ld)\n",
...
# endif
# ifndef XA_STRING_UTF8
  XA_STRING_UTF8= XInternAtom(stDisplay, "STRING_UTF8", False);
# endif
}
...
void setWindowSize(void)
{
  ...
  printf("browserWindow %d\n", browserWindow);

  if (browserWindow) return;

  printf("savedWindowSize %x (%d %d)\n", winSize, winSize >> 16, winSize & 0xffff);
...
static void display_winOpen(void)
{
  ...
  printf("saved window size is %d %d\n", sws >> 16, sws & 0xffff);
  if (headless)
    forgetXDisplay();
  else
    openXDisplay();
