/* Automatically generated from Squeak on #(31 March 2005 3:47:26 pm) */
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"SocketPlugin 31 March 2005 (i)"
#else
	"SocketPlugin 31 March 2005 (e)"
#endif
;
...
static sqInt netAddressToInt(unsigned char * ptrToByteArray) {
	...
	sz = interpreterProxy->byteSizeOf(((sqInt)(long)(ptrToByteArray) - sizeof(sqInt)));
	if (!(sz == 4)) {
...
EXPORT(sqInt) primitiveResolverStartNameLookup(void) {
	...
	if (!(interpreterProxy->failed())) {
		sz = interpreterProxy->byteSizeOf(((sqInt)(long)(name) - sizeof(sqInt)));
