/* Automatically generated from Squeak on #(31 March 2005 3:48:40 pm) */
...
#define AllButHashBits 18446744073172811775ULL
#define AllButMarkBit 9223372036854775807ULL
#define AllButMarkBitAndTypeMask 9223372036854775804ULL
#define AllButRootBit 13835058055282163711ULL
#define AllButTypeMask 18446744073709551612ULL
...
#define Byte3Mask 4278190080ULL
#define Byte3Shift 24
#define Byte3ShiftNegated -24
#define Byte4Mask 1095216660480ULL
#define Byte4Shift 32
#define Byte4ShiftNegated -32
#define Byte5Mask 280375465082880ULL
#define Byte5Shift 40
#define Byte5ShiftNegated -40
#define Byte6Mask 71776119061217280ULL
#define Byte7Mask 18374686479671623680ULL
#define Byte7Shift 56
#define Byte7ShiftNegated -56
#define Bytes3to0Mask 4294967295ULL
#define Bytes7to4Mask 18446744069414584320ULL
...
#define LargeContextSize 504
#define LastLinkIndex 1
#define LiteralStart 1
#define LongSizeMask 18446744073709551608ULL
#define MarkBit 9223372036854775808ULL
...
#define RootBit 4611686018427387904ULL
...
#define Upward 3
#define ValueIndex 1
#define WordMask 18446744073709551615ULL
#define XIndex 0
#define YIndex 1
...
sqInt instantiateSmallClasssizeInBytesfill(sqInt classPointer, sqInt sizeInBytes, sqInt fillValue);
...
sqInt isIntegerValue(sqInt valueWord);
...
void *primitiveTable[577] = {
	...
	/* 520*/ (void *)primitiveObsoleteIndexedPrimitive,
...
register struct foo * foo = &fum;
	...
	valuePointer = ((((foo->instructionPointer - foo->method) - (BaseHeaderSize - 2)) << 1) | 1);
	if (activeCntx < foo->youngStart) {
		possibleRootStoreIntovalue(activeCntx, valuePointer);
	}
	longAtput((activeCntx + BaseHeaderSize) + (InstructionPointerIndex << ShiftForWord), valuePointer);
	/* begin storePointer:ofObject:withValue: */
	valuePointer1 = (((((((usqInt) ((foo->stackPointer - foo->activeContext) - BaseHeaderSize)) >> ShiftForWord) - TempFrameStart) + 1) << 1) | 1);
	if (activeCntx < foo->youngStart) {
		possibleRootStoreIntovalue(activeCntx, valuePointer1);
	}
...
register struct foo * foo = &fum;
	...
	for (p = 0; p <= (CacheProbeMax - 1); p += 1) {
		...
		if ((methodCache[probe + MethodCacheSelector]) == 0) {
			...
			methodCache[probe + MethodCachePrimFunction] = (((long) foo->primitiveFunctionPointer));
			return null;
		}
	}
...
l1:	/* end sizeBitsOf: */;
	...
	lastIn = in + ((((sqInt) (extraSize + bodySize) >> 3)) * BytesPerWord);
...
register struct foo * foo = &fum;
	...
	/* begin success: */
	successValue = (result >= -1073741824) && (result <= 1073741823);
	foo->successFlag = successValue && foo->successFlag;
	return result;
}
...
register struct foo * foo = &fum;
	...
	/* begin success: */
	successValue = (integerResult >= -1073741824) && (integerResult <= 1073741823);
	foo->successFlag = successValue && foo->successFlag;
...
register struct foo * foo = &fum;
	...
	if (((methodCache[probe + MethodCacheSelector]) == foo->messageSelector) && ((methodCache[probe + MethodCacheClass]) == class)) {
		...
		foo->primitiveFunctionPointer = ((void *) (methodCache[probe + MethodCachePrimFunction]));
		ok = 1;
		goto l1;
	}
...
sqInt floatObjectOf(double aFloat) {
	...
	flag("Dan");
	newFloatObj = instantiateSmallClasssizeInBytesfill(longAt((foo->specialObjectsOop + BaseHeaderSize) + (ClassFloat << ShiftForWord)), 8 + BaseHeaderSize, 0);
...
register struct foo * foo = &fum;
	...
	if (lit < foo->youngStart) {
		possibleRootStoreIntovalue(lit, ConstZero);
	}
...
sqInt imageSegmentVersion(void) {
	...
	return (imageFormatVersion()) | (wholeWord & 4278190080ULL);
}
...
sqInt integerValueOf(sqInt objectPointer) {
	if ((objectPointer & 2147483648ULL) != 0) {
		return ((((usqInt) (objectPointer & 2147483647ULL)) >> 1) - 1073741823) - 1;
	} else {
		...
		case 143:
			/* experimentalBytecode */
			{
				...
				if (byte3 < 178) {
					...
					if ((result >= -1073741824) && (result <= 1073741823)) {
		...
		case 184:
			/* bytecodePrimMultiply */
			{
				...
				if (((rcvr & arg) & 1) != 0) {
					...
					arg = (arg >> 1);
					result = rcvr * arg;
					if (((arg == 0) || ((result / arg) == rcvr)) && ((result >= -1073741824) && (result <= 1073741823))) {
						/* begin internalPop:thenPush: */
						longAtPointerput(localSP -= (2 - 1) * BytesPerWord, ((result << 1) | 1));
						/* begin fetchNextBytecode */
						currentBytecode = byteAtPointer(++localIP);
			...
			/* bytecodePrimMakePoint */
			{
				...
				if ((rcvr & 1)) {
					if ((argument & 1)) {
						...
						pointResult = instantiateSmallClasssizeInBytesfill(longAt((foo->specialObjectsOop + BaseHeaderSize) + (ClassPoint << ShiftForWord)), 3 * BytesPerWord, foo->nilObj);
						/* begin storePointer:ofObject:withValue: */
						...
						/* begin makePointwithxValue:yValue: */
						pointResult1 = instantiateSmallClasssizeInBytesfill(longAt((foo->specialObjectsOop + BaseHeaderSize) + (ClassPoint << ShiftForWord)), 3 * BytesPerWord, foo->nilObj);
						/* begin storePointer:ofObject:withValue: */
					...
					/* begin makePointwithxValue:yValue: */
					pointResult2 = instantiateSmallClasssizeInBytesfill(longAt((foo->specialObjectsOop + BaseHeaderSize) + (ClassPoint << ShiftForWord)), 3 * BytesPerWord, foo->nilObj);
					/* begin storePointer:ofObject:withValue: */
		...
		case 200:
			/* bytecodePrimBlockCopy */
			{
				...
				l34:	/* end sizeBitsOf: */;
					...
					/* begin storePointer:ofObject:withValue: */
					if (newContext < foo->youngStart) {
						possibleRootStoreIntovalue(newContext, initialIP);
					}
					...
					/* begin storePointer:ofObject:withValue: */
					if (newContext < foo->youngStart) {
						possibleRootStoreIntovalue(newContext, ((0 << 1) | 1));
					}
...
sqInt isIntegerValue(sqInt valueWord) {
	return (valueWord >= -1073741824) && (valueWord <= 1073741823);
}
...
register struct foo * foo = &fum;
	...
	if (integerValue >= 0) {
		if ((integerValue >= -1073741824) && (integerValue <= 1073741823)) {
			return ((integerValue << 1) | 1);
		}
	}
	if (BytesPerWord == 4) {
		newLargeInteger = instantiateSmallClasssizeInBytesfill(longAt((foo->specialObjectsOop + BaseHeaderSize) + (ClassLargePositiveInteger << ShiftForWord)), BaseHeaderSize + 4, 0);
	} else {
...
sqInt positive64BitIntegerFor(sqLong integerValue) {
	...
	newLargeInteger = instantiateSmallClasssizeInBytesfill(longAt((foo->specialObjectsOop + BaseHeaderSize) + (ClassLargePositiveInteger << ShiftForWord)), BaseHeaderSize + 8, 0);
	for (i = 0; i <= 7; i += 1) {
...
register struct foo * foo = &fum;
	...
	entriesNeeded = ((sqInt) (lastPointerOf(array1)) >> 3);
	if (twoWayFlag) {
...
register struct foo * foo = &fum;
	...
	if (foo->successFlag) {
		if ((integerResult >= -1073741824) && (integerResult <= 1073741823)) {
			/* begin pop:thenPush: */
...
register struct foo * foo = &fum;
	...
	if (foo->successFlag) {
		/* begin pushInteger: */
		/* begin push: */
		longAtput(sp = foo->stackPointer + BytesPerWord, ((((((usqInt) (longAt(thisReceiver))) >> 17) & 4095) << 1) | 1));
		foo->stackPointer = sp;
	} else {
		/* begin unPop: */
		foo->stackPointer += 1 * BytesPerWord;
...
register struct foo * foo = &fum;
	...
	if (foo->argumentCount == 0) {
		...
		/* begin pushInteger: */
		/* begin push: */
		longAtput(sp = foo->stackPointer + BytesPerWord, ((((longAt(foo->freeBlock)) & AllButTypeMask) << 1) | 1));
		foo->stackPointer = sp;
		return null;
	}
	if (foo->argumentCount == 1) {
	...
	l1:	/* end booleanValueOf: */;
		...
		/* begin push: */
		longAtput(sp1 = foo->stackPointer + BytesPerWord, ((integerValue << 1) | 1));
		foo->stackPointer = sp1;
		return null;
	}
	/* begin primitiveFail */
...
register struct foo * foo = &fum;
	...
	/* begin pop2AndPushIntegerIfOK: */
	if (foo->successFlag) {
		if ((quotient >= -1073741824) && (quotient <= 1073741823)) {
			/* begin pop:thenPush: */
...
l2:	/* end checkedIntegerValueOf: */;
	if ((integerArgument != 0) && ((integerReceiver % integerArgument) == 0)) {
		/* begin pop2AndPushIntegerIfOK: */
		if (foo->successFlag) {
			if (((integerReceiver / integerArgument) >= -1073741824) && ((integerReceiver / integerArgument) <= 1073741823)) {
				/* begin pop:thenPush: */
...
l2:	/* end checkedIntegerValueOf: */;
	...
	if ((ConstZero >= -1073741824) && (ConstZero <= 1073741823)) {
		/* begin storePointer:ofObject:withValue: */
		if (lit < foo->youngStart) {
			possibleRootStoreIntovalue(lit, ((ConstZero << 1) | 1));
		}
		longAtput((lit + BaseHeaderSize) + (2 << ShiftForWord), ((ConstZero << 1) | 1));
	} else {
		/* begin primitiveFail */
		foo->successFlag = 0;
	}
	...
	if ((ConstZero >= -1073741824) && (ConstZero <= 1073741823)) {
		...
		longAtput((lit + BaseHeaderSize) + (3 << ShiftForWord), ((ConstZero << 1) | 1));
	} else {
		/* begin primitiveFail */
		foo->successFlag = 0;
	}
...
register struct foo * foo = &fum;
	...
	/* begin storeInteger:ofObject:withValue: */
	if (((evtBuf[0]) >= -1073741824) && ((evtBuf[0]) <= 1073741823)) {
		/* begin storePointer:ofObject:withValue: */
		if (arg < foo->youngStart) {
			possibleRootStoreIntovalue(arg, (((evtBuf[0]) << 1) | 1));
		}
	...
	/* begin storeInteger:ofObject:withValue: */
	if ((((evtBuf[1]) & MillisecondClockMask) >= -1073741824) && (((evtBuf[1]) & MillisecondClockMask) <= 1073741823)) {
		/* begin storePointer:ofObject:withValue: */
		if (arg < foo->youngStart) {
			possibleRootStoreIntovalue(arg, ((((evtBuf[1]) & MillisecondClockMask) << 1) | 1));
		}
	...
	for (i = 2; i <= 7; i += 1) {
		...
		if ((value >= -1073741824) && (value <= 1073741823)) {
			...
			if ((value >= -1073741824) && (value <= 1073741823)) {
				/* begin storePointer:ofObject:withValue: */
				if (arg < foo->youngStart) {
					possibleRootStoreIntovalue(arg, ((value << 1) | 1));
				}
...
register struct foo * foo = &fum;
	...
	/* begin pushInteger: */
	/* begin push: */
	longAtput(sp = foo->stackPointer + BytesPerWord, ((0 << 1) | 1));
	foo->stackPointer = sp;
}
...
register struct foo * foo = &fum;
	...
	while (segOop <= endSeg) {
		...
		while (!(fieldPtr > lastPtr)) {
			...
			if ((fieldOop & 1)) {
				...
				if ((fieldOop & 2147483648ULL) == 0) {
					mapOop = fieldOop + segmentWordArray;
				} else {
					outPtr = outPointerArray + (fieldOop & 2147483647ULL);
					if (outPtr > lastOut) {
						/* begin primitiveFail */
						foo->successFlag = 0;
						return null;
	...
	/* begin pop:thenPush: */
	longAtput(sp = foo->stackPointer - ((3 - 1) * BytesPerWord), ((segmentWordArray + BaseHeaderSize) + BytesPerWord) + (headerTypeBytes[(longAt((segmentWordArray + BaseHeaderSize) + BytesPerWord)) & TypeMask]));
	foo->stackPointer = sp;
}
...
register struct foo * foo = &fum;
	...
	/* begin pop2AndPushIntegerIfOK: */
	if (foo->successFlag) {
		if ((mod >= -1073741824) && (mod <= 1073741823)) {
			/* begin pop:thenPush: */
...
register struct foo * foo = &fum;
	...
	if (foo->successFlag) {
		...
		if ((index >= -1073741824) && (index <= 1073741823)) {
			/* begin storePointer:ofObject:withValue: */
			if (stream < foo->youngStart) {
				possibleRootStoreIntovalue(stream, ((index << 1) | 1));
			}
...
l7:	/* end stackObjectValue: */;
	...
	while (scanLastIndex <= scanStopIndex) {
		...
		if (!((stopReason = longAt((stops + BaseHeaderSize) + (ascii << ShiftForWord))) == nilOop)) {
			if (!((scanDestX >= -1073741824) && (scanDestX <= 1073741823))) {
				/* begin primitiveFail */
				foo->successFlag = 0;
				return null;
			}
			...
			if ((scanDestX >= -1073741824) && (scanDestX <= 1073741823)) {
				/* begin storePointer:ofObject:withValue: */
				if (rcvr < foo->youngStart) {
					possibleRootStoreIntovalue(rcvr, ((scanDestX << 1) | 1));
				}
			...
			/* begin storeInteger:ofObject:withValue: */
			if ((scanLastIndex >= -1073741824) && (scanLastIndex <= 1073741823)) {
				/* begin storePointer:ofObject:withValue: */
				if (rcvr < foo->youngStart) {
					possibleRootStoreIntovalue(rcvr, ((scanLastIndex << 1) | 1));
				}
	...
	/* begin storeInteger:ofObject:withValue: */
	if ((scanStopIndex >= -1073741824) && (scanStopIndex <= 1073741823)) {
		/* begin storePointer:ofObject:withValue: */
		if (rcvr < foo->youngStart) {
			possibleRootStoreIntovalue(rcvr, ((scanStopIndex << 1) | 1));
		}
...
register struct foo * foo = &fum;
	...
	/* begin pushInteger: */
	/* begin push: */
	longAtput(sp = foo->stackPointer + BytesPerWord, ((depth << 1) | 1));
	foo->stackPointer = sp;
}
...
register struct foo * foo = &fum;
	...
	/* begin storePointer:ofObject:withValue: */
	if (pointResult < foo->youngStart) {
		possibleRootStoreIntovalue(pointResult, ((((((usqInt) pointWord) >> 16) & 65535) << 1) | 1));
	}
			...
			if ((fieldOop & 1)) {
				...
				if ((header & TypeMask) == HeaderTypeFree) {
					...
					if (((longAt(fieldOop)) & MarkBit) == 0) {
						...
						mapOop = (lastOut - outPointerArray) | 2147483648ULL;
						/* begin forward:to:savingOopAt:andHeaderAt: */
...
l2:	/* end checkedIntegerValueOf: */;
	...
	/* begin success: */
	foo->successFlag = (newStackp <= ((((sqInt) (LargeContextSize - BaseHeaderSize) >> 3)) - CtxtTempFrameStart)) && foo->successFlag;
	if (!(foo->successFlag)) {
		/* begin primitiveFail */
		foo->successFlag = 0;
		return null;
...
l1:	/* end fetchStackPointerOf: */;
	...
	/* begin storePointer:ofObject:withValue: */
	if (ctxt < foo->youngStart) {
		possibleRootStoreIntovalue(ctxt, ((newStackp << 1) | 1));
	}
	...
	if (foo->argumentCount == 0) {
		...
		for (i = 0; i <= (paramsArraySize - 1); i += 1) {
			...
			if (result < foo->youngStart) {
				possibleRootStoreIntovalue(result, ((0 << 1) | 1));
			}
			longAtput((result + BaseHeaderSize) + (i << ShiftForWord), ((0 << 1) | 1));
		}
		/* begin storePointer:ofObject:withValue: */
		...
		longAtput((result + BaseHeaderSize) + (26 << ShiftForWord), ((BytesPerWord << 1) | 1));
		/* begin pop:thenPush: */
		longAtput(sp = foo->stackPointer - ((1 - 1) * BytesPerWord), result);
		foo->stackPointer = sp;
		return null;
...
l1:	/* end checkedIntegerValueOf: */;
	...
	if (foo->successFlag) {
		...
		if (blockContext < foo->youngStart) {
			possibleRootStoreIntovalue(blockContext, ((value << 1) | 1));
		}
		longAtput((blockContext + BaseHeaderSize) + (StackPointerIndex << ShiftForWord), ((value << 1) | 1));
...
l2:	/* end argumentCountOfBlock: */;
	...
	if (foo->successFlag) {
		...
		/* begin storePointer:ofObject:withValue: */
		if (blockContext < foo->youngStart) {
			possibleRootStoreIntovalue(blockContext, ((arrayArgumentCount << 1) | 1));
		}
...
l1:	/* end assertClassOf:is: */;
	if (foo->successFlag) {
		...
		if (excessSignals > 0) {
			/* begin storeInteger:ofObject:withValue: */
			if (((excessSignals - 1) >= -1073741824) && ((excessSignals - 1) <= 1073741823)) {
				/* begin storePointer:ofObject:withValue: */
				if (sema < foo->youngStart) {
					possibleRootStoreIntovalue(sema, (((excessSignals - 1) << 1) | 1));
				}
...
register struct foo * foo = &fum;
	...
	if (swapBytes) {
		...
		byteSwapByteObjectsFromto(memory + (headerTypeBytes[(longAt(memory)) & TypeMask]), foo->endOfMemory);
	}
...
register struct foo * foo = &fum;
	...
	for (ptr = (dispBitsPtr + (startIndex * 4)); ptr <= (dispBitsPtr + (endIndex * 4)); ptr += 4) {
		reversed = (long32At(ptr)) ^ 4294967295ULL;
...
register struct foo * foo = &fum;
	...
	activeProc = longAt(((longAt(((longAt((foo->specialObjectsOop + BaseHeaderSize) + (SchedulerAssociation << ShiftForWord))) + BaseHeaderSize) + (ValueIndex << ShiftForWord))) + BaseHeaderSize) + (ActiveProcessIndex << ShiftForWord));
	/* begin storePointer:ofObject:withValue: */
	valuePointer2 = foo->activeContext;
	if (activeProc < foo->youngStart) {
		possibleRootStoreIntovalue(activeProc, valuePointer2);
	}
	longAtput((activeProc + BaseHeaderSize) + (SuspendedContextIndex << ShiftForWord), valuePointer2);
	incrementalGC();
	fullGC();
...
register struct foo * foo = &fum;
	if ((integerValue >= -1073741824) && (integerValue <= 1073741823)) {
		/* begin storePointer:ofObject:withValue: */
		if (objectPointer < foo->youngStart) {
			possibleRootStoreIntovalue(objectPointer, ((integerValue << 1) | 1));
		}
...
register struct foo * foo = &fum;
	...
	if ((longAt((aSemaphore + BaseHeaderSize) + (FirstLinkIndex << ShiftForWord))) == foo->nilObj) {
		...
		if (((excessSignals + 1) >= -1073741824) && ((excessSignals + 1) <= 1073741823)) {
			/* begin storePointer:ofObject:withValue: */
			if (aSemaphore < foo->youngStart) {
				possibleRootStoreIntovalue(aSemaphore, (((excessSignals + 1) << 1) | 1));
			}
...
register struct foo * foo = &fum;
	...
	if (foo->compilerInitialized) {
		...
		activeCntx = foo->activeContext;
		/* begin storePointer:ofObject:withValue: */
		valuePointer2 = ((((foo->instructionPointer - foo->method) - (BaseHeaderSize - 2)) << 1) | 1);
		if (activeCntx < foo->youngStart) {
			possibleRootStoreIntovalue(activeCntx, valuePointer2);
		}
		longAtput((activeCntx + BaseHeaderSize) + (InstructionPointerIndex << ShiftForWord), valuePointer2);
		/* begin storePointer:ofObject:withValue: */
		valuePointer1 = (((((((usqInt) ((foo->stackPointer - foo->activeContext) - BaseHeaderSize)) >> ShiftForWord) - TempFrameStart) + 1) << 1) | 1);
		if (activeCntx < foo->youngStart) {
			possibleRootStoreIntovalue(activeCntx, valuePointer1);
		...
		/* begin storePointer:ofObject:withValue: */
		valuePointer3 = foo->nilObj;
		if (newProc < foo->youngStart) {
			possibleRootStoreIntovalue(newProc, valuePointer3);
		}
		longAtput((newProc + BaseHeaderSize) + (SuspendedContextIndex << ShiftForWord), valuePointer3);
	}
	foo->reclaimableContextCount = 0;
}
