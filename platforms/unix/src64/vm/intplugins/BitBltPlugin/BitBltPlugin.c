/* Automatically generated from Squeak on #(31 March 2005 3:49:08 pm) */
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"BitBltPlugin 31 March 2005 (i)"
#else
	"BitBltPlugin 31 March 2005 (e)"
#endif
;
static sqInt nWords;
		...
		while ((deltaX -= 1) != 0) {
			...
			if (srcAlpha == 255) {
				...
				if (!(srcAlpha == 0)) {
					...
					destWord = ((((usqInt) (destWord & 31744) << 9)) | (((usqInt) (destWord & 992) << 6))) | ((((usqInt) (destWord & 31) << 3)) | 4278190080ULL);
		...
		while ((deltaX -= 1) != 0) {
			...
			if (destMSB) {
				if (srcShift == 0) {
					...
					srcShift -= 8;
					dstMask = (((usqInt) dstMask) >> 8) | 4278190080ULL;
				}
			} else {
				if (srcShift == 32) {
					dstIndex += 4;
					srcShift = 0;
					dstMask = 4294967040ULL;
				} else {
					srcShift += 8;
		...
		do {
			...
			if (mapperFlags == (ColorMapPresent | ColorMapIndexedPart)) {
				do {
					...
					if (!(((srcShift1 += srcShiftInc) & 4294967264ULL) == 0)) {
						if (sourceMSB) {
							srcShift1 += 32;
						} else {
							srcShift1 -= 32;
...
static sqInt fixAlphawith(sqInt sourceWord, sqInt destinationWord) {
	...
	if (!((destinationWord & 4278190080ULL) == 0)) {
		return destinationWord;
	}
	return destinationWord | (sourceWord & 4278190080ULL);
}
...
static sqInt isIdentityMapwith(int *shifts, unsigned int *masks) {
	...
	if (((shifts[RedIndex]) == 0) && (((shifts[GreenIndex]) == 0) && (((shifts[BlueIndex]) == 0) && (((shifts[AlphaIndex]) == 0) && (((masks[RedIndex]) == 16711680) && (((masks[GreenIndex]) == 65280) && (((masks[BlueIndex]) == 255) && ((masks[AlphaIndex]) == 4278190080ULL)))))))) {
		return 1;
	}
	return 0;
}
...
static sqInt pickSourcePixelsflagssrcMaskdestMasksrcShiftIncdstShiftInc(sqInt nPixels, sqInt mapperFlags, sqInt srcMask, sqInt dstMask, sqInt srcShiftInc, sqInt dstShiftInc) {
	...
	if (mapperFlags == (ColorMapPresent | ColorMapIndexedPart)) {
		do {
			...
			if (!(((srcShift += srcShiftInc) & 4294967264ULL) == 0)) {
				if (sourceMSB) {
					srcShift += 32;
				} else {
					srcShift -= 32;
