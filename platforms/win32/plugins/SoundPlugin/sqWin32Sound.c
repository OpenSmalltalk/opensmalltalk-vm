...
static BOOL stringMatch(LPCSTR a, LPCSTR b)  /* same as !strcmp(a, b), but dealing with near/far and char/wchar */
{ 
	if (!a || !b) return FALSE;
	while (*a && *b)
		if (*a++ != *b++) return FALSE;
...
static BOOL stringMatchIn(LPCSTR a, LPCSTR b) /* same as !!strstr(a, b), but dealing with near/far and char/wchar */
{
	if (!a || !b) return FALSE;
	while (*a && *b)
		if (*a++ != *b++) return FALSE;
...
{ 
	if (!p) return "";
	sprintf(GUIDString, "{%lx, %x, %x, %x %x %x %x %x %x %x %x}@%lx",
...
void setDeviceName(DeviceInfo* device, LPCTSTR lpszDesc)
{
	...
	if (lpszDesc == NULL) { return; }
	/* Copy the device name into a new string. */
	newNameLength = strlen(lpszDesc)+1; /* leave room for terminating null character */
	device->name = (char*)malloc(newNameLength);
	if (!device->name) {
		/* XXXXX: Not sure what implications this would have, nor how to handle it properly. */
		DPRINTF(("ERROR: setDeviceName() cannot allocate space to copy name: %s\n", lpszDesc));
		return;
	}
	strcpy(device->name, lpszDesc);
}
...
void setDeviceGUID(DeviceInfo* device, LPGUID lpGUID)
{
	...
	if (lpGUID == NULL) { return; }
	/* Instantiate a new GUID, and copy the old one into it. */
	device->guid = (LPGUID)malloc(sizeof(GUID));
	if (!device->guid) {
		/* XXXXX: Not sure what implications this would have, nor how to handle it properly. */
...
void clearDeviceInfo(DeviceInfo* device)
{
...
void initializeDeviceInfoList(DeviceInfoList* list) 
{
	int i;
...
void shutdownDeviceInfoList(DeviceInfoList* list)
{
	int i;
	for(i=0; i < DEVICE_MAX; i++) {
...
MMRESULT flexMixerGetLineInfo(HMIXEROBJ hmxobj, LPMIXERLINE pmxl)
{
	...
	if (mmResult != MIXERR_INVALLINE) return mmResult;
	/* Output devices */
	if (pmxl->dwComponentType == MIXERLINE_COMPONENTTYPE_DST_SPEAKERS) pmxl->dwComponentType = MIXERLINE_COMPONENTTYPE_DST_HEADPHONES;
	else if (pmxl->dwComponentType == MIXERLINE_COMPONENTTYPE_DST_HEADPHONES) pmxl->dwComponentType = MIXERLINE_COMPONENTTYPE_DST_MONITOR;
	else if (pmxl->dwComponentType == MIXERLINE_COMPONENTTYPE_DST_MONITOR) pmxl->dwComponentType = MIXERLINE_COMPONENTTYPE_DST_DIGITAL;
	/* Input devices */
	else if (pmxl->dwComponentType == MIXERLINE_COMPONENTTYPE_DST_WAVEIN) pmxl->dwComponentType = MIXERLINE_COMPONENTTYPE_DST_LINE;
	else if (pmxl->dwComponentType == MIXERLINE_COMPONENTTYPE_DST_LINE) pmxl->dwComponentType = MIXERLINE_COMPONENTTYPE_DST_VOICEIN;
	else return mmResult;
	/* Now try again. */
...
char *getSoundPlayerDeviceName(int index)
{
	return playerDevices.devices[index].name;
} 

char *getSoundRecorderDeviceName(int index)
{
	return recorderDevices.devices[index].name;
} 
...
void setDefaultSoundPlayer(char *deviceName)
{
	...
	for (counter=0; counter<max; counter++) {
		DeviceInfo* device = &(playerDevices.devices[counter]);
		if (stringMatch(device->name, deviceName)) {
			DPRINTF(("setDefaultSoundPlayer('%s')\n", deviceName));
			setDeviceName(&playerDevices.defaultDevice, device->name);  
...
void setDefaultSoundRecorder(char *deviceName)
{
	...
	for (counter=0; counter<max; counter++) {
		...
		if (stringMatch(device->name, deviceName)) {
			...
			setDeviceName(&recorderDevices.defaultDevice, device->name);
...
{
	if (playerDevices.defaultDevice.name != NULL) {
...
{
	if (recorderDevices.defaultDevice.name != NULL) {
		// Force enumeration to verify that default device is still available.
...
void logDeviceNames()
{
	...
	deviceLogFile = fopen(logName, "wt");
	if (!deviceLogFile) {
...
static double closeDevs(HMIXER hmx, HWAVEIN hwaveIn, HWAVEOUT hwaveOut)
{ 
...
double accessMixerVolume(UINT deviceID, DWORD lineType, double inLevel) 
{
	double outLevel;
	UINT v;
...
{ return((int)(1000.0L * accessMixerVolume(recorderDevices.defaultDevice.mmID, MIXERLINE_COMPONENTTYPE_DST_WAVEIN, level / 1000.0L))); }

int snd_GetRecordLevel() 
{ return((int)(1000.0L * accessMixerVolume(recorderDevices.defaultDevice.mmID, MIXERLINE_COMPONENTTYPE_DST_WAVEIN, -1.0))); }
...
{ *left = accessMixerVolume(playerDevices.defaultDevice.mmID, MIXERLINE_COMPONENTTYPE_DST_SPEAKERS, -1.0); *right = *left; }
...
int dx_snd_StartRecording(int samplesPerSec, int stereo, int semaIndex) {
	...
	if(FAILED(hRes)) {
		...
		char* isCertified = (dsccaps.dwFlags & DSCCAPS_CERTIFIED) ? "yes" : "no";
		char* isEmulated = (dsccaps.dwFlags & DSCCAPS_EMULDRIVER) ? "yes" : "no";
		char* hasMulticapture = (dsccaps.dwFlags & DSCCAPS_MULTIPLECAPTURE) ? "yes" : "no";
...
int snd_StartRecording(int desiredSamplesPerSec, int stereo, int semaIndex) {
	if (AEC_ENABLED) {
		return aec_snd_StartRecording(desiredSamplesPerSec, stereo, semaIndex);
	}
	else {
...
double snd_GetRecordingSampleRate(void) {
	if (AEC_ENABLED) {
		return aec_snd_GetRecordingSampleRate();
	}
	else {
...
void snd_Volume(double *left, double *right) {
   dx_snd_Volume(left, right);
}

void snd_SetVolume(double left, double right) {
   dx_snd_SetVolume(left, right);
}
