/* Automatically generated from Squeak on 15 September 2012 4:47:52 pm 
   by VMMaker 4.10.2
 */
#include "sq.h"
#include <setjmp.h>
...
sqInt activateNewClosureMethod(sqInt blockClosure) {
	...
	valuePointer = ((((instructionPointer - method) - ((BASE_HEADER_SIZE) - 2)) << 1) | 1);
	longAtput((activeCntx + (BASE_HEADER_SIZE)) + (InstructionPointerIndex << (SHIFT_FOR_WORD)), valuePointer);
	/* begin storePointerUnchecked:ofObject:withValue: */
	valuePointer1 = (((((((usqInt) ((stackPointer - activeContext) - (BASE_HEADER_SIZE))) >> (SHIFT_FOR_WORD)) - TempFrameStart) + 1) << 1) | 1);
	...
	method = longAt((tmp + (BASE_HEADER_SIZE)) + (MethodIndex << (SHIFT_FOR_WORD)));
	tmp = ((longAt((newContext + (BASE_HEADER_SIZE)) + (InstructionPointerIndex << (SHIFT_FOR_WORD)))) >> 1);
	instructionPointer = ((method + tmp) + (BASE_HEADER_SIZE)) - 2;
	tmp = ((longAt((newContext + (BASE_HEADER_SIZE)) + (StackPointerIndex << (SHIFT_FOR_WORD)))) >> 1);
	stackPointer = (newContext + (BASE_HEADER_SIZE)) + (((TempFrameStart + tmp) - 1) * (BYTES_PER_WORD));
}
...
sqInt allocateOrRecycleContext(sqInt needsLarge) {
	...
	if (needsLarge == 0) {
		cntxt = instantiateContextsizeInBytes(fetchPointerofObject(ClassMethodContext, specialObjectsOop), SMALL_CONTEXT_SIZE);
	} else {
		cntxt = instantiateContextsizeInBytes(fetchPointerofObject(ClassMethodContext, specialObjectsOop), LARGE_CONTEXT_SIZE);
	}
	/* begin storePointerUnchecked:ofObject:withValue: */
	valuePointer = nilObj;
...
sqInt checkForInterrupts(void) {
	...
	if (pendingFinalizationSignals > 0) {
		...
		if ((fetchClassOf(sema)) == (fetchPointerofObject(ClassSemaphore, specialObjectsOop))) {
			synchronousSignal(sema);
		}
		...
		if (semaphoresUseBufferA) {
			for (i = 1; i <= semaphoresToSignalCountB; i += 1) {
				...
				if (index <= xSize) {
					...
					if ((fetchClassOf(sema1)) == (fetchPointerofObject(ClassSemaphore, specialObjectsOop))) {
...
l2:	/* end sufficientSpaceToAllocate: */;
	if (!(enoughSpace)) {
		...
		if (lastSavedProcess == nilObj) {
			...
			currentProc = longAt((sched + (BASE_HEADER_SIZE)) + (ActiveProcessIndex << (SHIFT_FOR_WORD)));
			/* begin storePointer:ofObject:withValue: */
			oop11 = specialObjectsOop;
			if ((((usqInt) oop11)) < (((usqInt) youngStart))) {
...
sqInt commonAt(sqInt stringy) {
	...
	if ((messageSelector == (fetchPointerofObject(16 * 2, fetchPointerofObject(SpecialSelectors, specialObjectsOop)))) && (lkupClass == (fetchClassOfNonInt(rcvr)))) {
...
sqInt commonAtPut(sqInt stringy) {
	...
	if ((messageSelector == (fetchPointerofObject(17 * 2, fetchPointerofObject(SpecialSelectors, specialObjectsOop)))) && (lkupClass == (fetchClassOfNonInt(rcvr)))) {
...
sqInt createActualMessageTo(sqInt aClass) {
	...
	argumentArray = instantiateClassindexableSize(fetchPointerofObject(ClassArray, specialObjectsOop), argumentCount);
	/* begin pushRemappableOop: */
	remapBuffer[(remapBufferCount += 1)] = argumentArray;
	message = instantiateClassindexableSize(fetchPointerofObject(ClassMessage, specialObjectsOop), 0);
	/* begin popRemappableOop */
	oop = remapBuffer[remapBufferCount];
	remapBufferCount -= 1;
	argumentArray = oop;
	...
	if (compilerInitialized) {
		...
		/* begin transfer:from:to: */
		count = argumentCount;
		src = stackPointer - ((argumentCount - 1) * (BYTES_PER_WORD));
		flag("Dan");
		...
		if (!((weakOop == nilObj) || (((weakOop & 1)) || (weakOop < youngStart)))) {
			...
			if (oopGone) {
				...
				if (nonWeakCnt >= 2) {
					...
					if ((fetchClassOf(listOop)) == (fetchPointerofObject(ClassWeakFinalizer, specialObjectsOop))) {
...
sqInt floatObjectOf(double  aFloat) {
	...
	flag("Dan");
	newFloatObj = instantiateSmallClasssizeInBytes(fetchPointerofObject(ClassFloat, specialObjectsOop), 8 + (BASE_HEADER_SIZE));
...
l1:	/* end fullCompaction */;
	...
	/* begin statpendingFinalizationSignals: */
...
sqInt fwdTableInit(sqInt blkSize) {
	...
	endOfMemory = freeBlock + (BASE_HEADER_SIZE);
	/* begin setSizeOfFree:to: */
	longAtput(endOfMemory, ((BASE_HEADER_SIZE) & (ALL_BUT_TYPE_MASK)) | HeaderTypeFree);
	fwdTableNext = ((endOfMemory + (BASE_HEADER_SIZE)) + 7) & ((WORD_MASK) - 7);
	flag("Dan");
...
sqInt imageSegmentVersion(void) {
	...
	wholeWord = longAt((fetchPointerofObject(SelectorDoesNotUnderstand, specialObjectsOop)) + (BASE_HEADER_SIZE));
...
sqInt initializeInterpreter(sqInt bytesToShift) {
	...
	activeContext = longAt((proc + (BASE_HEADER_SIZE)) + (SuspendedContextIndex << (SHIFT_FOR_WORD)));
	...
	stackPointer = (activeCntx + (BASE_HEADER_SIZE)) + (((TempFrameStart + tmp) - 1) * (BYTES_PER_WORD));
...
sqInt initializeObjectMemory(sqInt bytesToShift) {
	...
	/* begin fetchPointer:ofObject: */
	oop = specialObjectsOop;
	nilObj = longAt((oop + (BASE_HEADER_SIZE)) + (NilObject << (SHIFT_FOR_WORD)));
	/* begin fetchPointer:ofObject: */
	oop1 = specialObjectsOop;
	falseObj = longAt((oop1 + (BASE_HEADER_SIZE)) + (FalseObject << (SHIFT_FOR_WORD)));
	/* begin fetchPointer:ofObject: */
	oop2 = specialObjectsOop;
	trueObj = longAt((oop2 + (BASE_HEADER_SIZE)) + (TrueObject << (SHIFT_FOR_WORD)));
	rootTableCount = 0;
		...
		commonReturn:
			/* commonReturn */
			{
				...
				if ((localCntx == nilOop) || ((longAt((localCntx + (BASE_HEADER_SIZE)) + (InstructionPointerIndex << (SHIFT_FOR_WORD)))) == nilOop)) {
					...
					/* begin fetchPointer:ofObject: */
					oop1 = specialObjectsOop;
					messageSelector = longAt((oop1 + (BASE_HEADER_SIZE)) + (SelectorCannotReturn << (SHIFT_FOR_WORD)));
					argumentCount = 1;
					goto normalSend;
					goto l42;
				}
				...
				while (!(thisCntx == localCntx)) {
					if (thisCntx == nilOop) {
						...
						/* begin fetchPointer:ofObject: */
						oop2 = specialObjectsOop;
						messageSelector = longAt((oop2 + (BASE_HEADER_SIZE)) + (SelectorCannotReturn << (SHIFT_FOR_WORD)));
						argumentCount = 1;
						goto normalSend;
						goto l42;
					}
				...
				l43:	/* end isUnwindMarked: */;
					if (unwindMarked) {
						...
						/* begin fetchPointer:ofObject: */
						oop3 = specialObjectsOop;
						messageSelector = longAt((oop3 + (BASE_HEADER_SIZE)) + (SelectorAboutToReturn << (SHIFT_FOR_WORD)));
						argumentCount = 2;
						goto normalSend;
						goto l42;
					}
						...
						if ((((usqInt) thisCntx)) >= (((usqInt) youngStart))) {
							...
							if (((((usqInt) header1) >> 12) & 31) == 14) {
								...
								if ((header1 & (SIZE_MASK)) == (LARGE_CONTEXT_SIZE)) {
									/* begin storePointerUnchecked:ofObject:withValue: */
									valuePointer1 = freeLargeContexts;
...
;
		...
		case 131:
			/* singleExtendedSendBytecode */
			{
				...
				/* begin literal:ofMethod: */
				methodPointer = method;
				messageSelector = longAt((methodPointer + (BASE_HEADER_SIZE)) + (((descriptor & 31) + LiteralStart) << (SHIFT_FOR_WORD)));
		...
		normalSend:
			/* normalSend */
			{
				...
				if ((rcvr & 1)) {
					/* begin fetchPointer:ofObject: */
					oop = specialObjectsOop;
					lkupClass = longAt((oop + (BASE_HEADER_SIZE)) + (ClassInteger << (SHIFT_FOR_WORD)));
					goto l44;
				}
				ccIndex = (((usqInt) (longAt(rcvr))) >> 12) & 31;
				if (ccIndex == 0) {
...
;
		...
		commonSend:
			...
			l45:	/* end lookupInMethodCacheSel:class: */;
				...
				activeCntx = activeContext;
				/* begin storePointerUnchecked:ofObject:withValue: */
				valuePointer = (((((oopForPointer(localIP)) + 2) - (method + (BASE_HEADER_SIZE))) << 1) | 1);
				longAtput((activeCntx + (BASE_HEADER_SIZE)) + (InstructionPointerIndex << (SHIFT_FOR_WORD)), valuePointer);
				/* begin storePointerUnchecked:ofObject:withValue: */
		...
		case 132:
			/* doubleExtendedDoAnythingBytecode */
			{
				...
				if (opType == 0) {
					/* begin literal:ofMethod: */
					methodPointer = method;
					messageSelector = longAt((methodPointer + (BASE_HEADER_SIZE)) + ((byte3 + LiteralStart) << (SHIFT_FOR_WORD)));
				...
				if (opType == 1) {
					/* begin literal:ofMethod: */
					methodPointer1 = method;
					messageSelector = longAt((methodPointer1 + (BASE_HEADER_SIZE)) + ((byte3 + LiteralStart) << (SHIFT_FOR_WORD)));
					argumentCount = byte2 & 31;
		...
		commonSupersend:
			/* superclassSend */
			{
				...
				if ((rcvr & 1)) {
					/* begin fetchPointer:ofObject: */
					oop = specialObjectsOop;
					receiverClass = longAt((oop + (BASE_HEADER_SIZE)) + (ClassInteger << (SHIFT_FOR_WORD)));
					goto l47;
				}
				ccIndex = (((usqInt) (longAt(rcvr))) >> 12) & 31;
				if (ccIndex == 0) {
...
;
		...
		case 134:
			/* secondExtendedSendBytecode */
			{
				...
				/* begin literal:ofMethod: */
				methodPointer = method;
				messageSelector = longAt((methodPointer + (BASE_HEADER_SIZE)) + (((descriptor & 63) + LiteralStart) << (SHIFT_FOR_WORD)));
...
;
		...
		case 138:
			/* pushNewArrayBytecode */
			{
				...
				stackPointer = oopForPointer(localSP);
				theHomeContext = localHomeContext;
				array = instantiateClassindexableSize(fetchPointerofObject(ClassArray, specialObjectsOop), size);
				/* begin internalizeIPandSP */
				localIP = pointerForOop(instructionPointer);
				localSP = pointerForOop(stackPointer);
				localHomeContext = theHomeContext;
...
;
		...
		case 143:
			/* pushClosureCopyCopiedValuesBytecode */
			{
				...
				newClosure1 = instantiateSmallClasssizeInBytes(fetchPointerofObject(ClassBlockClosure, specialObjectsOop), ((BYTES_PER_WORD) * (ClosureFirstCopiedValueIndex + numCopied)) + (BASE_HEADER_SIZE));
			...
			/* shortConditionalJump */
			{
				...
				if (boolean == falseObj) {
					...
					if (!(boolean == trueObj)) {
						/* begin fetchPointer:ofObject: */
						oop = specialObjectsOop;
						messageSelector = longAt((oop + (BASE_HEADER_SIZE)) + (SelectorMustBeBoolean << (SHIFT_FOR_WORD)));
						argumentCount = 0;
						goto normalSend;
						goto l5;
					}
			...
			/* bytecodePrimMakePoint */
			{
				...
				if ((rcvr & 1)) {
					if ((argument & 1)) {
						...
						pointResult = instantiateSmallClasssizeInBytes(fetchPointerofObject(ClassPoint, specialObjectsOop), 3 * (BYTES_PER_WORD));
						...
						/* begin makePointwithxValue:yValue: */
						pointResult1 = instantiateSmallClasssizeInBytes(fetchPointerofObject(ClassPoint, specialObjectsOop), 3 * (BYTES_PER_WORD));
						...
						pt = pointResult1;
						/* begin storePointer:ofObject:withValue: */
						valuePointer = longAt(stackPointer - (0 * (BYTES_PER_WORD)));
					...
					if (!((fetchClassOf(rcvr)) == (fetchPointerofObject(ClassFloat, specialObjectsOop)))) {
						/* begin success: */
						if (!(0)) {
							if (!primFailCode) {
								primFailCode = 1;
					...
					/* begin makePointwithxValue:yValue: */
					pointResult2 = instantiateSmallClasssizeInBytes(fetchPointerofObject(ClassPoint, specialObjectsOop), 3 * (BYTES_PER_WORD));
		...
		case 200:
			/* bytecodePrimBlockCopy */
			{
				...
				l33:	/* end sizeBitsOf: */;
					...
					newContext = instantiateContextsizeInBytes(fetchPointerofObject(ClassBlockContext, specialObjectsOop), contextSize);
					/* begin popRemappableOop */
		...
		case 201:
			/* bytecodePrimValue */
			{
				...
				l35:	/* end fetchClassOfNonInt: */;
					if (rcvrClass == (fetchPointerofObject(ClassBlockClosure, specialObjectsOop))) {
						/* begin externalizeIPandSP */
						instructionPointer = oopForPointer(localIP);
						stackPointer = oopForPointer(localSP);
						theHomeContext = localHomeContext;
						...
						if (rcvrClass == (fetchPointerofObject(ClassBlockContext, specialObjectsOop))) {
							/* begin externalizeIPandSP */
							instructionPointer = oopForPointer(localIP);
							stackPointer = oopForPointer(localSP);
							theHomeContext = localHomeContext;
...
;
		...
		case 208:
			/* sendLiteralSelectorBytecode */
			{
				...
				/* begin literal:ofMethod: */
				methodPointer = method;
				messageSelector = longAt((methodPointer + (BASE_HEADER_SIZE)) + ((offset + LiteralStart) << (SHIFT_FOR_WORD)));
...
sqInt isFloatObject(sqInt oop) {
	return (fetchClassOf(oop)) == (fetchPointerofObject(ClassFloat, specialObjectsOop));
}
...
sqInt lookupMethodInClass(sqInt class) {
	...
	while (currentClass != nilObj) {
		...
		if (dictionary == nilObj) {
			...
			/* begin fetchPointer:ofObject: */
			oop1 = specialObjectsOop;
			messageSelector = longAt((oop1 + (BASE_HEADER_SIZE)) + (SelectorCannotInterpret << (SHIFT_FOR_WORD)));
	...
	l1:	/* end sizeBitsOf: */;
		...
		while (1) {
			...
			if (nextSelector == messageSelector) {
				...
				newMethod = longAt((methodArray + (BASE_HEADER_SIZE)) + ((index - SelectorStart) << (SHIFT_FOR_WORD)));
	...
	/* begin fetchPointer:ofObject: */
	oop3 = specialObjectsOop;
	messageSelector = longAt((oop3 + (BASE_HEADER_SIZE)) + (SelectorDoesNotUnderstand << (SHIFT_FOR_WORD)));
...
sqInt normalizeFloatOrderingInImage(void) {
	...
	l2:	/* end firstAccessibleObject */;
		while (!(oop == null)) {
			if (!(((longAt(oop)) & TypeMask) == HeaderTypeFree)) {
				if ((fetchClassOf(oop)) == (fetchPointerofObject(ClassFloat, specialObjectsOop))) {
...
l1:	/* end fetchClassOf: */;
	if ((c == (fetchPointerofObject(ClassMethodContext, specialObjectsOop))) || (c == (fetchPointerofObject(ClassBlockContext, specialObjectsOop)))) {
...
# endif  // SQ_HOST32
	...
	if ((BYTES_PER_WORD) == 4) {
		...
		newLargeInteger = instantiateSmallClasssizeInBytes(fetchPointerofObject(ClassLargePositiveInteger, specialObjectsOop), (BASE_HEADER_SIZE) + 4);
	} else {

		/* Cant use instantiateSmallClass: due to integral word requirement. */

		newLargeInteger = instantiateClassindexableSize(fetchPointerofObject(ClassLargePositiveInteger, specialObjectsOop), 4);
	}
...
sqInt positive64BitIntegerFor(sqLong integerValue) {
	...
	newLargeInteger = instantiateClassindexableSize(fetchPointerofObject(ClassLargePositiveInteger, specialObjectsOop), sz);
	for (i = 0; i <= (sz - 1); i += 1) {
...
sqInt postGCAction(void) {
	...
	if (oopisGreaterThan((longAt(freeBlock)) & (ALL_BUT_TYPE_MASK), shrinkThreshold)) {
...
sqInt primitiveChangeClass(void) {
	...
	/* begin stackObjectValue: */
	oop2 = longAt(stackPointer - (0 * (BYTES_PER_WORD)));
	if ((oop2 & 1)) {
		/* begin primitiveFail */
		if (primFailCode == 0) {
			primFailCode = 1;
...
l2:	/* end stackObjectValue: */;
	/* begin stackObjectValue: */
	oop3 = longAt(stackPointer - (1 * (BYTES_PER_WORD)));
	if ((oop3 & 1)) {
		/* begin primitiveFail */
		if (primFailCode == 0) {
			primFailCode = 1;
...
sqInt primitiveClipboardText(void) {
	...
	if (argumentCount == 1) {
		...
		s = instantiateClassindexableSize(fetchPointerofObject(ClassString, specialObjectsOop), sz);
...
l2:	/* end stackIntegerValue: */;
	...
	/* begin success: */
	successBoolean = (fetchClassOf(copiedValues)) == (fetchPointerofObject(ClassArray, specialObjectsOop));
	if (!(successBoolean)) {
		if (!primFailCode) {
			primFailCode = 1;
		}
...
l1:	/* end sizeBitsOf: */;
	...
	newClosure1 = instantiateSmallClasssizeInBytes(fetchPointerofObject(ClassBlockClosure, specialObjectsOop), ((BYTES_PER_WORD) * (ClosureFirstCopiedValueIndex + numCopiedValues)) + (BASE_HEADER_SIZE));
	longAtput((newClosure1 + (BASE_HEADER_SIZE)) + (ClosureStartPCIndex << (SHIFT_FOR_WORD)), ((initialIP << 1) | 1));
	longAtput((newClosure1 + (BASE_HEADER_SIZE)) + (ClosureNumArgsIndex << (SHIFT_FOR_WORD)), ((numArgs << 1) | 1));
	newClosure = newClosure1;
	/* begin storePointerUnchecked:ofObject:withValue: */
	valuePointer1 = longAt(stackPointer - (2 * (BYTES_PER_WORD)));
	longAtput((newClosure + (BASE_HEADER_SIZE)) + (ClosureOuterContextIndex << (SHIFT_FOR_WORD)), valuePointer1);
...
sqInt primitiveExecuteMethod(void) {
	...
	primitiveIndex = (primBits & 511) + (((usqInt) primBits) >> 19);
	/* begin success: */
	successBoolean = (argumentCount - 1) == (argumentCountOf(newMethod));
	if (!(successBoolean)) {
		if (!primFailCode) {
			primFailCode = 1;
		}
...
sqInt primitiveExternalCall(void) {
	...
	/* begin success: */
	successBoolean1 = (literalCountOf(newMethod)) > 0;
	if (!(successBoolean1)) {
		if (!primFailCode) {
			primFailCode = 1;
		}
...
sqInt primitiveFloatAddtoArg(sqInt rcvrOop, sqInt argOop) {
	...
	if ((fetchClassOfNonInt(rcvrOop)) == (fetchPointerofObject(ClassFloat, specialObjectsOop))) {
...
l1:	/* end loadFloatOrIntFrom: */;
	...
	if ((fetchClassOfNonInt(argOop)) == (fetchPointerofObject(ClassFloat, specialObjectsOop))) {
...
EXPORT(sqInt) primitiveInterpreterSourceVersion(void) {
	...
	len = strlen(cString);
	versionString = instantiateClassindexableSize(fetchPointerofObject(ClassString, specialObjectsOop), len);
	/* begin arrayValueOf: */
...
sqInt primitiveInterruptSemaphore(void) {
	...
	stackPointer -= BYTES_PER_WORD;
	arg = top;
	if ((fetchClassOf(arg)) == (fetchPointerofObject(ClassSemaphore, specialObjectsOop))) {
		/* begin storePointer:ofObject:withValue: */
		oop = specialObjectsOop;
		if ((((usqInt) oop)) < (((usqInt) youngStart))) {
...
sqInt primitiveInvokeObjectAsMethod(void) {
	...
	runArgs = instantiateClassindexableSize(fetchPointerofObject(ClassArray, specialObjectsOop), argumentCount);
	beRootIfOld(runArgs);
	/* begin transfer:from:to: */
	count = argumentCount;
	src = stackPointer - ((argumentCount - 1) * (BYTES_PER_WORD));
	...
	/* begin fetchPointer:ofObject: */
	oop = specialObjectsOop;
	messageSelector = longAt((oop + (BASE_HEADER_SIZE)) + (SelectorRunWithIn << (SHIFT_FOR_WORD)));
	argumentCount = 3;
	/* begin push: */
...
l1:	/* end stackIntegerValue: */;
	...
	nameOop = instantiateClassindexableSize(fetchPointerofObject(ClassString, specialObjectsOop), length);
	for (i = 0; i <= (length - 1); i += 1) {
...
l1:	/* end fetchClassOf: */;
	...
	if (((newMethod & 1) == 0) && (((((usqInt) (longAt(newMethod))) >> 8) & 15) >= 12)) {
		/* begin success: */
		successBoolean = (argumentCountOf(newMethod)) == argumentCount;
		if (!(successBoolean)) {
			if (!primFailCode) {
				primFailCode = 1;
			}
	...
	if (!primFailCode) {
		...
		for (i = 1; i <= argumentCount; i += 1) {
			...
			oop = activeContext;
			/* begin fetchPointer:ofObject: */
			fieldIndex1 = (argumentCount - i) + selectorIndex;
			oop4 = activeContext;
...
sqInt primitivePerformAt(sqInt lookupClass) {
	...
	if (((newMethod & 1) == 0) && (((((usqInt) (longAt(newMethod))) >> 8) & 15) >= 12)) {
		/* begin success: */
		successBoolean1 = (argumentCountOf(newMethod)) == argumentCount;
		if (!(successBoolean1)) {
			if (!primFailCode) {
				primFailCode = 1;
			}
...
EXPORT(sqInt) primitiveRootTable(void) {
	...
	oop = instantiateClassindexableSize(fetchPointerofObject(ClassArray, specialObjectsOop), sz);
...
l3:	/* end stackIntegerValue: */;
	...
	left = null;
l4:	/* end stackIntegerValue: */;
	displayBitsOfLeftTopRightBottom(fetchPointerofObject(TheDisplay, specialObjectsOop), left, top, right, bottom);
	if (!primFailCode) {
		ioForceDisplayUpdate();
		/* begin pop: */
...
sqInt primitiveStoreImageSegment(void) {
	...
	markAndTrace(lkupClass);
	markAndTrace(receiverClass);
	for (i4 = 1; i4 <= remapBufferCount; i4 += 1) {
		oop = remapBuffer[i4];
		if (!((oop & 1))) {
			markAndTrace(oop);
		}
	}
	for (i4 = 1; i4 <= jmpDepth; i4 += 1) {
		oop = suspendedCallbacks[i4];
		if (!((oop & 1))) {
			markAndTrace(oop);
		}
		oop = suspendedMethods[i4];
		if (!((oop & 1))) {
			markAndTrace(oop);
		}
	}
	...
	if (process == activeProc) {
		...
		if (process == firstLink) {
			...
			if (process == lastLink) {
				...
				valuePointer1 = nilObj;
				if ((((usqInt) myList)) < (((usqInt) youngStart))) {
					possibleRootStoreIntovalue(myList, valuePointer1);
				}
				longAtput((myList + (BASE_HEADER_SIZE)) + (LastLinkIndex << (SHIFT_FOR_WORD)), valuePointer1);
			}
		} else {
			tempLink = firstLink;
					while (1) {
		...
		if ((((usqInt) process)) < (((usqInt) youngStart))) {
			possibleRootStoreIntovalue(process, valuePointer2);
		}
		longAtput((process + (BASE_HEADER_SIZE)) + (NextLinkIndex << (SHIFT_FOR_WORD)), valuePointer2);
...
EXPORT(sqInt) primitiveUtcWithOffset(void) {
	...
	remapBuffer[(remapBufferCount += 1)] = oop;
	resultArray = instantiateClassindexableSize(fetchPointerofObject(ClassArray, specialObjectsOop), 2);
...
sqInt primitiveVMParameter(void) {
	...
	paramsArraySize = 40;
	if (argumentCount == 0) {
		result = instantiateClassindexableSize(fetchPointerofObject(ClassArray, specialObjectsOop), paramsArraySize);
		/* begin pushRemappableOop: */
		...
		/* begin storePointer:ofObject:withValue: */
		valuePointer10 = ((statRootTableOverflows << 1) | 1);
		if ((((usqInt) result)) < (((usqInt) youngStart))) {
		...
		/* begin storePointer:ofObject:withValue: */
		valuePointer24 = ((statSpecialMarkCount << 1) | 1);
		if ((((usqInt) result)) < (((usqInt) youngStart))) {
		...
		/* begin storePointer:ofObject:withValue: */
		valuePointer25 = ((statpendingFinalizationSignals << 1) | 1);
		if ((((usqInt) result)) < (((usqInt) youngStart))) {
...
l1:	/* end argumentCountOfBlock: */;
	...
	if (!primFailCode) {
		...
		activeCntx = activeContext;
		/* begin storePointerUnchecked:ofObject:withValue: */
		valuePointer2 = ((((instructionPointer - method) - ((BASE_HEADER_SIZE) - 2)) << 1) | 1);
		...
		stackPointer = (blockContext + (BASE_HEADER_SIZE)) + (((TempFrameStart + tmp) - 1) * (BYTES_PER_WORD));
	}
}
...
EXPORT(sqInt) printAllStacks(void) {
	...
	oop = memory + (headerTypeBytes[(longAt(memory)) & TypeMask]);
	while ((((usqInt) oop)) < (((usqInt) endOfMemory))) {
		if ((fetchClassOf(oop)) == (fetchPointerofObject(ClassSemaphore, specialObjectsOop))) {
			/* begin cr */
			printf("\n");
...
sqInt printCallStackOf(sqInt aContext) {
	...
	ctxt = aContext;
	while (!(ctxt == nilObj)) {
		if ((fetchClassOf(ctxt)) == (fetchPointerofObject(ClassBlockContext, specialObjectsOop))) {
...
sqInt readImageFromFileHeapSizeStartingAt(sqImageFile  f, usqInt desiredHeapSize, squeakFileOffsetType  imageOffset) {
	...
	if (((memory = allocateMemoryMinimumImageFileHeaderSize(heapSize, minimumMemory, f, headerSize))) == null) {
...
EXPORT(sqInt) reestablishContextPriorToCallback(sqInt callbackContext) {
	...
	if ((fetchClassOf(callbackContext)) != (fetchPointerofObject(ClassMethodContext, specialObjectsOop))) {
		return 0;
	}
	...
	stackPointer = (calloutContext + (BASE_HEADER_SIZE)) + (((TempFrameStart + tmp) - 1) * (BYTES_PER_WORD));
	return 1;
}
...
EXPORT(sqInt) sendInvokeCallbackStackRegistersJmpbuf(sqInt thunkPtr, sqInt stackPtr, sqInt regsPtr, sqInt jmpBufPtr) {
	...
	/* begin fetchPointer:ofObject: */
	oop4 = specialObjectsOop;
	receiver = longAt((oop4 + (BASE_HEADER_SIZE)) + (ClassAlien << (SHIFT_FOR_WORD)));
	/* begin fetchClassOfNonInt: */
	oop5 = receiver;
	ccIndex = (((usqInt) (longAt(oop5))) >> 12) & 31;
	if (ccIndex == 0) {
		lkupClass = (longAt(oop5 - (BASE_HEADER_SIZE))) & (ALL_BUT_TYPE_MASK);
		goto l1;
	} else {
		/* begin fetchPointer:ofObject: */
		/* begin fetchPointer:ofObject: */
		oop7 = specialObjectsOop;
		oop11 = longAt((oop7 + (BASE_HEADER_SIZE)) + (CompactClasses << (SHIFT_FOR_WORD)));
...
l1:	/* end fetchClassOfNonInt: */;
	/* begin fetchPointer:ofObject: */
	oop6 = specialObjectsOop;
	messageSelector = longAt((oop6 + (BASE_HEADER_SIZE)) + (InvokeCallbackSelector << (SHIFT_FOR_WORD)));
	...
	stackPointer = (activeCntx1 + (BASE_HEADER_SIZE)) + (((TempFrameStart + tmp) - 1) * (BYTES_PER_WORD));
	callInterpreter();
	return 1;
}
...
l1:	/* end fetchClassOf: */;
	if (largeClass == (fetchPointerofObject(ClassLargePositiveInteger, specialObjectsOop))) {
		negative = 0;
	} else {
		if (largeClass == (fetchPointerofObject(ClassLargeNegativeInteger, specialObjectsOop))) {
			negative = 1;
		} else {
			/* begin primitiveFail */
			if (primFailCode == 0) {
...
sqInt snapshot(sqInt embedded) {
	...
	if (compilerInitialized) {
		...
		activeCntx = activeContext;
		/* begin storePointerUnchecked:ofObject:withValue: */
		valuePointer1 = ((((instructionPointer - method) - ((BASE_HEADER_SIZE) - 2)) << 1) | 1);
		longAtput((activeCntx + (BASE_HEADER_SIZE)) + (InstructionPointerIndex << (SHIFT_FOR_WORD)), valuePointer1);
		/* begin storePointerUnchecked:ofObject:withValue: */
		valuePointer2 = (((((((usqInt) ((stackPointer - activeContext) - (BASE_HEADER_SIZE))) >> (SHIFT_FOR_WORD)) - TempFrameStart) + 1) << 1) | 1);
	...
	oop3 = memory + (headerTypeBytes[(longAt(memory)) & TypeMask]);
	while ((((usqInt) oop3)) < (((usqInt) endOfMemory))) {
		...
		/* begin objectAfter: */
		if (DoAssertionChecks) {
			if ((((usqInt) oop3)) >= (((usqInt) endOfMemory))) {
				error("no objects after the end of memory");
			}
		}
		if (((longAt(oop3)) & TypeMask) == HeaderTypeFree) {
		...
		oop3 = (oop3 + sz1) + (headerTypeBytes[(longAt(oop3 + sz1)) & TypeMask]);
	}
	/* begin clearRootsTable */
	for (i1 = 1; i1 <= rootTableCount; i1 += 1) {
		oop11 = rootTable[i1];
...
double stackFloatValue(sqInt offset) {
	...
	if (!((fetchClassOf(floatPointer)) == (fetchPointerofObject(ClassFloat, specialObjectsOop)))) {
		/* begin primitiveFail */
		if (primFailCode == 0) {
			primFailCode = 1;
		}
...
sqInt transferTo(sqInt aProc) {
	...
	if (compilerInitialized) {
		...
		longAtput((activeCntx + (BASE_HEADER_SIZE)) + (InstructionPointerIndex << (SHIFT_FOR_WORD)), valuePointer2);
		/* begin storePointerUnchecked:ofObject:withValue: */
		valuePointer11 = (((((((usqInt) ((stackPointer - activeContext) - (BASE_HEADER_SIZE))) >> (SHIFT_FOR_WORD)) - TempFrameStart) + 1) << 1) | 1);
		...
		stackPointer = (aContext + (BASE_HEADER_SIZE)) + (((TempFrameStart + tmp) - 1) * (BYTES_PER_WORD));
		/* begin storePointer:ofObject:withValue: */
		valuePointer1 = nilObj;
