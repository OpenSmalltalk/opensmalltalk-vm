...
!CCodeGeneratorGlobalStructure methodsFor: 'C code generator' stamp: 'JMM 4/17/2002 00:39'!
emitCVariablesOn: aStream
	"Store the global variable declarations on the given stream.
	...
	self buildSortedVariablesCollection do: [ :var |
		...
		target _ (self placeInStructure: var) 
			...
			ifFalse: [nonstruct].
		(self isGeneratingPluginCode and:[self isTranslatingLocally])
			ifTrue:[target nextPutAll:'static '].
		(variableDeclarations includesKey: varString) ifTrue: [
...
!Interpreter class methodsFor: 'translation' stamp: 'JMM 4/16/2002 22:42'!
...
them make better assembler"
	...
	| doInlining cg exports fileName tStamp fstat |
	...
	exports _ #vm -> cg exportedPrimitiveNames asArray.
...
!MacOSPowerPCOS9VMMaker methodsFor: 'generate sources' stamp: 'JMM 4/16/2002 22:34'!
generateInterpreterFile
	...
	"exp is nil if we skipped making the core vm because there seems to be 
	an uptodate file already. If the exports list doesn't yet include the vm, 
	make sure the vm exports list is generated"
	exp
		ifNil: [(self exportsIncludes: #vm)
				ifFalse: [self addExports: Interpreter generateExportsList]]
		ifNotNil: [self addExports: exp]! !
...
CCodeGenerator removeSelector: #buildSortedVariablesCollection!
CCodeGenerator removeSelector: #globalVariables!
CCodeGenerator removeSelector: #placeInStructure:!
