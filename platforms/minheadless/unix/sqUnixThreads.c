...
ioSignalOSSemaphore(sqOSSemaphore *sem)
{
	...
	if (++sem->count <= 0) {
		THRLOG(PRIdSQINT " pthread_cond_signal 0x%x\n", tid, sem);
		err = pthread_cond_signal(&sem->cond);
		THRLOG(PRIdSQINT " pthread_cond_signal 0x%x => %d\n", tid, sem, err);
	}
	else
		THRLOG(PRIdSQINT " ioSig 0x%x ++count = %d\n", tid, sem, sem->count);
...
ioWaitOnOSSemaphore(sqOSSemaphore *sem)
{
	...
	if (--sem->count < 0) {
		THRLOG(PRIdSQINT " pthread_cond_wait 0x%x\n", tid, sem);
		err = pthread_cond_wait(&sem->cond, &sem->mutex);
		THRLOG(PRIdSQINT " proceeding 0x%x (pcw err %d)\n", tid, sem, err);
	}
	else
		THRLOG(PRIdSQINT " ioWait 0x%x --count = %d\n", tid, sem, sem->count);
	if ((err = pthread_mutex_unlock(&sem->mutex)))
