...
int pointlessFunctionToTakeUpSpace(void) {
int i;
	fetchClassOf(i++);
	fetchClassOf(i++);
	fetchClassOf(i++);
...
int primitiveExecuteRawInstructionArray(void) {
	int rx, result;
	int (*rawFunction)(void);
	rx = stackValue(0);
	rawFunction = pointlessFunctionToTakeUpSpace/* (int (*)(void)) (rx +4)*/;
	memcpy((void*)rawFunction, (void*)(rx+4), (size_t)100);
	result = integerObjectOf(rawFunction());
