...
static NoDbgRegParms int compactIndexOfClass(sqInt objOop);
...
initSegmentBridgeWithBytesat(usqLong numBytes, sqInt address)
{
	...
	if (numSlots) {
		longAtput((void *)(address),((((usqInt)((numSlotsMask())) << (numSlotsFullShift())))) + numSlots);
		longAtput((void *)(address + BaseHeaderSize),(((((((usqInt)((numSlotsMask())) << (numSlotsFullShift())))) + (1U << (pinnedBitShift()))) + (1ULL << (markedBitFullShift()))) + ((((usqInt)((wordIndexableFormat())) << (formatShift()))))) + (segmentBridgePun()));
	}
	else {
		longAtput((void *)(address),(((1U << (pinnedBitShift())) + (1ULL << (markedBitFullShift()))) + ((((usqInt)((wordIndexableFormat())) << (formatShift()))))) + (segmentBridgePun()));
	}
}
...
setIsImmutableOfto(sqInt objOop, sqInt aBoolean)
{
	longAtput((void *)(objOop),(aBoolean
			...
			: (longAt((void *)(objOop))) & (~(usqIntptr_t)(1U << (immutableBitShift())))));
}
...
{
	return (((corpseBitsTimesAllocationUnit()) < 0) ? (((usqInt)(1)) >> (-(corpseBitsTimesAllocationUnit()))) : (1ULL << (corpseBitsTimesAllocationUnit())));
}
...
printHeaderOf(sqInt objOop)
{
	...
	if (hasOverflowHeader(objOop)) {
		...
		print(" slotc ");
		printHexnp(((((usqInt)(((sqInt)((usqInt)((longAt((void *)(objOop - BaseHeaderSize)))) << 8)))))) >> 8);
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (fmt <= (lastPointerFormat())) {
		if (isIntegerValue(integerValue)) {
			...
			assert((isNonImmediate(objOop))
			 && (!(isForwarded(objOop))));
			assert(validStorePointerUncheckedArgs(fieldIndex, objOop, (((usqInt)integerValue << 3) | 1)));
			longAtput((void *)((objOop + BaseHeaderSize) + ((((usqInt)(fieldIndex) << (shiftForWord()))))),(((usqInt)integerValue << 3) | 1));
		}
		else {
			/* primitiveFailFor: */
			GIV(primFailCode) = PrimErrInappropriate;
