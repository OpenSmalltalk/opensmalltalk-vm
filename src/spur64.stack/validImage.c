/* Automatically generated by
   ...
   from
	ImageLeakChecker VMMaker.oscog-eem.3199 uuid: 3866dedc-ccfd-4fc1-a487-85095ada8bba
 */
static char __buildInfo[] = "ImageLeakChecker VMMaker.oscog-eem.3199 uuid: 3866dedc-ccfd-4fc1-a487-85095ada8bba " __DATE__ ;
...
const char *interpreterVersion = "Open Smalltalk ImageChecker VM [ImageLeakChecker VMMaker.oscog-eem.3199]";
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	classFormat = ((fetchPointerofObject(InstanceSpecificationIndex, classObj)) >> 3);
	instSpec = instSpecOfClassFormat(classFormat);
	...
	case weakArrayFormat():
		numSlots = (fixedFieldsOfClassFormat(classFormat)) + nElements;
		fillValue = GIV(nilObj);
		break;
	...
	default:
		...
		if ((nElements != 0)
		 || (instSpec > (lastPointerFormat()))) {
			return null;
		}
		numSlots = fixedFieldsOfClassFormat(classFormat);
		fillValue = GIV(nilObj);
	...
	if (classIndex == 0) {
		...
		flag("todo");
		classIndex = (((hash = rawHashBitsOf(classObj))) != 0
			? hash
	...
	if (numSlots > (maxSlotsForAlloc())) {
		/* begin primitiveFailFor: */
		GIV(primFailCode) = PrimErrUnsupported;
		return null;
	}
	newObj = allocateSlotsForPinningInOldSpacebytesformatclassIndex(numSlots, objectBytesForSlots(numSlots), instSpec, classIndex);
	if (!(newObj == null)) {
		/* begin fillObj:numSlots:with: */
		assert(oopisLessThan(((newObj + BaseHeaderSize) + (numSlots * BytesPerOop)) - 1, addressAfter(newObj)));
		for (p = (((usqInt)(newObj + BaseHeaderSize))); p <= (((usqInt)(((newObj + BaseHeaderSize) + (numSlots * BytesPerOop)) - 1))); p += (allocationUnit())) {
...
/*	Allocate an instance of a variable class, excepting CompiledMethod. */
	...
	/* Spur64BitMemoryManager>>#instantiateUninitializedClass:indexableSize: */
sqInt
instantiateUninitializedClassindexableSize(sqInt classObj, usqInt nElements)
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
    sqInt classFormat;
    sqInt classIndex;
    sqInt err;
	...
	instSpec = instSpecOfClassFormat(classFormat);
	classIndex = rawHashBitsOf(classObj);
	assert(isPureBitsFormat(instSpec));
	switch (instSpec) {
	case sixtyFourBitIndexableFormat():
		numSlots = nElements;
		break;
	...
	if (numSlots > (maxSlotsForNewSpaceAlloc())) {
		...
		newObj = allocateSlotsInOldSpacebytesformatclassIndex(numSlots, objectBytesForSlots(numSlots), instSpec, classIndex);
	}
	else {
