/* Automatically generated by
   ...
   from
	ImageLeakChecker VMMaker.oscog-eem.3419 uuid: 70017fea-1b4a-4603-a657-4d77106c4cf0
 */
static char __buildInfo[] = "ImageLeakChecker VMMaker.oscog-eem.3419 uuid: 70017fea-1b4a-4603-a657-4d77106c4cf0 " __DATE__ ;
...
static sqInt NoDbgRegParms isFloatInstance(sqInt oop);
...
static void NoDbgRegParms checkFreeSpaceignoring(sqInt gcModes, sqInt anOopToIgnore);
...
const char *interpreterVersion = "Open Smalltalk ImageChecker VM [ImageLeakChecker VMMaker.oscog-eem.3419]";
	...
	if ((stackOrNil == GIV(nilObj))
	 || ((fetchPointerofObject(ObjStackTopx, stackOrNil)) >= ObjStackLimit)) {
		...
		thang = (stackOrNil == GIV(nilObj)
			...
			: stackOrNil);
		assert((formatOf(freeOrNewPage)) == (wordIndexableFormat()));
		longAtput((freeOrNewPage + BaseHeaderSize) + (((sqInt)((usqInt)(ObjStackNextx) << (shiftForWord())))), thang);
		/* begin storePointer:ofObjStack:withValue: */
		assert((formatOf(freeOrNewPage)) == (wordIndexableFormat()));
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	l3:	/* end numStrongSlotsOfInephemeral: */;
				...
				if (isNonImmediate(field)) {
					...
					if (index > 0) {
						if ((topOfObjStack(GIV(markStack))) != objToScan) {
							...
							noCheckPushonObjStack(objToScan, objStack4);
						}
						/* begin push:onObjStack: */
				...
				if (isNonImmediate(field)) {
					...
					if (((byteAt(field + 7)) > (traceImmediatelySlotLimit()))
					 && (((numStrongSlots = numStrongSlotsOfInephemeral(field))) > (traceImmediatelySlotLimit()))) {
						...
						noCheckPushonObjStack(objOop2, objStack2);
					}
	l9:	;
				}
			}
...
printStringDataOfon(sqInt oop, FILE *aStream)
{
	...
	if (isBytesNonImm(oop)) {
		...
		n = (i = 0);
		limit = (((lengthOfformat(oop, formatOf(oop))) < 0x100) ? (lengthOfformat(oop, formatOf(oop))) : 0x100);
		while (i < limit) {
		...
		fwprintf(aStream,
				...
				((int) n),
				wideBuffer,
				((lengthOfformat(oop, formatOf(oop))) > limit
			? "..."
			: ""));
	}
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		/* begin readHeapFrom:at:dataBytes: */
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	l3:	/* end lengthOf:format: */;
		...
		if ((lengthOfformat(oop, formatOf(oop))) > lastIndex) {
			print("...");
		}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	print(" nLits ");
	printNum(literalCountOfMethodHeader(methodHeaderOop));
	print(" nArgs ");
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	cnt = ((((max = 128)) < ((len = lengthOfformat(oop, formatOf(oop))))) ? ((max = 128)) : ((len = lengthOfformat(oop, formatOf(oop)))));
	i = 0;
