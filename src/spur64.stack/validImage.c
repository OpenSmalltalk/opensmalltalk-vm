/* Automatically generated by
   ...
   from
	ImageLeakChecker VMMaker.oscog-eem.3077 uuid: 8ad52133-62f2-4e1e-bf92-9b29ba104d62
 */
static char __buildInfo[] = "ImageLeakChecker VMMaker.oscog-eem.3077 uuid: 8ad52133-62f2-4e1e-bf92-9b29ba104d62 " __DATE__ ;
...
static void NoDbgRegParms printFloat(sqInt f);
...
const char *interpreterVersion = "Open Smalltalk ImageChecker VM [ImageLeakChecker VMMaker.oscog-eem.3077]";
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	if (GIV(verbose)) {
		fprintf(GIV(transcript),
				"%.*s0x%" PRIxSQINT "",
				((BytesPerWord * 8) - ((((highBit(n)) < 1) ? 1 : (highBit(n))))) / 4,
				"                ",
				n);
	}
}
...
{
	fprintf(aStream,
			"0x%" PRIxSQINT " is a forwarded hdr%" PRIdSQINT " slot size %" PRIuSQINT "d object to 0x%" PRIxSQINT "\n",
			oop,
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if ((numBytes >= ((numFreeLists()) * (allocationUnit())))
	 && (printAsTreeNode)) {
		...
		print(" < ");
		/* begin printHexPtrnp: */
		p3 = ((void *) (fetchPointerofFreeChunk(freeChunkSmallerIndex(), freeChunk)));
		fprintf(GIV(transcript),
				"0x%" PRIxSQINT "",
				((usqInt)p3));
		print(" > ");
		/* begin printHexPtrnp: */
		p4 = ((void *) (fetchPointerofFreeChunk(freeChunkLargerIndex(), freeChunk)));
		fprintf(GIV(transcript),
				"0x%" PRIxSQINT "",
				((usqInt)p4));
	}
...
{
	fprintf(aStream,
			"0x%" PRIxSQINT " is a free chunk of size %" PRIdSQINT " 0th field:0x%" PRIxSQINT "\n",
			oop,
			bytesInBody(oop),
			fetchPointerofFreeChunk(0, oop));
}
...
printImmediateObjecton(sqInt oop, FILE *aStream)
{
	...
	if ((((oop) & 7) == 1)) {
		fprintf(aStream,
				"0x%" PRIxSQINT "=%" PRIdSQINT "\n",
				oop,
				((sqInt)((oop >> 3))));
	}
	if (((oop & (characterTag())) != 0)) {
		fprintf(aStream,
				"0x%" PRIxSQINT "=$%" PRIdSQINT " ($%c)\n",
				oop,
...
printNonPointerDataOfon(sqInt oop, FILE *aStream)
{
	...
	if (format == (sixtyFourBitIndexableFormat())) {
		...
		for (index = 1; index <= lastIndex; index += 1) {
			v64 = ((usqLong) (fetchLong64ofObject(index - 1, oop)));
			fprintf(aStream,
					"%.*s0x%" PRIxSQINT "%c",
					(64 - ((((highBit(v64)) < 1) ? 1 : (highBit(v64))))) / 4,
					sixteenSpaces,
					v64,
	...
	if (format < (firstShortFormat())) {
		...
		elementsPerLine = 8;
		for (index = 1; index <= lastIndex; index += 1) {
			v32 = ((unsigned) (fetchLong32ofObject(index - 1, oop)));
			fprintf(aStream,
					"%.*s0x%" PRIxSQINT "%c",
					(32 - ((((highBit(v32)) < 1) ? 1 : (highBit(v32))))) / 4,
					sixteenSpaces,
					v32,
					(((index % elementsPerLine) == 0)
			 || (index == lastIndex)
	...
	if (format < (firstByteFormat())) {
		...
		elementsPerLine = 10;
		for (index = 1; index <= lastIndex; index += 1) {
			v16 = fetchShort16ofObject(index - 1, oop);
			fprintf(aStream,
					"%.*s0x%" PRIxSQINT "%c",
					(16 - ((((highBit(v16)) < 1) ? 1 : (highBit(v16))))) / 4,
					sixteenSpaces,
					v16,
					(((index % elementsPerLine) == 0)
			 || (index == lastIndex)
	...
	elementsPerLine = 16;
	for (index = 1; index <= lastIndex; index += 1) {
		v8 = fetchByteofObject(index - 1, oop);
		fprintf(aStream,
				"%.*s0x%" PRIxSQINT "%c",
				(8 - ((((highBit(v8)) < 1) ? 1 : (highBit(v8))))) / 4,
				sixteenSpaces,
				v8,
				(((index % elementsPerLine) == 0)
		 || (index == lastIndex)
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (isImmediate(oop)) {
		...
		fprintf(GIV(transcript),
				"unknown immediate 0x%" PRIxSQINT "",
				oop);
		return;
	}
	if (!(addressCouldBeObj(oop))) {
	...
	if ((!((longAt(oop)) & ((classIndexMask()) - (isForwardedObjectClassIndexPun()))))) {
		...
		fprintf(GIV(transcript),
				" is a forwarder to 0x%" PRIxSQINT "",
				target);
		return;
	}
	...
	l1:	/* end lengthOf: */;
		...
		for (index = startIP; index <= lastIndex; index += 1) {
			if (column == 1) {
				fprintf(GIV(transcript),
						"0x%08p: ",
						((usqIntptr_t)(((oop + BaseHeaderSize) + index) - 1)));
			}
