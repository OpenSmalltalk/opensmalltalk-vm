/* Automatically generated by
	...
	(Compiler-ct.535)
   from
	ImageLeakChecker VMMaker.oscog-eem.3784 uuid: ad2305b0-44cb-456b-8b07-7a1a9825bc2b
 */
static char __buildInfo[] = "ImageLeakChecker VMMaker.oscog-eem.3784 uuid: ad2305b0-44cb-456b-8b07-7a1a9825bc2b " __DATE__ ;
...
static NoDbgRegParms sqInt firstBytecodeOfAlternateHeadermethod(sqInt methodHeader, sqInt theMethod);
...
extern int startPCOfMethod(sqInt aCompiledMethod);
...
const char *interpreterVersion = "Open Smalltalk ImageChecker VM [ImageLeakChecker VMMaker.oscog-eem.3784]";
...
{
	return (numSlots
			? ((((usqInt)((numSlots + (numSlots & 1))) << (shiftForWord())))) + ((numSlots >= (numSlotsMask())
		? BaseHeaderSize + BaseHeaderSize
		: BaseHeaderSize))
...
byteSizeOf(sqInt oop)
{
	...
	if (fmt >= (firstShortFormat())) {
		return numBytes - ((((usqInt)((fmt & 3)) << 1)));
	}

	/* fmt >= self firstLongFormat */
	return numBytes - ((((usqInt)((fmt & 1)) << 2)));
}
...
numBytesOfBitsformat(sqInt objOop, sqInt format)
{
	...
	if (format < (firstShortFormat())) {
		return (format <= (sixtyFourBitIndexableFormat())
				? numBytes
				: numBytes - ((((usqInt)((format & 1)) << 2))));
	}
	return (format < (firstByteFormat())
			? numBytes - ((((usqInt)((format & 3)) << 1)))
			: numBytes - (format & 7));
}
...
getShortFromFileswap(sqImageFile aFile, sqInt swapFlag)
{
	...
	return (swapFlag
			? ((((usqInt)(w)) >> 8) & 0xFF) | ((((usqInt)((w & 0xFF)) << 8)))
			: w);
}
