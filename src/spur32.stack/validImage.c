/* Automatically generated by
   ...
   from
	ImageLeakChecker VMMaker.oscog-eem.3445 uuid: bf34d535-d388-48bb-b497-6a6661b81d9a
 */
static char __buildInfo[] = "ImageLeakChecker VMMaker.oscog-eem.3445 uuid: bf34d535-d388-48bb-b497-6a6661b81d9a " __DATE__ ;
...
static NoDbgRegParms sqInt isImmediateFloat(sqInt oop);
...
static NoDbgRegParms int fetchLong32ofObject(sqInt fieldIndex, sqInt oop);
...
static NoDbgRegParms int isEnumerableObject(sqInt objOop);
...
static NoDbgRegParms size_t readImageFromFileHeapSizeStartingAt(sqImageFile f, usqInt desiredHeapSize, squeakFileOffsetType imageOffset);
...
const char *interpreterVersion = "Open Smalltalk ImageChecker VM [ImageLeakChecker VMMaker.oscog-eem.3445]";
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (GIV(globalSessionID) == 0) {
		GIV(globalSessionID) = ((time(NULL)) + 1 /* begin ioMSecs */) & 0x7FFFFFFF;
	}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while ((argi < argc)
	 && (((argv[argi])[0]) == '-')) {
		if ((strcmp(		/* begin asCommandLineArgument: */
			(PharoVM
			? "--" "version"
			: "-" "version"), argv[argi])) == 0) {
			printf("%s\n", interpreterVersion);
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
		...
		if (((GIV(freeListsMask) & (1U << initialIndex)) != 0)) {
			if (((node = GIV(freeLists)[initialIndex])) == 0) {
				...
				while (node != 0) {
					...
					next = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, node);
					...
					while (node != 0) {
						...
						if (((segmentContainingObj(node))->containsPinned)) {
							if (prev == 0) {
								...
								GIV(freeLists)[index] = ((next1 = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, node)));
								if (next1 != 0) {
									storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, next1, 0);
								}
							}
							else {
								/* begin setNextFreeChunkOf:withValue:isLilliputianSize: */
								storePointerofFreeChunkwithValue(0 /* begin freeChunkNextIndex */, prev, next);
								if ((next != 0)) {
									storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, next, prev);
								}
							}
						...
						if (((segmentContainingObj(node))->containsPinned)) {
							if (prev == 0) {
								...
								assert(((bytesInBody(node)) == (index * (allocationUnit())))
								 && ((index > 1)
								 && ((startOfObject(node)) == node)));
								GIV(freeLists)[index] = ((next2 = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, node)));
								if (next2 != 0) {
									storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, next2, 0);
								}
							}
							else {
								/* begin setNextFreeChunkOf:withValue:isLilliputianSize: */
	...
	while (child != 0) {
		...
		if (childBytes == totalBytes) {
			...
			while (1) {
				prev = node;
				node = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, node);
				if (!(node != 0)) break;
				if (((segmentContainingObj(node))->containsPinned)) {
					/* begin assertValidFreeObject: */
					assert(assertInnerValidFreeObject(node));
					nextFreeChunk = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, node);
					/* begin setNextFreeChunkOf:withValue:isLilliputianSize: */
					storePointerofFreeChunkwithValue(0 /* begin freeChunkNextIndex */, prev, nextFreeChunk);
					if ((nextFreeChunk != 0)) {
						storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, nextFreeChunk, prev);
					}
			...
			if (((segmentContainingObj(child))->containsPinned)) {
				next = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, child);
				if (next == 0) {
		...
		if (child != 0) {
			...
			if (childBytes <= (totalBytes + (allocationUnit()))) {
				...
				if (acceptedNode == 0) {
					...
					acceptedChunk = node;
					do {
						acceptedChunk = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, acceptedChunk);
						if ((acceptedChunk != 0)
	...
	if (acceptedNode != 0) {
		if (acceptedChunk != 0) {
			...
			while (1) {
				next = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, acceptedNode);
				if (!(next != acceptedChunk)) break;
				acceptedNode = next;
			}
			nextFreeChunk1 = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, acceptedChunk);
			/* begin setNextFreeChunkOf:withValue:isLilliputianSize: */
			storePointerofFreeChunkwithValue(0 /* begin freeChunkNextIndex */, acceptedNode, nextFreeChunk1);
			if ((nextFreeChunk1 != 0)) {
				storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, nextFreeChunk1, acceptedNode);
			}
	...
	if (numSlots >= (numSlotsMask())) {
		...
		long64Atput(chunk + BaseHeaderSize, ((((((usqLong) (numSlotsMask()))) << (numSlotsFullShift())) + (((sqInt)((usqInt)(formatField) << (formatShift()))))) + classIndex) | (1U << (pinnedBitShift())));
...
allocateSlotsInOldSpacebytesformatclassIndex(sqInt numSlots, usqInt totalBytes, sqInt formatField, sqInt classIndex)
{
	...
	if (numSlots >= (numSlotsMask())) {
		...
		longAtput(chunk, numSlots);
		longAtput(chunk + 4, ((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
		long64Atput(chunk + BaseHeaderSize, (((((usqLong) (numSlotsMask()))) << (numSlotsFullShift())) + (((sqInt)((usqInt)(formatField) << (formatShift()))))) + classIndex);
		checkFreeSpace(GCModeNewSpace);
		return chunk + BaseHeaderSize;
	}
	...
	while (1) {
		...
		if (bytesInNode > bytesInArg) {
			...
			if (subNode == 0) {
				...
				storePointerofFreeChunkwithValue(2 /* begin freeChunkParentIndex */, freeTree, treeNode);
				return;
			}
		}
		else {
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (index < (numFreeLists())) {
		...
		storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, freeChunk, 0);
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (child != 0) {
		...
		if (childBytes == chunkBytes) {
			...
			nextFreeChunk = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, child);
			/* begin setNextFreeChunkOf:withValue:isLilliputianSize: */
			storePointerofFreeChunkwithValue(0 /* begin freeChunkNextIndex */, freeChunk, nextFreeChunk);
			if ((nextFreeChunk != 0)) {
				storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, nextFreeChunk, freeChunk);
			}
			/* begin setNextFreeChunkOf:withValue:isLilliputianSize: */
			storePointerofFreeChunkwithValue(0 /* begin freeChunkNextIndex */, child, freeChunk);
			if ((freeChunk != 0)) {
				storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, freeChunk, child);
			}
			return child;
		}
		parent = child;
	...
	storePointerofFreeChunkwithValue(2 /* begin freeChunkParentIndex */, freeChunk, parent);
		...
		while (oopisLessThan(obj, GIV(freeOldSpaceStart))) {
			...
			if (classIndex >= (isForwardedObjectClassIndexPun())) {
				...
				if (classIndex == (isFreeObjectClassIndexPun())) {
					...
					field = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, obj);
					if (field != 0) {
						storePointerNoAssertofFreeChunkwithValue(0 /* begin freeChunkNextIndex */, obj, swizzleObjin(field, obj));
					}
					chunkBytes = bytesInBody(obj);
					if (chunkBytes >= ((numFreeLists()) * (allocationUnit()))) {
						for (index = 2 /* begin freeChunkParentIndex */, indexLimiT = (freeChunkLargerIndex()); index <= indexLimiT; index += 1) {
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
		 ...
		 && ((1U << index) <= GIV(freeListsMask))) {
			if (((GIV(freeListsMask) & (1U << index)) != 0)) {
				if (((chunk = GIV(freeLists)[index])) != 0) {
					...
					GIV(freeLists)[index] = ((next = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, chunk)));
					if (next != 0) {
						storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, next, 0);
					}
					assert((bytesInBody(chunk)) == (index * (allocationUnit())));
			...
			if (((GIV(freeListsMask) & (1U << index)) != 0)) {
				if (((chunk = GIV(freeLists)[index])) != 0) {
					...
					assert(((bytesInBody(chunk)) == (index * (allocationUnit())))
					 && ((index > 1)
					 && ((startOfObject(chunk)) == chunk)));
					GIV(freeLists)[index] = ((next1 = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, chunk)));
					if (next1 != 0) {
						storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, next1, 0);
					}
					assert((bytesInBody(chunk)) == (index * (allocationUnit())));
	...
	while (child != 0) {
		...
		if (childBytes == chunkBytes) {
			/* size match; try to remove from list at node. */
			chunk = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, child);
			if (chunk != 0) {
				/* begin assertValidFreeObject: */
				assert(assertInnerValidFreeObject(chunk));
				nextFreeChunk = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, chunk);
				/* begin setNextFreeChunkOf:withValue:isLilliputianSize: */
				storePointerofFreeChunkwithValue(0 /* begin freeChunkNextIndex */, child, nextFreeChunk);
				if ((nextFreeChunk != 0)) {
					storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, nextFreeChunk, child);
				}
	...
	chunk = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, parent);
	if (chunk != 0) {
		assert((chunkBytes == nodeBytes)
		 || ((chunkBytes + (allocationUnit())) < nodeBytes));
		nextFreeChunk1 = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, chunk);
		/* begin setNextFreeChunkOf:withValue:isLilliputianSize: */
		storePointerofFreeChunkwithValue(0 /* begin freeChunkNextIndex */, parent, nextFreeChunk1);
		if ((nextFreeChunk1 != 0)) {
			storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, nextFreeChunk1, parent);
		}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	chunk = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, objOop);
	assert((chunk == 0)
	 || (isFreeOop(chunk)));
	...
	chunk = fetchPointerofFreeChunk(1 /* begin freeChunkPrevIndex */, objOop);
		...
		do {
			...
			if (((smallChild == 0)
			 ...
			 || ((largeChild == 0
				...
				cameFrom = treeNode;
				treeNode = fetchPointerofFreeChunk(2 /* begin freeChunkParentIndex */, treeNode);
			}
			else {
				if ((smallChild != 0)
				 && (cameFrom != smallChild)) {
	...
	if (isLargeFreeObject(objOop)) {
		/* Tree assertions */
		chunk = fetchPointerofFreeChunk(2 /* begin freeChunkParentIndex */, objOop);
		assert((chunk == 0)
		 || ((isFreeOop(chunk))
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if ((gcModes > 0)
	 && (((checkForLeaks & (GCCheckFreeSpace | gcModes)) == (GCCheckFreeSpace | gcModes)))) {
		/* begin runLeakCheckerForFreeSpace:ignoring: */
	}
}
...
(GIV(pastSpaceStart) > (((pastSpace()).start))
	...
	while (1) {
		...
		if (isFreeObject(objOop1)) {
			...
			fieldOop = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, objOop1);
			if ((fieldOop != 0)
			 && ((heapMapAtWord(pointerForOop(fieldOop))) != 0)) {
				...
				ok = 0;
						...
						fieldOop = fetchPointerofFreeChunk(1 /* begin freeChunkPrevIndex */, objOop1);
			if ((fieldOop != 0)
			 && ((heapMapAtWord(pointerForOop(fieldOop))) != 0)) {
				print("leak in free chunk ");
				printHex(objOop1);
			...
			if (isLargeFreeObject(objOop1)) {
				for (fi = 2 /* begin freeChunkParentIndex */, fiLimiT = (freeChunkLargerIndex()); fi <= fiLimiT; fi += 1) {
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (numSlots > (maxSlotsForNewSpaceAlloc())) {
		...
		if (numSlots >= (numSlotsMask())) {
			...
			longAtput(GIV(freeStart), numSlots);
			longAtput(GIV(freeStart) + 4, ((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
			long64Atput(newObj1, (((((usqLong) (numSlotsMask()))) << (numSlotsFullShift())) + (((sqInt)((usqInt)(fmt) << (formatShift()))))) + classIndex1);
		}
		else {
	...
	if ((delta <= (allocationUnit()))
	 && (isOldObject(objOop))) {
		...
		if (numSlots >= (numSlotsMask())) {
			...
			longAtput(GIV(freeStart), numSlots);
			longAtput(GIV(freeStart) + 4, ((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
			long64Atput(newObj, (((((usqLong) (numSlotsMask()))) << (numSlotsFullShift())) + (((sqInt)((usqInt)(format) << (formatShift()))))) + classIndex);
		}
		else {
	...
	if ((stackOrNil == GIV(nilObj))
	 || ((fetchPointerofObject(ObjStackTopx, stackOrNil)) >= ObjStackLimit)) {
		...
		if (freeOrNewPage != 0) {
			...
			longAtput((stackOrNil + BaseHeaderSize) + (((int)((usqInt)(ObjStackFreex) << (shiftForWord())))), 0);
			assert((!GIV(marking))
			...
			longAtput((freeOrNewPage + BaseHeaderSize) + (((int)((usqInt)(ObjStackFreex) << (shiftForWord())))), 0);
			if (GIV(marking)) {
		...
		/* begin storePointer:ofObjStack:withValue: */
		assert((formatOf(freeOrNewPage)) == (wordIndexableFormat()));
		longAtput((freeOrNewPage + BaseHeaderSize) + (((int)((usqInt)(ObjStackMyx) << (shiftForWord())))), objStackRootIndex);
		/* begin storePointer:ofObjStack:withValue: */
		assert((formatOf(freeOrNewPage)) == (wordIndexableFormat()));
		longAtput((freeOrNewPage + BaseHeaderSize) + (((int)((usqInt)(ObjStackNextx) << (shiftForWord())))), (stackOrNil == GIV(nilObj)
			? 0
			: stackOrNil));
		/* begin storePointer:ofObjStack:withValue: */
		assert((formatOf(freeOrNewPage)) == (wordIndexableFormat()));
		longAtput((freeOrNewPage + BaseHeaderSize) + (((int)((usqInt)(ObjStackTopx) << (shiftForWord())))), 0);
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, newNode, 0);
	for (i = 2 /* begin freeChunkParentIndex */, iLimiT = (freeChunkLargerIndex()); i <= iLimiT; i += 1) {
		...
		if (i == 2 /* begin freeChunkParentIndex */) {
			...
			if (relative != 0) {
				...
				storePointerofFreeChunkwithValue(2 /* begin freeChunkParentIndex */, relative, newNode);
			}
		}
...
initFreeTreeChunkbytes(sqInt freeChunk, sqInt chunkBytes)
{
	...
	storePointerofFreeChunkwithValue(0 /* begin freeChunkNextIndex */, freeChunk, 0);
	storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, freeChunk, 0);
	storePointerofFreeChunkwithValue(2 /* begin freeChunkParentIndex */, freeChunk, 0);
...
inOrderPrintFreeTreeprintList(sqInt freeChunk, sqInt printNextList)
{
	...
	if (printNextList) {
		next = freeChunk;
		while (((next = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, next))) != 0) {
			/* begin tab */
			printChar('\t');
...
noCheckPushonObjStack(sqInt objOop, sqInt objStack)
{
	...
	if (topx >= ObjStackLimit) {
		...
		longAtput((objStack + BaseHeaderSize) + (((int)((usqInt)((ObjStackFixedSlots + topx)) << (shiftForWord())))), objOop);
		/* begin storePointer:ofObjStack:withValue: */
		assert((formatOf(objStack)) == (wordIndexableFormat()));
		longAtput((objStack + BaseHeaderSize) + (((int)((usqInt)(ObjStackTopx) << (shiftForWord())))), topx + 1);
	}
	return objOop;
}
...
popObjStack(sqInt objStack)
{
	...
	/* begin storePointer:ofObjStack:withValue: */
	assert((formatOf(objStack)) == (wordIndexableFormat()));
	longAtput((objStack + BaseHeaderSize) + (((int)((usqInt)(ObjStackTopx) << (shiftForWord())))), topx);
	if ((topx == 0)
	 && (((nextPage = fetchPointerofObject(ObjStackNextx, objStack))) != 0)) {
		/* begin storePointer:ofObjStack:withValue: */
		assert((formatOf(nextPage)) == (wordIndexableFormat()));
		longAtput((nextPage + BaseHeaderSize) + (((int)((usqInt)(ObjStackFreex) << (shiftForWord())))), objStack);
		/* begin storePointer:ofObjStack:withValue: */
		assert((formatOf(objStack)) == (wordIndexableFormat()));
		longAtput((objStack + BaseHeaderSize) + (((int)((usqInt)(ObjStackNextx) << (shiftForWord())))), 0);
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	printNum(numBytes);
	print(" next ");
	printHexnp(fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, freeChunk));
		print(" prev ");
	printHexnp(fetchPointerofFreeChunk(1 /* begin freeChunkPrevIndex */, freeChunk));
	if ((numBytes >= ((numFreeLists()) * (allocationUnit())))
	 && (printAsTreeNode)) {
		print(" ^ ");
		printHexnp(fetchPointerofFreeChunk(2 /* begin freeChunkParentIndex */, freeChunk));
		print(" < ");
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (freeChunk != 0) {
		printFreeChunk(freeChunk);
		freeChunk = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, freeChunk);
	}
	return 0;
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	reserve = 0 /* begin interpreterAllocationReserveBytes */;
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (numSlots >= (numSlotsMask())) {
		...
		longAtput(GIV(freeStart), numSlots);
		longAtput(GIV(freeStart) + 4, ((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
		long64Atput(newObj, (((((usqLong) (numSlotsMask()))) << (numSlotsFullShift())) + (((sqInt)((usqInt)(formatField) << (formatShift()))))) + ClassByteStringCompactIndex);
	}
	else {
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (((stackOrNil = fetchPointerofObject(ObjStackFreex, firstPage))) != 0) {
		...
		do {
			...
			longAtput((page + BaseHeaderSize) + (((int)((usqInt)(ObjStackFreex) << (shiftForWord())))), stackOrNil);
			page = stackOrNil;
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	for (i = 1, iLimiT = ((numFreeLists()) - 1); i <= iLimiT; i += 1) {
		...
		while (listNode != 0) {
			...
			nextNode = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, listNode);
	...
	do {
		...
		if (((smallChild == 0)
		 ...
		 || ((largeChild == 0
			...
			cameFrom = treeNode1;
			treeNode1 = fetchPointerofFreeChunk(2 /* begin freeChunkParentIndex */, treeNode1);
		}
		else {
			if ((smallChild != 0)
			 && (cameFrom != smallChild)) {
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	prev = fetchPointerofFreeChunk(1 /* begin freeChunkPrevIndex */, freeChunk);
	if (prev != 0) {
		setNextFreeChunkOfwithValuechunkBytes(prev, fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, freeChunk), chunkBytes);
		return freeChunk;
	}
	if ((index < (numFreeLists()))
	 && ((1U << index) <= GIV(freeListsMask))) {
		...
		GIV(freeLists)[index] = ((next1 = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, freeChunk)));
		if (next1 != 0) {
			storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, next1, 0);
		}
		return freeChunk;
	}
	next = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, freeChunk);
	if (next == 0) {
		/* no list; remove the interior node */
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	parent = fetchPointerofFreeChunk(2 /* begin freeChunkParentIndex */, freeTreeNode);
	if (parent == 0) {
		/* no parent; stitch the subnodes back into the root */
		if (smaller == 0) {
			if (larger != 0) {
				storePointerofFreeChunkwithValue(2 /* begin freeChunkParentIndex */, larger, 0);
			}
			GIV(freeLists)[0] = larger;
		}
		else {
			storePointerofFreeChunkwithValue(2 /* begin freeChunkParentIndex */, smaller, 0);
		...
		if (smaller == 0) {
			...
			if (larger != 0) {
				storePointerofFreeChunkwithValue(2 /* begin freeChunkParentIndex */, larger, parent);
			}
		}
		else {
			storePointerofFreeChunkwithValue((freeTreeNode == (fetchPointerofFreeChunk(freeChunkSmallerIndex(), parent))
			...
			storePointerofFreeChunkwithValue(2 /* begin freeChunkParentIndex */, smaller, parent);
			if (larger != 0) {
				addFreeSubTree(larger);
			}
		}
...
updateListStartingAt(sqInt freeNode)
{
	...
	prev = freeNode;
	storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, prev, 0);
	while (1) {
		obj = fetchPointerofFreeChunk(0 /* begin freeChunkNextIndex */, prev);
		if (!(obj != 0)) break;
		storePointerofFreeChunkwithValue(1 /* begin freeChunkPrevIndex */, obj, prev);
		prev = obj;
	}
}
...
validFreeTreeChunkparent(sqInt chunk, sqInt parent)
{
	...
	if (parent != (fetchPointerofFreeChunk(2 /* begin freeChunkParentIndex */, chunk))) {
		return "bad parent";
	}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	allocationReserve = 0 /* begin interpreterAllocationReserveBytes */;
