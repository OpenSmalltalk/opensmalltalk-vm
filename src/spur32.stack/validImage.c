/* Automatically generated by
	...
	(Compiler-ct.535)
   from
	ImageLeakChecker VMMaker.oscog-eem.3792 uuid: b00773bb-68a0-441a-b1b0-211aa4a67e97
 */
static char __buildInfo[] = "ImageLeakChecker VMMaker.oscog-eem.3792 uuid: b00773bb-68a0-441a-b1b0-211aa4a67e97 " __DATE__ ;
...
static sqInt corpseBitsTimesAllocationUnit(void);
extern usqInt corpseForCorpseOffset(sqInt corpseOffset);
...
static NoDbgRegParms sqInt isYetToBeScavenged(sqInt oop);
static sqInt maxNewSpaceBytes(void);
static NoDbgRegParms sqInt nextCorpseOrNil(sqInt corpse);
extern void openScavengeLog(void);
extern void printEphemeronList(void);
extern void printRememberedSet(void);
extern sqInt remember(sqInt objOop);
static SpurSegmentInfo firstSegment(void);
...
static NoDbgRegParms sqLong fetchIntegerofObject(sqInt fieldIndex, sqInt objOop);
...
static sqInt instSpecForImmediateClasses(void);
...
extern sqInt isonObjStack(sqInt oop, sqInt objStack);
static NoDbgRegParms sqInt keyOfEphemeron(sqInt objOop);
static NoDbgRegParms sqInt keyOfMaybeFiredEphemeron(sqInt objOop);
...
static sqInt log2AllocationUnit(void);
...
extern void printMournQueue(void);
extern void printNewObjectsInObjStack(sqInt objStack);
...
static NoDbgRegParms sqInt printObjStackPagemyIndexpageTypeprintContents(sqInt objStackPage, sqInt myx, sqInt pageType, sqInt printContents);
...
static sqInt shiftForAllocationUnit(void);
...
static NoDbgRegParms sqInt storeByteofObjectwithValue(sqInt byteIndex, sqInt oop, sqInt valueByte);
extern usqInt storeCheckBoundary(void);
static NoDbgRegParms void storeIntegerofObjectwithValue(sqInt fieldIndex, sqInt objOop, sqLong integerValue);
extern sqInt storeLong32ofObjectwithValue(sqInt fieldIndex, sqInt obj, sqInt valueWord);
static NoDbgRegParms sqLong storeLong64ofObjectwithValue(sqInt longIndex, sqInt objOop, sqLong value);
...
static NoDbgRegParms sqInt storeShort16ofObjectwithValue(sqInt shortIndex, sqInt objOop, sqInt value);
static NoDbgRegParms sqInt storeUncheckedNoAssertofAnywithValue(sqInt fieldIndex, sqInt objOop, sqInt valuePointer);
...
static NoDbgRegParms void * arrayValueOf(sqInt arrayOop);
...
static char * traceSources[] = {
		"?", "enterCritical", "exitCritical", "resume", "syncsig", "suspend", "wait",
		"yield", "priority"
	};
sqInt checkForLeaks;
sqInt imageHeaderFlags;
...
const char *interpreterVersion = "Open Smalltalk ImageChecker VM [ImageLeakChecker VMMaker.oscog-eem.3792]";
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	GIV(interruptKeycode) = ((0x800)) + (((sqInt)'.'));
...
addressAfter(sqInt objOop)
{
	...
	if (numSlots == (numSlotsMask())) {
		numSlots = longAt((void *)(objOop - BaseHeaderSize));
	}
...
l1:
	...
	if (numSlots >= (numSlotsMask())) {
		longAtput((void *)(chunk),numSlots);
		longAtput((void *)(chunk + 4),((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
		long64Atput((void *)(chunk + BaseHeaderSize),(((((((usqLong) (numSlotsMask()))) << (numSlotsFullShift()))) + ((((usqInt)(formatField) << (formatShift()))))) + classIndex) | (1U << (pinnedBitShift())));
	...
	long64Atput((void *)(chunk),(((((((usqLong) numSlots)) << (numSlotsFullShift()))) + ((((usqInt)(formatField) << (formatShift()))))) + classIndex) | (1U << (pinnedBitShift())));
	checkFreeSpace(GCModeNewSpace);
	return chunk;
}
...
bytesInBody(sqInt objOop)
{
	...
	headerNumSlots = byteAt((void *)(objOop + (numSlotsFieldByteOffset())));
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	longAtput((void *)(startAddress),((startAddress + (allocationUnit())) == objOop
			? 0
			: 1));
	longAtput((void *)(startAddress + 4),((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
}
...
initFreeChunkWithBytesat(usqLong numBytes, sqInt address)
{
	...
	if (numBytes >= (((((usqInt)((numSlotsMask())) << (shiftForWord())))) + BaseHeaderSize)) {
		...
		longAtput((void *)(address),numSlots);
		longAtput((void *)(address + 4),((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
		longAtput((void *)(address + 8),0);
		longAtput((void *)(address + 12),((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));

		/* 0's classIndex; 0 = classIndex of free chunks */
		return address + 8;
	}
	...
	assert(numSlots < (numSlotsMask()));
	longAtput((void *)(address),0);
	longAtput((void *)(address + 4),((sqLong)((usqLong)(numSlots) << (numSlotsHalfShift()))));
...
initSegmentBridgeWithBytesat(usqLong numBytes, sqInt address)
{
	...
	if (numSlots) {
		longAtput((void *)(address),numSlots);
		longAtput((void *)(address + 4),((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
		longAtput((void *)(address + 8),((1U << (pinnedBitShift())) + ((((usqInt)((wordIndexableFormat())) << (formatShift()))))) + (segmentBridgePun()));
		longAtput((void *)(address + 12),((((usqInt)((numSlotsMask())) << (numSlotsHalfShift())))) + (1U << (markedBitHalfShift())));
	}
	else {
		longAtput((void *)(address),((1U << (pinnedBitShift())) + ((((usqInt)((wordIndexableFormat())) << (formatShift()))))) + (segmentBridgePun()));
		longAtput((void *)(address + 4),1U << (markedBitHalfShift()));
	}
}
...
integerObjectOf(sqInt value)
{
	...
	/* Spur32BitMemoryManager>>#isFloatInstance: */
static NoDbgRegParms sqInt
...
{
	return ((byteAt((void *)(objOop + (formatFieldByteOffset())))) & (formatMask())) == (firstLongFormat());
}
...
/*	Answer the C double precision floating point value of the argument,
	...
	/* Spur32BitMemoryManager>>#noFailFloatValueOf: */
double
...
setClassIndexOfto(sqInt objOop, sqInt classIndex)
{
	...
	longAtput((void *)(objOop),((longAt((void *)(objOop))) & ((unsigned int)~(classIndexMask()))) + classIndex);
}
...
{
	longAtput((void *)(objOop),(aBoolean
			? (longAt((void *)(objOop))) | (1U << (immutableBitShift()))
			: (longAt((void *)(objOop))) & ((unsigned int)~(1U << (immutableBitShift())))));
}
...
/*	Answer the effective field width of the corpseOffset. See the corpse
	offset methods, corpseForCorpseOffset: et al.
 */

	/* SpurGenerationScavenger>>#corpseBitsTimesAllocationUnit */
static sqInt
corpseBitsTimesAllocationUnit(void)
{
	return ((identityHashFieldWidth()) + 8) + (log2AllocationUnit());
}


/*	Use the identityHash and format-containing byte to construct a 30 bit
	offset through non-future newSpace and use this to implement lists for
	weak array
	and ephemeron processing. 30 bits of 8 byte allocationUnits units is 2 ^
	33 bytes, or 8Gb, big enough for newSpace for a good few years yet. */
/*	debugging */

	/* SpurGenerationScavenger>>#corpseForCorpseOffset: */
usqInt
corpseForCorpseOffset(sqInt corpseOffset)
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	return ((((usqInt)((corpseOffset - 1)) << (shiftForAllocationUnit())))) + GIV(newSpaceStart);
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	return ((((usqInt)((headOfCorpseList - 1)) << (shiftForAllocationUnit())))) + GIV(newSpaceStart);
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (!newObj) {
		...
		if (!newObj) {
			ifNilSqInt = growOldSpaceByAtLeast((numSlots + 0x400) * BytesPerOop);
			if (!ifNilSqInt) {
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
    sqInt previousFSStart;

	previousFSStart = GIV(futureSurvivorStart);
	GIV(futureSurvivorStart) = (GIV(futureSpace).start);
	return previousFSStart;
}
...
/*	Answer whether the oop is in eden or pastSpace and yet-to-be scavenged. */

	/* SpurGenerationScavenger>>#isYetToBeScavenged: */
static NoDbgRegParms sqInt
isYetToBeScavenged(sqInt oop)
{
    sqInt target;
	...
	return (isReallyYoung(target))
	 && (!(isInFutureSpace(target)));
}


/*	Answer the maximum size of newSpace (8Gb). See the corpse offset methods,
	corpseForCorpseOffset: et al.
 */

	/* SpurGenerationScavenger>>#maxNewSpaceBytes */
static sqInt
maxNewSpaceBytes(void)
{
	return (((corpseBitsTimesAllocationUnit()) < 0) ? (((usqInt)(1)) >> (-(corpseBitsTimesAllocationUnit()))) : (1U << (corpseBitsTimesAllocationUnit())));
}
...
/*	corpse is the corpse of a weak array that has been added to the weakList.
	...
	/* SpurGenerationScavenger>>#nextCorpseOrNil: */
static NoDbgRegParms sqInt
nextCorpseOrNil(sqInt corpse)
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
    usqInt listOffset;

	assert(isYoung(corpse));
	listOffset = ((((usqInt)((rawHashBitsOf(corpse))) << 8))) + (byteAt((void *)(corpse + (formatFieldByteOffset()))));
	return (listOffset
			? ((((usqInt)((listOffset - 1)) << (shiftForAllocationUnit())))) + GIV(newSpaceStart)
			: 0);
}
...
/*	Print the objects on the ephemeronList, indicating active or inactive. */

	/* SpurGenerationScavenger>>#printEphemeronList */
void
printEphemeronList(void)
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
    usqInt corpse;
    sqInt ephemeron;
    sqInt ephemeronList;

	corpse = firstCorpse(ephemeronList);
	if (!corpse) {
		print("empty");
		cr();
		return;
	...
	while (corpse) {
		ephemeron = followForwarded(corpse);
		fprintf(GIV(transcript),
				"%x -> %p %s\n",
				corpse,
				((void *)ephemeron),
				(isYetToBeScavenged(keyOfMaybeFiredEphemeron(ephemeron))
				? "active"
				: "inactive"));
		corpse = nextCorpseOrNil(corpse);
	}
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	/* SpurLeakCheckingSegmentManager>>#firstSegment */
static SpurSegmentInfo
firstSegment(void)
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	return GIV(segments)[0];
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	assert(((firstSegment()).segStart) == 0);
	GIV(oldHeapBase) = GIV(oldImageBaseAddress);
	GIV(oldHeapLimit) = GIV(oldImageBaseAddress) + ((GIV(numSegments) == 1
		? ((firstSegment()).segSize)
		: ((((lastSegment()).segStart)) - (((lastSegment()).swizzle))) + (((lastSegment()).segSize))));
}
	...
	if (!(oopisGreaterThanOrEqualToandLessThan(objOop, GIV(oldHeapBase), GIV(oldHeapLimit)))) {
		if (GIV(verbose)) {
			...
			if (containerOopOrNil) {
				printf("Oop %p is out of bounds at/in %p\n", ((void *)objOop), ((void *)(containerOopOrNil - GIV(nilObj))));
			}
			else {
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	return ((address & (BaseHeaderSize - 1)) == 0)
	 && ((/* Inlined isInOldSpace: for speed, because it also segregates the newSpace objects */
		...
		while (oopisLessThan(obj, GIV(freeOldSpaceStart))) {
			...
			if (classIndex >= (isForwardedObjectClassIndexPun())) {
				...
				if (classIndex == (isFreeObjectClassIndexPun())) {
					...
					if (field) {
						storeUncheckedNoAssertofAnywithValue(0 /* freeChunkNextIndex */, obj, swizzleObjin(field, obj));
					}
					...
					if (chunkBytes >= ((numFreeLists()) * (allocationUnit()))) {
						for (index = 2 /* freeChunkParentIndex */; index <= (freeChunkLargerIndex()); index += 1) {
							...
							if (field) {
								storeUncheckedNoAssertofAnywithValue(index, obj, swizzleObjin(field, obj));
							}
						}
					}
				}
...
allocateSlotsInOldSpacebytesformatclassIndex(sqInt numSlots, usqInt totalBytes, sqInt formatField, sqInt classIndex)
{
	...
	if ((chunk = allocateOldSpaceChunkOfBytes(totalBytes))) {
		if (numSlots >= (numSlotsMask())) {
			long64Atput((void *)(chunk),((((usqLong)((numSlotsMask())) << (numSlotsFullShift())))) + numSlots);
...
l4:;
	...
	if (numRememberedObjectsInHeap != GIV(rememberedSetSize)) {
		fprintf(GIV(transcript),
				...
				numRememberedObjectsInHeap,
				GIV(rememberedSetSize));
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (numSlots > (maxSlotsForNewSpaceAlloc())) {
		...
		if (numSlots >= (numSlotsMask())) {
			...
			long64Atput((void *)(newObjUsqInt),((((((usqLong) (numSlotsMask()))) << (numSlotsFullShift()))) + ((((usqInt)(fmt) << (formatShift()))))) + classIndex);
		}
		else {
			long64Atput((void *)(newObjUsqInt),((((((usqLong) numSlots)) << (numSlotsFullShift()))) + ((((usqInt)(fmt) << (formatShift()))))) + classIndex);
		}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (!delta) {
		if (hasOverflowHeader(objOop)) {
			/* rawOverflowSlotsOf:put: */
			longAtput((void *)(objOop - BaseHeaderSize),numSlots);
		}
		else {
			/* rawNumSlotsOf:put: */
	...
	if ((delta <= (allocationUnit()))
	 && (isOldObject(objOop))) {
		...
		if (numSlots >= (numSlotsMask())) {
			longAtput((void *)(GIV(freeStart)),numSlots);
			longAtput((void *)(GIV(freeStart) + 4),((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
			long64Atput((void *)(newObj),((((((usqLong) (numSlotsMask()))) << (numSlotsFullShift()))) + ((((usqInt)(format) << (formatShift()))))) + classIndex);
		}
		else {
			long64Atput((void *)(newObj),((((((usqLong) numSlots)) << (numSlotsFullShift()))) + ((((usqInt)(format) << (formatShift()))))) + classIndex);
		}
...
/*	An obj stack is a stack of objects stored in a hidden root slot, such as
	the markStack
	or the mournQueue. It is a linked list of segments, with the hot end at
	the head of
	the list. It is a word object (formats 9 or 10). The stack pointer is in
	ObjStackTopx and stack[ObjStackTopx] = 0 means empty. The list goes
	through ObjStackNextx.
	We don't want to shrink objStacks, since they're used in GC and its good
	to keep
	their memory around. So unused pages created by popping emptying pages are
	kept on the ObjStackFreex list. */
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if ((stackOrNil == GIV(nilObj))
	 || ((fetchPointerofObject(ObjStackTopx, stackOrNil)) >= ObjStackLimit)) {
		...
		if (freeOrNewPage) {
			nextFreePage = fetchPointerofObject(ObjStackFreex, freeOrNewPage);
		...
		/* begin storePointer:ofObjStack:withValue: */
		assert((formatOf(freeOrNewPage)) == (wordIndexableFormat()));
		longAtput((void *)((freeOrNewPage + BaseHeaderSize) + ((((usqInt)(ObjStackFreex) << (shiftForWord()))))),nextFreePage);
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		/* classTableIndex must never index the first page, which is reserved for classes known to the VM. */
		majorIndex = ((((majorIndex + 1) & ((classTableRootSlots()) - 1)) < 1) ? 1 : ((majorIndex + 1) & ((classTableRootSlots()) - 1)));
...
fetchClassTagOfNonImm(sqInt obj)
{
	...
	/* SpurMemoryManager>>#fetchInteger:ofObject: */
static NoDbgRegParms sqLong
fetchIntegerofObject(sqInt fieldIndex, sqInt objOop)
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
    sqInt fmt;
    sqInt oop;
	...
	if (fmt <= (lastPointerFormat())) {
		oop = fetchPointerofObject(fieldIndex, objOop);
		return ((oop & 1)
				? (oop >> 1)
				: (GIV(primFailCode) = PrimErrInappropriate));
	}
	return (fmt < (firstShortFormat())
			? (fmt < (firstLongFormat())
					? fetchLong64ofObject(fieldIndex, objOop)
					: fetchLong32ofObject(fieldIndex, objOop))
			: (fmt < (firstByteFormat())
					? fetchShort16ofObject(fieldIndex, objOop)
					: (fmt < (firstCompiledMethodFormat())
							? fetchByteofObject(fieldIndex, objOop)
...
{
	return long64At((void *)((objOop + BaseHeaderSize) + ((((usqInt)(longIndex) << 3)))));
}
...
{
	return shortAt((void *)((objOop + BaseHeaderSize) + ((((usqInt)(shortIndex) << 1)))));
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		followingWord = longAt((void *)(followingWordAddress + 4));
		objSqInt = ((((usqInt)(followingWord)) >> (numSlotsHalfShift())) == (numSlotsMask())
...
forwardto(sqInt obj1, sqInt obj2)
{
	...
	longAtput((void *)(obj1),((longAt((void *)(obj1))) & ((unsigned int)~(((((usqInt)((formatMask())) << (formatShift())))) + (classIndexMask())))) + ((isForwardedObjectClassIndexPun()) + ((((usqInt)((forwardedFormat())) << (formatShift()))))));
#  if IMMUTABILITY
...
/*	Use the format for forwarded objects for immediate classes. Immediate
	classes (SmallInteger,
	Character, etc) can't be instantiated, so the inst spec should be an
	invalid one to cause the
	instantiation primitives to fail. The forwardedFormat is internal to the
	SpurMemoryManager, used to mark forwarded objects only, and so is
	suitable.  */

	/* SpurMemoryManager>>#instSpecForImmediateClasses */
static sqInt
instSpecForImmediateClasses(void)
{
...
{
	return ((byteAt((void *)(oop + (formatFieldByteOffset())))) & (formatMask())) == (arrayFormat());
}
...
{
	return ((byteAt((void *)(objOop + (formatFieldByteOffset())))) & (formatMask())) >= (firstByteFormat());
}
...
isGrey(sqInt objOop)
{
	return ((byteAt((void *)(objOop + (markBitsByteOffset())))) & (1U << (greyBitByteShift()))) != 0;
}
...
{
	return ((((usqInt)((byteAt((void *)(objOop + (immutableExtraBitsByteOffset())))))) >> (immutableBitByteShift())) & 1) != 0;
}
...
{
	return (isNonImmediate(objOop))
	 && (isIndexableFormat((byteAt((void *)(objOop + (formatFieldByteOffset())))) & (formatMask())));
}
...
{
	return ((byteAt((void *)(objOop + (formatFieldByteOffset())))) & (formatMask())) == (sixtyFourBitIndexableFormat());
}
...
/*	Answer if objOop is an ephemeron that may or may not have been fired.
	Unfired ephemerons have ephemeronFormat as their format. Fired ephemerons
	have their format set to nonIndexablePointerFormat, so to disambiguate,
	check the class is an ephemeron class in this case. */
...
isMaybeFiredEphemeron(sqInt objOop)
{
	...
	format = (byteAt((void *)(objOop + (formatFieldByteOffset())))) & (formatMask());
	return (format == (ephemeronFormat()))
	 || ((format == (nonIndexablePointerFormat()))
	 && ((instSpecOfClass(fetchClassOfNonImm(objOop))) == (ephemeronFormat())));
}
...
{
	return ((byteAt((void *)(objOop + (formatFieldByteOffset())))) & (formatMask())) <= (lastPointerFormat());
}
...
{
	return ((((byteAt((void *)(objOop + (formatFieldByteOffset())))) & (formatMask())) >= (firstShortFormat())) && (((byteAt((void *)(objOop + (formatFieldByteOffset())))) & (formatMask())) <= ((firstByteFormat()) - 1)));
}
...
/*	Answer whether classIndex is the class index of a live class, robustly for
	ANY 32-bit
	value, since the argument is an inline-cache tag read from a send site and
	need not be a
	class index. During heavy code-zone churn (e.g. the mass class/metaclass
	reshape caused
	by adding an instance variable to ClassDescription) a send site can be
	left with a stale
	selector-index tag (the unlinked / super / Open-PIC form, which is
	negative for special
	selectors) while linked to a method or closed-PIC checked entry -- freeing
	a CogMethod
	does not eagerly unlink sends to it, so a send can outlive/point past its
	original target.
	Such a tag is not a valid class tag, so answer false; the caller then
	unlinks the send and
	it self-heals on the next send. Every other tag consumer already tolerates
	a non-class tag
	(the JIT checked entry simply misses); guarding here rather than only
	asserting avoids
	indexing the class-table root out of bounds in production. */
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (!(((classIndex >= 0) && (classIndex <= ((1U << (classIndexFieldWidth())) - 1))))) {
		return 0;
	}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (!(((byteAt((void *)(objStackPage + (formatFieldByteOffset())))) & (formatMask())) == (wordIndexableFormat()))) {
...
{
	return isPureBitsFormat((byteAt((void *)(objOop + (formatFieldByteOffset())))) & (formatMask()));
}
...
{
	return ((((byteAt((void *)(objOop + (formatFieldByteOffset())))) & (formatMask())) >= (firstLongFormat())) && (((byteAt((void *)(objOop + (formatFieldByteOffset())))) & (formatMask())) <= ((firstByteFormat()) - 1)));
}
...
/*	Answer the object the ephemeron guards. This is its first element. */
	...
	/* SpurMemoryManager>>#keyOfMaybeFiredEphemeron: */
static NoDbgRegParms sqInt
keyOfMaybeFiredEphemeron(sqInt objOop)
{
	assert(isMaybeFiredEphemeron(objOop));
	return fetchPointerofObject(0, objOop);
}
...
/*	All objects are a multiple of 8 bytes in length */
	...
	/* SpurMemoryManager>>#log2AllocationUnit */
static sqInt
log2AllocationUnit(void)
{
	return 3;
}
...
l1:;
			...
			while (index > 0) {
				...
				if (isNonImmediate(field)) {
					...
					/* There should be no weaklings on the mark stack. */
					noCheckPushonObjStack(field, objStack1);
					if (((byteAt((void *)(field + (numSlotsFieldByteOffset())))) > (traceImmediatelySlotLimit()))
					 && (((numStrongSlots = numStrongSlotsOfInephemeral(field))) > (traceImmediatelySlotLimit()))) {
						objOop1 = (((usqInt)numStrongSlots << 1) | 1);
						objStack1 = GIV(markStack);
...
{
	return (1U << (unusedBitShift())) | ((1U << (pinnedBitShift())) | (1U << (rememberedBitShift())));
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (oopisLessThan(objOop, GIV(oldSpaceStart))) {
		...
		while (oopisLessThan(objOopSqInt, GIV(freeStart))) {
			...
			followingWord = longAt((void *)(followingWordAddress + 4));
			objOopSqInt = ((((usqInt)(followingWord)) >> (numSlotsHalfShift())) == (numSlotsMask())
...
printEntity(sqInt oop)
{
	...
	if (printFlags) {
		...
		print((((byteAt((void *)(oop + (formatFieldByteOffset())))) & (formatMask())) <= 15
				? "f:0"
				: "f:"));
		printHexnpnp((byteAt((void *)(oop + (formatFieldByteOffset())))) & (formatMask()));
		print((isGrey(oop)
				? " g"
				: " ."));
...
printHeaderOf(sqInt objOop)
{
	...
	if (hasOverflowHeader(objOop)) {
		...
		print(" slotc ");
		printHexnp(longAt((void *)(objOop - BaseHeaderSize)));
	...
	print(" fmt ");
	printHexnp((byteAt((void *)(objOop + (formatFieldByteOffset())))) & (formatMask()));
	print(" cidx ");
...
l1:
	...
	/* SpurMemoryManager>>#printMournQueue */
void
printMournQueue(void)
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
    sqInt format;
    sqInt iSqInt;
    sqInt mourner;
...
/*	useful for debugging */
	...
	/* SpurMemoryManager>>#printNewObjectsInObjStack: */
void
printNewObjectsInObjStack(sqInt objStack)
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
    sqInt iSqInt;
    sqInt n;
    sqInt numOnThisPage;
...
l1:
	...
	if (format < (firstShortFormat())) {
		...
		for (index = 1; index <= lastIndex; index += 1) {
			fprintf(aStream,
					"%10p%c",
					((void *)(((usqInt)(fetchLong32ofObject(index - 1, oop))))),
					(((index % elementsPerLine) == 0)
			 || (index == lastIndex)
					? '\n'
					: ' '));
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (objStack == GIV(nilObj)) {
		...
		total = printObjStackPagemyIndexpageTypeprintContents(objStack, fetchPointerofObject(ObjStackMyx, objStack), ObjStackMyx, 1);
		fprintf(GIV(transcript),
				"total %" PRIdSQINT "\n",
				total);
	}
...
/*	Print the page and all pages accessible from it. Answer the total number
	of objects on all non free pages.
 */
...
printObjStackPagemyIndexpageTypeprintContents(sqInt objStackPage, sqInt myx, sqInt pageType, sqInt printContents)
{
	...
	return (isFreePage
			? 0
			: totalContents + (fetchPointerofObject(ObjStackTopx, objStackPage)));
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	fprintf(GIV(transcript),
			...
			pageType,
			((void *)objStackPage),
			classIndexOf(objStackPage),
			wordSizeClassIndexPun(),
			(byteAt((void *)(objStackPage + (formatFieldByteOffset())))) & (formatMask()),
			wordIndexableFormat(),
			numSlotsOfAny(objStackPage),
			ObjStackPageSlots,
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (objStack == GIV(nilObj)) {
		...
		total = printObjStackPagemyIndexpageTypeprintContents(objStack, fetchPointerofObject(ObjStackMyx, objStack), ObjStackMyx, 0);
		fprintf(GIV(transcript),
				"total %" PRIdSQINT "\n",
				total);
	}
			...
			if (fmt <= (lastPointerFormat())) {
				if ((fmt == (indexablePointersFormat()))
				 && ((classIndexOf(objSqInt)) == ClassMethodContextCompactIndex)) {
					...
					assert((ReceiverIndex + ((sp >> 1))) < (lengthOf(objSqInt)));
					contextSize = (sp >> 1);
					/* end fetchStackPointerOf: */
l1:
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	assert(validStorePointerUncheckedArgs(RememberedSetRootIndex, GIV(hiddenRootsObj), anObj));
	longAtput((void *)((GIV(hiddenRootsObj) + BaseHeaderSize) + ((((usqInt)(RememberedSetRootIndex) << (shiftForWord()))))),anObj);
}
...
setFormatOfto(sqInt objOop, sqInt format)
{
	...
	byteAtput((void *)(objOop + (formatFieldByteOffset())),((byteAt((void *)(objOop + (formatFieldByteOffset())))) & (0xFF - (formatMask()))) + format);
}
...
{
	long32Atput((void *)(objOop + 4),((((long32At((void *)(objOop + 4))) | (identityHashHalfWordMask())) - (identityHashHalfWordMask()))) + (hash & (identityHashHalfWordMask())));
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	requestedSurvivorBytes = ((GIV(oldSpaceStart) - GIV(newSpaceStart)) - reserve) / (scavengerDenominator());
	...
	if ((((corpseBitsTimesAllocationUnit()) / 8) < BytesPerWord)
	 && (((survivorBytes * 2) + actualEdenBytes) > (maxNewSpaceBytes()))) {
		error("requested new space size exceeds 8Gb limit imposed by ephemeron/weak implementation.");
	}
...
startOfObject(sqInt objOop)
{
	...
	/* SpurMemoryManager>>#storeByte:ofObject:withValue: */
static NoDbgRegParms sqInt
storeByteofObjectwithValue(sqInt byteIndex, sqInt oop, sqInt valueByte)
{
	return byteAtput((void *)((oop + BaseHeaderSize) + byteIndex),valueByte);
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	/* SpurMemoryManager>>#storeInteger:ofObject:withValue: */
static NoDbgRegParms void
storeIntegerofObjectwithValue(sqInt fieldIndex, sqInt objOop, sqLong integerValue)
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
    sqInt fmt;
	...
	if (fmt <= (lastPointerFormat())) {
		if (isIntegerValue(integerValue)) {
			...
			assert((isNonImmediate(objOop))
			 && (!(isForwarded(objOop))));
			assert(validStorePointerUncheckedArgs(fieldIndex, objOop, (((usqInt)integerValue << 1) | 1)));
			longAtput((void *)((objOop + BaseHeaderSize) + ((((usqInt)(fieldIndex) << (shiftForWord()))))),(((usqInt)integerValue << 1) | 1));
		}
		else {
			/* primitiveFailFor: */
			GIV(primFailCode) = PrimErrInappropriate;
		...
		if (fmt < (firstShortFormat())) {
			if (fmt < (firstLongFormat())) {
				storeLong64ofObjectwithValue(fieldIndex, objOop, integerValue);
			}
			else {
				storeLong32ofObjectwithValue(fieldIndex, objOop, integerValue);
			}
		}
		else {
			if (fmt < (firstByteFormat())) {
				storeShort16ofObjectwithValue(fieldIndex, objOop, integerValue);
			}
			else {
				if (fmt < (firstCompiledMethodFormat())) {
					storeByteofObjectwithValue(fieldIndex, objOop, integerValue);
				}
				else {
					/* primitiveFailFor: */
					GIV(primFailCode) = PrimErrUnsupported;
...
storeLong32ofObjectwithValue(sqInt fieldIndex, sqInt obj, sqInt valueWord)
{
	...
	/* SpurMemoryManager>>#storeLong64:ofObject:withValue: */
static NoDbgRegParms sqLong
storeLong64ofObjectwithValue(sqInt longIndex, sqInt objOop, sqLong value)
{
	return long64Atput((void *)((objOop + BaseHeaderSize) + ((((usqInt)(longIndex) << 3)))),value);
}
...
storePointerofObjectwithValue(sqInt fieldIndex, sqInt objOop, sqInt valuePointer)
{
	...
	/* SpurMemoryManager>>#storeShort16:ofObject:withValue: */
static NoDbgRegParms sqInt
storeShort16ofObjectwithValue(sqInt shortIndex, sqInt objOop, sqInt value)
{
	return shortAtput((void *)((objOop + BaseHeaderSize) + ((((usqInt)(shortIndex) << 1)))),value);
}

	/* SpurMemoryManager>>#storeUncheckedNoAssert:ofAny:withValue: */
static NoDbgRegParms sqInt
storeUncheckedNoAssertofAnywithValue(sqInt fieldIndex, sqInt objOop, sqInt valuePointer)
{
	return longAtput((void *)((objOop + BaseHeaderSize) + ((((usqInt)(fieldIndex) << (shiftForWord()))))),valuePointer);
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (numSlots >= (numSlotsMask())) {
		longAtput((void *)(GIV(freeStart)),numSlots);
		longAtput((void *)(GIV(freeStart) + 4),((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
		long64Atput((void *)(newObj),((((((usqLong) (numSlotsMask()))) << (numSlotsFullShift()))) + ((((usqInt)(formatField) << (formatShift()))))) + ClassByteStringCompactIndex);
	}
	else {
		long64Atput((void *)(newObj),((((((usqLong) numSlots)) << (numSlotsFullShift()))) + ((((usqInt)(formatField) << (formatShift()))))) + ClassByteStringCompactIndex);
	}
...
validFreeTreeChunkparent(sqInt chunk, sqInt parent)
{
	...
	if ((reason = validFreeTreeChunkparent(fetchPointerofFreeChunk(freeChunkSmallerIndex(), chunk), chunk))) {
		return reason;
	}
	if ((reason = validFreeTreeChunkparent(fetchPointerofFreeChunk(freeChunkLargerIndex(), chunk), chunk))) {
		return reason;
	}
	return null;
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		bridgeSpan = (byteAt((void *)(bridgehead + (numSlotsFieldByteOffset())))
					? BytesPerOop * (longAt((void *)(bridge - BaseHeaderSize)))
					: 0);
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	name = fetchPointerofObject(GIV(classNameIndex), aClass);
	if (!(isBytes(name))) {
		return 0;
	}
...
/*	With the full block implementation a CompiledBlock's method is found along
	...
	the home method, or nilObj if it cannot be found. */
...
/*	Answer if the argument is a full block method or not. Full block methods
	...
	/* StackInterpreter>>#isFullBlockMethod: */
sqInt
isFullBlockMethod(sqInt aMethodObj)
{
	return isOopCompiledMethod(ultimateLiteralOf(aMethodObj));
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if ((cls = fetchClassOfNonImm(oop))) {
		className = nameOfClasslengthIntoisMetaInto(cls, (&length), (&isMeta));
		if (isMeta) {
			fprintf(GIV(transcript),
					"%p: %.*s",
					((void *)oop),
...
/*	Answer a pointer to the characters of a class's name, setting length and
	isMeta through the pointer arguments.
	Answer 'bad class', 9, false on failure to look up the name. */

	/* StackInterpreter>>#nameOfClass:lengthInto:isMetaInto: */
char *
nameOfClasslengthIntoisMetaInto(sqInt classOop, sqInt *lengthPtr, sqInt *isMetaPtr)
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
    sqInt maybeNameOop;
    sqInt maybeThisClassOop;
    usqInt numSlots;

	if (isPointers(classOop)) {
		numSlots = numSlotsOf(classOop);
		if (numSlots == GIV(metaclassNumSlots)) {
			maybeThisClassOop = fetchPointerofObject(GIV(thisClassIndex), classOop);
			if (addressCouldBeClassObj(maybeThisClassOop)) {
				...
				return nameOfClasslengthIntoisMetaInto(fetchPointerofObject(GIV(thisClassIndex), classOop), lengthPtr, null);
			}
		}
		else {
			if ((numSlots > GIV(classNameIndex))
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	/* StackInterpreter>>#printMethodReferenceFor:on: */
void
printMethodReferenceForon(sqInt aMethod, FILE *stream)
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
    sqInt homeMethod;
    sqInt isMeta;
    sqInt length;
	...
	nameOfClass = nameOfClasslengthIntoisMetaInto(safeMethodClassOf(homeMethod), (&length), (&isMeta));
	fprintf(stream,
			"%.*s%s>>#%.*s",
			((int) length),
			nameOfClass,
			...
			? " class"
			: ""),
			(methodSel == GIV(nilObj)
			? 3
			: numBytesOf(methodSel)),
			(methodSel == GIV(nilObj)
			? "nil"
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	ifNilSqInt = (cls = fetchClassOfNonImm(oop));
	if (!ifNilSqInt) {
		fprintf(GIV(transcript),
				"%p has a nil class!!\n",
				((void *)oop));
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	GIV(oldImageBaseAddress) = getLongFromFileswap(f, swapBytes);
	...
	if (!((GIV(lastHash) = seed & (identityHashHalfWordMask())))) {
		while (((GIV(lastHash) = (((usqInt) 1 /* ioUTCMicrosecondsNow */)) & (identityHashHalfWordMask()))) == 0) {
		}
	}
	GIV(savedWindowSize) = getLongFromFileswap(f, swapBytes);
	imageHeaderFlags = getLongFromFileswap(f, swapBytes);
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	literal = literalofMethod((literalCountOf(methodPointer)) - 1, methodPointer);
	if (isOopForwarded(literal)) {
		literal = followForwarded(literal);
	}
	if (!((isPointers(literal))
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	className = nameOfClasslengthIntoisMetaInto(fetchClassOfNonImm(oop), (&length), (&isMeta));
	fprintf(GIV(transcript),
			(isMeta
			? "%p: %.*s\n"
			: "%p: a(n) %.*s\n"),
