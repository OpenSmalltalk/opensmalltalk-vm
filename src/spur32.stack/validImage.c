/* Automatically generated by
	...
	(Compiler-eem.514)
   from
	ImageLeakChecker VMMaker.oscog-eem.3539 uuid: c011f0c8-e6f5-4422-af77-57ef75984fda
 */
static char __buildInfo[] = "ImageLeakChecker VMMaker.oscog-eem.3539 uuid: c011f0c8-e6f5-4422-af77-57ef75984fda " __DATE__ ;
...
static NoDbgRegParms usqInt rawOverflowSlotsOf(sqInt objOop);
...
static NoDbgRegParms usqInt rawNumSlotsOf(sqInt objOop);
...
const char *interpreterVersion = "Open Smalltalk ImageChecker VM [ImageLeakChecker VMMaker.oscog-eem.3539]";
...
l1:
	...
	if (numSlots >= (numSlotsMask())) {
		...
		long64Atput(chunk + BaseHeaderSize, (((((((usqLong) (numSlotsMask()))) << (numSlotsFullShift()))) + ((((usqInt)(formatField) << (formatShift()))))) + classIndex) | (1U << (pinnedBitShift())));
	...
	long64Atput(chunk, (((((((usqLong) numSlots)) << (numSlotsFullShift()))) + ((((usqInt)(formatField) << (formatShift()))))) + classIndex) | (1U << (pinnedBitShift())));
	checkFreeSpace(GCModeNewSpace);
	return chunk;
}
...
allocateSlotsInOldSpacebytesformatclassIndex(sqInt numSlots, usqInt totalBytes, sqInt formatField, sqInt classIndex)
{
	...
	if (numSlots >= (numSlotsMask())) {
		longAtput(chunk, numSlots);
		longAtput(chunk + 4, ((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
		long64Atput(chunk + BaseHeaderSize, ((((((usqLong) (numSlotsMask()))) << (numSlotsFullShift()))) + ((((usqInt)(formatField) << (formatShift()))))) + classIndex);
		checkFreeSpace(GCModeNewSpace);
		return chunk + BaseHeaderSize;
	}

	/* for header parsing we put a saturated slot count in the prepended overflow size word */
	long64Atput(chunk, ((((((usqLong) numSlots)) << (numSlotsFullShift()))) + ((((usqInt)(formatField) << (formatShift()))))) + classIndex);
	checkFreeSpace(GCModeNewSpace);
	return chunk;
}
...
initSegmentBridgeWithBytesat(usqLong numBytes, sqInt address)
{
	...
	if (numSlots) {
		longAtput(address, numSlots);
		longAtput(address + 4, ((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
		longAtput(address + 8, ((1U << (pinnedBitShift())) + ((((usqInt)((wordIndexableFormat())) << (formatShift()))))) + (segmentBridgePun()));
		longAtput(address + 12, ((((usqInt)((numSlotsMask())) << (numSlotsHalfShift())))) + (1U << (markedBitHalfShift())));
	}
	else {
		longAtput(address, ((1U << (pinnedBitShift())) + ((((usqInt)((wordIndexableFormat())) << (formatShift()))))) + (segmentBridgePun()));
...
objectBytesForSlots(sqInt numSlots)
{
	...
	/* Spur32BitMemoryManager>>#rawOverflowSlotsOf: */
static NoDbgRegParms usqInt
...
setFormatOfto(sqInt objOop, sqInt format)
{
	...
	longAtput(objOop, ((longAt(objOop)) & ((unsigned int)~(((sqInt)((usqInt)((formatMask())) << (formatShift())))))) + ((((usqInt)(format) << (formatShift())))));
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (isMarked(obj)) {
		...
		byteAtput(newObj + (markedBitByteOffset()), (byteAt(newObj + (markedBitByteOffset()))) | (1U << (markedBitByteShift())));

		/* begin setIsMarkedOf:to: */
		assert(!(isFreeObject(obj)));
		byteAtput(obj + (markedBitByteOffset()), (byteAt(obj + (markedBitByteOffset()))) & (0xFF - (1U << (markedBitByteShift()))));
	}
	freeObject(obj);
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (((rawNumSlotsOf(oop)) == (numSlotsMask()))
	 && ((rawNumSlotsOf(oop - BaseHeaderSize)) != (numSlotsMask()))) {
		fprintf(GIV(transcript),
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (numSlots > (maxSlotsForNewSpaceAlloc())) {
		...
		if (numSlots >= (numSlotsMask())) {
			...
			long64Atput(newObjUsqInt, ((((((usqLong) (numSlotsMask()))) << (numSlotsFullShift()))) + ((((usqInt)(fmt) << (formatShift()))))) + classIndex);
		}
		else {
			long64Atput(newObjUsqInt, ((((((usqLong) numSlots)) << (numSlotsFullShift()))) + ((((usqInt)(fmt) << (formatShift()))))) + classIndex);
		}
	...
	if ((delta <= (allocationUnit()))
	 && (isOldObject(objOop))) {
		...
		if (numSlots >= (numSlotsMask())) {
			longAtput(GIV(freeStart), numSlots);
			longAtput(GIV(freeStart) + 4, ((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
			long64Atput(newObj, ((((((usqLong) (numSlotsMask()))) << (numSlotsFullShift()))) + ((((usqInt)(format) << (formatShift()))))) + classIndex);
		}
		else {
			long64Atput(newObj, ((((((usqLong) numSlots)) << (numSlotsFullShift()))) + ((((usqInt)(format) << (formatShift()))))) + classIndex);
		}
	 ...
	 || ((fetchPointerofObject(ObjStackTopx, stackOrNil)) >= ObjStackLimit)) {
		...
		if (freeOrNewPage) {
			...
			if (GIV(marking)) {
				...
				byteAtput(freeOrNewPage + (markedBitByteOffset()), (byteAt(freeOrNewPage + (markedBitByteOffset()))) | (1U << (markedBitByteShift())));
			}
		}
...
forwardto(sqInt obj1, sqInt obj2)
{
	...
	longAtput(obj1, ((longAt(obj1)) & ((unsigned int)~(((((usqInt)((formatMask())) << (formatShift())))) + (classIndexMask())))) + ((isForwardedObjectClassIndexPun()) + ((((usqInt)((forwardedFormat())) << (formatShift()))))));
#  if IMMUTABILITY
...
{
	return (rawNumSlotsOf(objOop)) == (numSlotsMask());
}
...
{
	return ((byteAt(objOop + (markedBitByteOffset()))) & (1U << (markedBitByteShift()))) != 0;
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (!(isMarked(classObj))) {
		...
		byteAtput(classObj + (markedBitByteOffset()), (byteAt(classObj + (markedBitByteOffset()))) | (1U << (markedBitByteShift())));
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	byteAtput(objOop + (markedBitByteOffset()), (byteAt(objOop + (markedBitByteOffset()))) | (1U << (markedBitByteShift())));
	format = formatOf(objOop);
	if (isPureBitsFormat(format)) {
		...
		if (scanLargeObject) {
			...
			while (index > 0) {
				...
				if (isNonImmediate(field)) {
					...
					byteAtput(field + (markedBitByteOffset()), (byteAt(field + (markedBitByteOffset()))) | (1U << (markedBitByteShift())));
...
l1:;
			...
			while (index > 0) {
				...
				if (isNonImmediate(field)) {
					...
					/* There should be no weaklings on the mark stack. */
					noCheckPushonObjStack(field, objStack1);
					if (((rawNumSlotsOf(field)) > (traceImmediatelySlotLimit()))
...
numSlotsOf(sqInt objOop)
{
	...
	if (((numSlots = rawNumSlotsOf(objOop))) == (numSlotsMask())) {
		/* begin rawOverflowSlotsOf: */
		return longAt(objOop - BaseHeaderSize);
	}
	else {
...
printEntity(sqInt oop)
{
	...
	/* begin space */
	printChar(' ');
	printHexnpnp(rawNumSlotsOf(oop));
	print("/");
...
printHeaderOf(sqInt objOop)
{
	...
	if (hasOverflowHeader(objOop)) {
		print(" hdr16 slotf ");
		printHexnp(rawNumSlotsOf(objOop));
		print(" slotc ");
		printHexnp(rawOverflowSlotsOf(objOop));
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	longAtput((GIV(unscannedEphemerons).top = ((GIV(unscannedEphemerons).top)) + BytesPerOop), anEphemeron);
	return 1;
}
...
rawHashBitsOf(sqInt objOop)
{
	...
	/* SpurMemoryManager>>#rawNumSlotsOf: */
static NoDbgRegParms usqInt
rawNumSlotsOf(sqInt objOop)
{
	return byteAt(objOop + 7);
}
...
{
	long32Atput(objOop + 4, ((((long32At(objOop + 4)) | (identityHashHalfWordMask())) - (identityHashHalfWordMask()))) + (hash & (identityHashHalfWordMask())));
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (numSlots >= (numSlotsMask())) {
		longAtput(GIV(freeStart), numSlots);
		longAtput(GIV(freeStart) + 4, ((sqInt)((usqInt)((numSlotsMask())) << (numSlotsHalfShift()))));
		long64Atput(newObj, ((((((usqLong) (numSlotsMask()))) << (numSlotsFullShift()))) + ((((usqInt)(formatField) << (formatShift()))))) + ClassByteStringCompactIndex);
	}
	else {
		long64Atput(newObj, ((((((usqLong) numSlots)) << (numSlotsFullShift()))) + ((((usqInt)(formatField) << (formatShift()))))) + ClassByteStringCompactIndex);
	}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		if (rawNumSlotsOf(bridgehead)) {
			bridgeSpan = BytesPerOop * (rawOverflowSlotsOf(bridge));
		}
		else {
			bridgeSpan = 0;
		}
