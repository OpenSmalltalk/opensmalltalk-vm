/* Automatically generated by
	...
	(Compiler-eem.514)
   from
	ImageLeakChecker VMMaker.oscog-eem.3535 uuid: 5d68cdd6-e146-48df-836a-2394c8ae29f0
 */
static char __buildInfo[] = "ImageLeakChecker VMMaker.oscog-eem.3535 uuid: 5d68cdd6-e146-48df-836a-2394c8ae29f0 " __DATE__ ;
...
const char *interpreterVersion = "Open Smalltalk ImageChecker VM [ImageLeakChecker VMMaker.oscog-eem.3535]";
	...
	if (isPointersFormat(fmt)) {
		...
		for (i = 0; i < numSlots; i += 1) {
			...
			assert((isImmediate(oop))
			 ...
			 || (((isCompiledMethod(newObj))
			 && (i > (literalCountOf(newObj))))
			 || ((i == 0)
		...
		if (isPureBitsFormat(format)) {
			...
			for (i = 0; i < numSlots; i += 1) {
				...
				assert((isImmediate(valuePointer))
				 ...
				 || (((isCompiledMethod(copy))
				 && (i > (literalCountOf(copy))))
				 || ((i == 0)
				 && (GIV(gcPhaseInProgress) == SlidingCompactionInProgress)))))));
			...
			if (classOrNil != GIV(nilObj)) {
				...
				if (!(rawHashBitsOf(classOrNil))) {
					...
					assert((isImmediate(GIV(nilObj)))
					 ...
					 || (((isCompiledMethod(page))
					 && (j > (literalCountOf(page))))
					 || ((j == 0)
					 && (GIV(gcPhaseInProgress) == SlidingCompactionInProgress)))))));
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	assert((isImmediate(anObj))
	 ...
	 || (((isCompiledMethod(GIV(hiddenRootsObj)))
	 && (RememberedSetRootIndex > (literalCountOf(GIV(hiddenRootsObj)))))))));
