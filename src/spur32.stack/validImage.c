...
/*	Allocate an instance of a variable class, excepting CompiledMethod. */
	...
	/* Spur32BitMemoryManager>>#inOldSpaceInstantiatePinnedClass:indexableSize: */
sqInt
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	classFormat = ((fetchPointerofObject(InstanceSpecificationIndex, classObj)) >> 1);
	...
	case sixtyFourBitIndexableFormat():
		if (nElements > ((maxSlotsForAlloc()) / 2)) {
			/* begin primitiveFailFor: */
...
/*	Allocate an instance of a variable class, excepting CompiledMethod. */
	...
	/* Spur32BitMemoryManager>>#instantiateUninitializedClass:indexableSize: */
sqInt
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (numSlots > (maxSlotsForNewSpaceAlloc())) {
		...
		if (numSlots >= (numSlotsMask())) {
			...
			long64Atput(newObj1, (((((usqLong) numSlots)) << (numSlotsFullShift())) + (((sqInt)((usqInt)(instSpec) << (formatShift()))))) + classIndex);
		}
		assert((numBytes % (allocationUnit())) == 0);
		assert((newObj1 % (allocationUnit())) == 0);
		GIV(freeStart) += numBytes;
		newObj = newObj1;
	l7:	/* end allocateSlots:format:classIndex: */;
	}
	return newObj;
}
