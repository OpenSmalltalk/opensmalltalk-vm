/* Automatically generated by
   ...
   from
	GdbARMPlugin Cog-eem.122 uuid: 1ca89f5f-0258-44b3-b7c0-acb52206aaf2
 */
static char __buildInfo[] = "GdbARMPlugin Cog-eem.122 uuid: 1ca89f5f-0258-44b3-b7c0-acb52206aaf2 " __DATE__ ;
...
EXPORT(sqInt) primitiveRunInMemoryMinimumAddressERFromTo(void);
...
EXPORT(sqInt) primitiveSingleStepInMemoryMinimumAddressERFromTo(void);
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"GdbARMPlugin Cog-eem.122 (i)"
#else
	"GdbARMPlugin Cog-eem.122 (e)"
#endif
;
...
EXPORT(sqInt)
primitiveRunInMemoryMinimumAddressERFromTo(void)
{
	void *cpu;
	sqInt cpuAlien;
	unsigned long maxExecAndReadOnlyAddress;
	sqInt maybeErr;
	char *memory;
	unsigned long minAddress;
	unsigned long minExecAndReadOnlyAddress;

	success(isWordsOrBytes(stackValue(3)));
	memory = ((char *) (firstIndexableField(stackValue(3))));
	minAddress = positive32BitValueOf(stackValue(2));
	minExecAndReadOnlyAddress = positive32BitValueOf(stackValue(1));
	maxExecAndReadOnlyAddress = positive32BitValueOf(stackValue(0));
	cpuAlien = stackValue(4);
	if (failed()) {
		return null;
	}
	...
	maybeErr = runCPUInSizeMinAddressEROFromTo(cpu, memory, byteSizeOf(((sqInt)(long)(memory) - BaseHeaderSize)), minAddress, minExecAndReadOnlyAddress, maxExecAndReadOnlyAddress);
	setInterruptCheckChain(prevInterruptCheckChain);
	if (maybeErr != 0) {
		primitiveFailFor(PrimErrInappropriate);
		return null;
...
EXPORT(sqInt)
primitiveSingleStepInMemoryMinimumAddressERFromTo(void)
{
	void *cpu;
	sqInt cpuAlien;
	unsigned long maxExecAndReadOnlyAddress;
	...
	maybeErr = singleStepCPUInSizeMinAddressEROFromTo(cpu, memory, byteSizeOf(((sqInt)(long)(memory) - BaseHeaderSize)), minAddress, minExecAndReadOnlyAddress, maxExecAndReadOnlyAddress);
	setInterruptCheckChain(prevInterruptCheckChain);
	if (maybeErr != 0) {
		primitiveFailFor(PrimErrInappropriate);
		return null;
...
void* GdbARMPlugin_exports[][3] = {
	...
	{"GdbARMPlugin", "primitiveRunInMemoryMinimumAddressERFromTo", (void*)primitiveRunInMemoryMinimumAddressERFromTo},
	...
	{"GdbARMPlugin", "primitiveSingleStepInMemoryMinimumAddressERFromTo", (void*)primitiveSingleStepInMemoryMinimumAddressERFromTo},
