 ...
 */
static char __buildInfo[] = "BochsIA32Plugin Cog-eem.40 uuid: 1348aa2c-fc34-4398-84be-5f3aac98257d " __DATE__ ;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"BochsIA32Plugin Cog-eem.40 (i)"
#else
	"BochsIA32Plugin Cog-eem.40 (e)"
#endif
;
...
primitiveDisassembleAtInMemory(void) {
	...
	interpreterProxy->storePointerofObjectwithValue(0, resultObj, ((instrLenOrErr << 1) | 1));
...
primitiveErrorAndLog(void) {
	...
	interpreterProxy->storePointerofObjectwithValue(0, resultObj, (((errorAcorn()) << 1) | 1));
	if (logLen > 0) {
		interpreterProxy->pushRemappableOop(resultObj);
		logObj = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classString(), logLen);
		if (interpreterProxy->failed()) {
...
/*	<Integer> */
/*	<Integer> */
/*	Run the cpu using the argument as the memory until it halts or hits an
	exception. 
 */
...
primitiveRunInMemoryMinimumAddressReadWrite(void) {
	...
	void *cpu;
	char *memory;
	unsigned long minimumReadAddress;
	unsigned long minimumWriteAddress;
	...
	minimumReadAddress = interpreterProxy->positive32BitValueOf(interpreterProxy->stackValue(1));
	minimumWriteAddress = interpreterProxy->positive32BitValueOf(interpreterProxy->stackValue(0));
	...
	prevInterruptCheckChain = interpreterProxy->setInterruptCheckChain(forceStopRunning);
	if (prevInterruptCheckChain == (forceStopRunning)) {
		prevInterruptCheckChain == 0;
	}
	maybeErr = runCPUInSizeMinAddressReadWrite(cpu, memory, interpreterProxy->byteSizeOf(((sqInt)(long)(memory) - 4)), minimumReadAddress, minimumWriteAddress);
...
primitiveSingleStepInMemoryMinimumAddressReadWrite(void) {
	...
	maybeErr = singleStepCPUInSizeMinAddressReadWrite(cpu, memory, interpreterProxy->byteSizeOf(((sqInt)(long)(memory) - 4)), minimumReadAddress, minimumWriteAddress);
	if (maybeErr != 0) {
		interpreterProxy->primitiveFailFor(PrimErrInappropriate);
		return null;
	}
