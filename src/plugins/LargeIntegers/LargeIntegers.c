/* Automatically generated by
	SmartSyntaxPluginCodeGenerator VMMaker.oscog-eem.3408 uuid: 99ace54d-4adf-4a36-9064-0f4fe0bbaae2
   from
	LargeIntegersPlugin VMMaker.oscog-eem.3408 uuid: 99ace54d-4adf-4a36-9064-0f4fe0bbaae2
 */
static char __buildInfo[] = "LargeIntegersPlugin VMMaker.oscog-eem.3408 uuid: 99ace54d-4adf-4a36-9064-0f4fe0bbaae2 " __DATE__ ;
...
static const char *moduleName = "LargeIntegers v2.0 VMMaker.oscog-eem.3408 " INT_EXT;
...
anyBitOfLargeIntfromto(sqInt anOop, sqInt start, sqInt stopArg)
{
	...
	if (((cDigitOfat(firstIndexableField(magnitude), firstDigitIx - 1)) & firstMask) != 0) {
		return 1;
	}
	for (ix = (firstDigitIx + 1); ix < lastDigitIx; ix += 1) {
		if ((cDigitOfat(firstIndexableField(magnitude), ix - 1)) != 0) {
			return 1;
		}
	}
	if (((cDigitOfat(firstIndexableField(magnitude), lastDigitIx - 1)) & lastMask) != 0) {
		return 1;
	}
	return 0;
}
...
digitDivLargewithnegative(sqInt firstInteger, sqInt secondInteger, sqInt neg)
{
	...
	d = 32 - (cHighBit32(cDigitOfat(firstIndexableField(secondInteger), secondDigitLen - 1)));
...
digitMultiplyLargewithnegative(sqInt firstInteger, sqInt secondInteger, sqInt neg)
{
	...
	l2:	/* end cDigitMultiply:len:with:len:into:len: */;
	return (neg
...
digitSubLargewith(sqInt firstInteger, sqInt secondInteger)
{
	...
	if (firstDigitLen == secondDigitLen) {
		while ((firstDigitLen > 1)
		 && ((cDigitOfat(firstIndexableField(firstInteger), firstDigitLen - 1)) == (cDigitOfat(firstIndexableField(secondInteger), firstDigitLen - 1)))) {
			firstDigitLen -= 1;
		}
...
normalizeNegative(sqInt aLargeNegativeInteger)
{
	...
	while ((digitLen != 0)
	 && ((cDigitOfat(firstIndexableField(aLargeNegativeInteger), digitLen - 1)) == 0)) {
		digitLen -= 1;
	}
	if (digitLen == 0) {
	...
	if (digitLen <= sLen) {
		...
		val2 = val;
		if (digitLen > 1) {
			val2 = (val2 << 32) + (cDigitOfat(firstIndexableField(aLargeNegativeInteger), 0));
		}
		if (val2 <= minVal) {
...
normalizePositive(sqInt aLargePositiveInteger)
{
	...
	while ((digitLen != 0)
	 && ((cDigitOfat(firstIndexableField(aLargePositiveInteger), digitLen - 1)) == 0)) {
		digitLen -= 1;
	}
	if (digitLen == 0) {
		return integerObjectOf(0);
	...
	if (digitLen <= sLen) {
		...
		if (digitLen > 1) {
			...
			val2 = ((((unsigned long long)val2)) << 32) + (cDigitOfat(firstIndexableField(aLargePositiveInteger), 0));
		}
		if (val2 <= maxVal) {
...
primDigitDivNegative(void)
{
	...
	if (!(failed())) {
		...
		d = 32 - (cHighBit32(cDigitOfat(firstIndexableField(secondAsLargeInteger), secondDigitLen - 1)));
...
primDigitMultiplyNegative(void)
{
	...
	l2:	/* end cDigitMultiply:len:with:len:into:len: */;
		_return_value = (neg
			? normalizeNegative(prod)
			: normalizePositive(prod));
	l3:	/* end digitMultiplyLarge:with:negative: */;
		if (!(failed())) {
			methodReturnValue(_return_value);
		}
	}
...
primDigitSubtract(void)
{
	...
	if (!(failed())) {
		...
		if (firstDigitLen == secondDigitLen) {
			while ((firstDigitLen > 1)
			 && ((cDigitOfat(firstIndexableField(firstLarge), firstDigitLen - 1)) == (cDigitOfat(firstIndexableField(secondLarge), firstDigitLen - 1)))) {
				firstDigitLen -= 1;
			}
			secondDigitLen = firstDigitLen;
		}
