/* Automatically generated by
	SmartSyntaxPluginCodeGenerator VMMaker.oscog-eem.3419 uuid: 70017fea-1b4a-4603-a657-4d77106c4cf0
   from
	LargeIntegersPlugin VMMaker.oscog-eem.3419 uuid: 70017fea-1b4a-4603-a657-4d77106c4cf0
 */
static char __buildInfo[] = "LargeIntegersPlugin VMMaker.oscog-eem.3419 uuid: 70017fea-1b4a-4603-a657-4d77106c4cf0 " __DATE__ ;
...
static const char *moduleName = "LargeIntegers v2.0 VMMaker.oscog-eem.3419 " INT_EXT;
...
anyBitOfLargeIntfromto(sqInt anOop, sqInt start, sqInt stopArg)
{
	...
	unsigned int *cPointer1;
	unsigned int *cPointer2;
	unsigned int *cPointer3;
	unsigned int digit;
	usqInt firstDigitIx;
	...
	magnitude = anOop;
	stop = ((stopArg < (cDigitHighBitlen(firstIndexableField(magnitude), ((byteSizeOf(magnitude)) + 3) / 4))) ? stopArg : (cDigitHighBitlen(firstIndexableField(magnitude), ((byteSizeOf(magnitude)) + 3) / 4)));
	if (start > stop) {
		return 0;
	}
	...
	if (firstDigitIx == lastDigitIx) {
		...
		cPointer = firstIndexableField(magnitude);
		digit = SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer[firstDigitIx - 1]));
		return (digit & (firstMask & lastMask)) != 0;
	}
	if ((((/* begin cDigitOf:at: */
	(cPointer1 = firstIndexableField(magnitude)),
	SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer1[firstDigitIx - 1])))) & firstMask) != 0) {
		return 1;
	}
	for (ix = (firstDigitIx + 1); ix < lastDigitIx; ix += 1) {
		if (((/* begin cDigitOf:at: */
		(cPointer2 = firstIndexableField(magnitude)),
		SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer2[ix - 1])))) != 0) {
			return 1;
		}
	}
	if ((((/* begin cDigitOf:at: */
	(cPointer3 = firstIndexableField(magnitude)),
	SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer3[lastDigitIx - 1])))) & lastMask) != 0) {
		return 1;
	}
	return 0;
}
...
cDigitAddlenwithleninto(unsigned int *pWordShort, sqInt shortLen, unsigned int *pWordLong, sqInt longLen, unsigned int *pWordRes)
{
	...
	for (i = 0; i < shortLen; i += 1) {
		...
		pWordRes[i] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (accum & 0xFFFFFFFFU)))));
	}
...
cDigitDivlenremlenquolen(unsigned int *pDiv, sqInt divLen, unsigned int *pRem, sqInt remLen, unsigned int *pQuo, sqInt quoLen)
{
	...
	for (k = 1; k <= ql; k += 1) {
		...
		for (i = 1; i <= divLen; i += 1) {
			...
			/* begin cDigitOf:at:put: */
			pRem[l - 1] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (b & 0xFFFFFFFFU)))));
		...
		if (a > 0) {
			...
			for (i = 1; i <= divLen; i += 1) {
				...
				/* begin cDigitOf:at:put: */
				pRem[l - 1] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (a & 0xFFFFFFFFU)))));
				l += 1;
			}
		}
		/* begin cDigitOf:at:put: */
...
cDigitLshiftfromlentolen(sqInt shiftCount, unsigned int *pFrom, sqInt lenFrom, unsigned int *pTo, sqInt lenTo)
{
	...
	for (i = 0; i <= limit; i += 1) {
		...
		/* begin cDigitOf:at:put: */
		pTo[i + digitShift] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) ((carry | (((usqInt)(digit) << bitShift))) & 0xFFFFFFFFU)))));
...
cDigitMontgomerylentimeslenmodulolenmInvModBinto(unsigned int *pFirst, sqInt firstLen, unsigned int *pSecond, sqInt secondLen, unsigned int *pThird, sqInt thirdLen, unsigned int mInv, unsigned int *pRes)
{
	...
	for (i = 0; i <= limit1; i += 1) {
		...
		for (k = 1; k <= limit2; k += 1) {
			...
			/* begin cDigitOf:at:put: */
			pRes[k - 1] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (accum & 0xFFFFFFFFU)))));
			accum = (((accum) >> 32) + ((accum2) >> 32)) + ((accum3) >> 32);
		}
		...
		accum += lastDigit;
		/* begin cDigitOf:at:put: */
		pRes[limit3] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (accum & 0xFFFFFFFFU)))));
		lastDigit = (accum) >> 32;
	}
	...
	if (!((lastDigit == 0)
		...
		for (i = 0; i <= limit3; i += 1) {
			...
			/* begin cDigitOf:at:put: */
			pRes[i] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (accum & 0xFFFFFFFFU)))));
			accum = 0 - ((accum) >> 0x3F);
		}
	}
	return 0;
	...
	for (i = 0; i <= limitShort; i += 1) {
		if (((digit = SQ_SWAP_4_BYTES_IF_BIGENDIAN((pWordShort[i])))) != 0) {
			...
			for (j = 0; j <= limitLong; j += 1) {
				...
				carry = (ab) >> 32;
				/* begin cDigitOf:at:put: */
				pWordRes[k] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (ab & 0xFFFFFFFFU)))));
				k += 1;
			}
			if (k < resLen) {
				/* begin cDigitOf:at:put: */
...
cDigitRshiftfromlentolen(sqInt shiftCount, unsigned int *pFrom, sqInt lenFrom, unsigned int *pTo, sqInt lenTo)
{
	...
	for (j = start; j <= limit; j += 1) {
		...
		/* begin cDigitOf:at:put: */
		pTo[j - start] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) ((carry | (((usqInt)(digit) << leftShift))) & 0xFFFFFFFFU)))));
...
digitAddLargewith(sqInt firstInteger, sqInt secondInteger)
{
	...
	pWordShort = firstIndexableField(shortInt);
	pWordLong = firstIndexableField(longInt);
	pWordRes = firstIndexableField(sum);
	accum = 0;
	for (i1 = 0; i1 < shortDigitLen; i1 += 1) {
		...
		/* begin cDigitOf:at:put: */
		pWordRes[i1] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (accum & 0xFFFFFFFFU)))));
	}
	...
	if (over > 0) {
		...
		pFrom = firstIndexableField(sum);
		pTo = firstIndexableField(newSum);
		...
		sum = newSum;
		/* begin cDigitOf:at:put: */
		cPointer = firstIndexableField(sum);
...
digitBitLogicwithopIndex(sqInt firstInteger, sqInt secondInteger, sqInt opIx)
{
	...
	pWordShort = firstIndexableField(shortLarge);
	pWordLong = firstIndexableField(longLarge);
	pWordRes = firstIndexableField(result);
...
digitCompareLargewith(sqInt firstInteger, sqInt secondInteger)
{
	...
	return integerObjectOf((cDigitComparewithlen(firstIndexableField(firstInteger), firstIndexableField(secondInteger), firstDigitLen)));
}
...
digitDivLargewithnegative(sqInt firstInteger, sqInt secondInteger, sqInt neg)
{
	...
	d = 32 - (cHighBit32((/* begin cDigitOf:at: */
(cPointer = firstIndexableField(secondInteger)),
SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer[secondDigitLen - 1])))));
...
digitMontgomerytimesmodulomInvModB(sqInt firstLarge, sqInt secondLarge, sqInt thirdLarge, unsigned int mInv)
{
	...
	pFirst = firstIndexableField(firstLarge);
	pSecond = firstIndexableField(secondLarge);
	pThird = firstIndexableField(thirdLarge);
...
digitMultiplyLargewithnegative(sqInt firstInteger, sqInt secondInteger, sqInt neg)
{
	...
	pWordShort = firstIndexableField(shortInt);
	pWordLong = firstIndexableField(longInt);
	pWordRes = firstIndexableField(prod);
	if ((((shortLen + 3) / 4) == 1)
	 && ((pWordShort[0]) == 0)) {
		goto l5;
	}
	...
	l5:	/* end cDigitMultiply:len:with:len:into:len: */;
	return (neg
...
digitOfLargeIntat(sqInt aBytesOop, sqInt ix)
{
	...
	if (ix > (((byteSizeOf(aBytesOop)) + 3) / 4)) {
		...
		cPointer = firstIndexableField(aBytesOop);
...
digitSubLargewith(sqInt firstInteger, sqInt secondInteger)
{
	...
	if (firstDigitLen == secondDigitLen) {
		while ((firstDigitLen > 1)
		 && (((/* begin cDigitOf:at: */
		(cPointer = firstIndexableField(firstInteger)),
		SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer[firstDigitLen - 1])))) == ((/* begin cDigitOf:at: */
		(cPointer1 = firstIndexableField(secondInteger)),
		SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer1[firstDigitLen - 1])))))) {
			firstDigitLen -= 1;
		}
	...
	if ((firstDigitLen < secondDigitLen)
	 ...
	 && (((/* begin cDigitOf:at: */
	(cPointer2 = firstIndexableField(firstInteger)),
	SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer2[firstDigitLen - 1])))) < ((/* begin cDigitOf:at: */
	(cPointer3 = firstIndexableField(secondInteger)),
	SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer3[firstDigitLen - 1]))))))) {
	...
	pWordSmall = firstIndexableField(smaller);
	pWordLarge = firstIndexableField(larger);
	pWordRes = firstIndexableField(res);
	z = 0;
...
digitLshift(sqInt anOop, sqInt shiftCount)
{
	...
	if (((highBit = cDigitHighBitlen(firstIndexableField(anOop), oldDigitLen))) == 0) {
	...
	pFrom = firstIndexableField(anOop);
	pTo = firstIndexableField(newOop);
	digitShift = shiftCount / 32;
	bitShift = shiftCount % 32;
	...
	l5:	/* end cDigitLshift:from:len:to:len: */;
	return newOop;
}
...
digitRshiftlookfirst(sqInt anOop, sqInt shiftCount, sqInt a)
{
	...
	pUint32 = firstIndexableField(anOop);
	realLength = a;
	do {
		if (realLength == 0) {
			oldBitLen = 0;
	...
	l5:	/* end cDigitRshift:from:len:to:len: */;
	return newOop;
}
...
largeIntgrowTo(sqInt aBytesObject, sqInt newByteLen)
{
	...
	copyLen = (oldDigitLen < newDigitLen
		...
		: newDigitLen);
	/* begin cDigitCopyFrom:to:len: */
	pFrom = firstIndexableField(aBytesObject);
...
normalizeNegative(sqInt aLargeNegativeInteger)
{
	...
	while ((digitLen != 0)
	 && (((/* begin cDigitOf:at: */
	(cPointer1 = firstIndexableField(aLargeNegativeInteger)),
	SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer1[digitLen - 1])))) == 0)) {
		digitLen -= 1;
	}
	if (digitLen == 0) {
	...
	cPointer = firstIndexableField(aLargeNegativeInteger);
	...
	if (digitLen <= sLen) {
		...
		val2 = val;
		if (digitLen > 1) {
			val2 = (val2 << 32) + ((/* begin cDigitOf:at: */
(cPointer2 = firstIndexableField(aLargeNegativeInteger)),
SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer2[0]))));
		}
		if (val2 <= minVal) {
...
normalizePositive(sqInt aLargePositiveInteger)
{
	...
	while ((digitLen != 0)
	 && (((/* begin cDigitOf:at: */
	(cPointer1 = firstIndexableField(aLargePositiveInteger)),
	SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer1[digitLen - 1])))) == 0)) {
		digitLen -= 1;
	}
	if (digitLen == 0) {
	...
	cPointer = firstIndexableField(aLargePositiveInteger);
	val = SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer[digitLen - 1]));
	...
	if (digitLen <= sLen) {
		...
		if (digitLen > 1) {
			...
			val2 = ((((unsigned long long)val2)) << 32) + ((/* begin cDigitOf:at: */
(cPointer2 = firstIndexableField(aLargePositiveInteger)),
SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer2[0]))));
		}
		if (val2 <= maxVal) {
...
primAnyBitFromTo(void)
{
	...
	if (isIntegerObject(integer)) {
		...
		/* begin anyBitOfLargeInt:from:to: */
		magnitude = integer;
		stop = ((to < (cDigitHighBitlen(firstIndexableField(magnitude), ((byteSizeOf(magnitude)) + 3) / 4))) ? to : (cDigitHighBitlen(firstIndexableField(magnitude), ((byteSizeOf(magnitude)) + 3) / 4)));
		if (from > stop) {
			someBitIsSet = 0;
			goto l2;
		}
		firstDigitIx = ((from - 1) / 32) + 1;
		lastDigitIx = ((stop - 1) / 32) + 1;
		firstMask = 0xFFFFFFFFU << ((from - 1) & 0x1F);
		lastMask = ((usqInt)(0xFFFFFFFFU)) >> (0x1F - ((stop - 1) & 0x1F));
		if (firstDigitIx == lastDigitIx) {
			/* begin unsafeDigitOfLargeInt:at: */
		...
		someBitIsSet = 0;
	l2:	/* end anyBitOfLargeInt:from:to: */;
	}
	if (!(failed())) {
...
primDigitCompare(void)
{
	...
	if (!(failed())) {
		...
		_return_value = integerObjectOf((cDigitComparewithlen(firstIndexableField(firstInteger), firstIndexableField(secondInteger), firstDigitLen)));
	l3:	/* end digitCompareLarge:with: */;
		if (!(failed())) {
			methodReturnValue(_return_value);
		}
	}
...
primDigitDivNegative(void)
{
	...
	if (!(failed())) {
		...
		d = 32 - (cHighBit32((/* begin cDigitOf:at: */
(cPointer = firstIndexableField(secondAsLargeInteger)),
SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer[secondDigitLen - 1])))));
...
primDigitMultiplyNegative(void)
{
	...
	l5:	/* end cDigitMultiply:len:with:len:into:len: */;
		_return_value = (neg
			? normalizeNegative(prod)
			: normalizePositive(prod));
	l6:	/* end digitMultiplyLarge:with:negative: */;
		if (!(failed())) {
			methodReturnValue(_return_value);
		}
	}
...
primDigitSubtract(void)
{
	...
	if (!(failed())) {
		...
		if (firstDigitLen == secondDigitLen) {
			while ((firstDigitLen > 1)
			 && (((/* begin cDigitOf:at: */
			(cPointer = firstIndexableField(firstLarge)),
			SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer[firstDigitLen - 1])))) == ((/* begin cDigitOf:at: */
			(cPointer1 = firstIndexableField(secondLarge)),
			SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer1[firstDigitLen - 1])))))) {
				firstDigitLen -= 1;
			}
			secondDigitLen = firstDigitLen;
		...
		if ((firstDigitLen < secondDigitLen)
		 || ((firstDigitLen == secondDigitLen)
		 && (((/* begin cDigitOf:at: */
		(cPointer2 = firstIndexableField(firstLarge)),
		SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer2[firstDigitLen - 1])))) < ((/* begin cDigitOf:at: */
		(cPointer3 = firstIndexableField(secondLarge)),
		SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer3[firstDigitLen - 1]))))))) {
			larger = secondLarge;
			largeDigitLen = secondDigitLen;
			smaller = firstLarge;
...
primMontgomeryTimesModulo(void)
{
	...
	l14:	/* end digitMontgomery:times:modulo:mInvModB: */;
		if (!(failed())) {
			methodReturnValue(_return_value);
		}
	}
