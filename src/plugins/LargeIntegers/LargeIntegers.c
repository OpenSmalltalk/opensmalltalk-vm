/* Automatically generated by
   ...
   from
	LargeIntegersPlugin VMMaker.oscog-eem.3456 uuid: e316760f-1758-4b6b-aa08-f84bc7c44ef3
 */
static char __buildInfo[] = "LargeIntegersPlugin VMMaker.oscog-eem.3456 uuid: e316760f-1758-4b6b-aa08-f84bc7c44ef3 " __DATE__ ;
...
static const char *moduleName = "LargeIntegers v2.0 VMMaker.oscog-eem.3456 " INT_EXT;
...
anyBitOfLargeIntfromto(sqInt anOop, sqInt start, sqInt stopArg)
{
	...
	if ((SQ_SWAP_4_BYTES_IF_BIGENDIAN(((((unsigned int *) (firstIndexableField(magnitude))))[firstDigitIx - 1]))) & firstMask) {
		return 1;
	}
	for (ix = (firstDigitIx + 1); ix < lastDigitIx; ix += 1) {
		if (SQ_SWAP_4_BYTES_IF_BIGENDIAN(((((unsigned int *) (firstIndexableField(magnitude))))[ix - 1]))) {
			return 1;
		}
	}
	if ((SQ_SWAP_4_BYTES_IF_BIGENDIAN(((((unsigned int *) (firstIndexableField(magnitude))))[lastDigitIx - 1]))) & lastMask) {
		return 1;
	}
	return 0;
}
...
digitLshift(sqInt anOop, sqInt shiftCount)
{
	...
	if (!((highBit = cDigitHighBitlen(((unsigned int *) (firstIndexableField(anOop))), oldDigitLen)))) {
	...
	if (!bitShift) {
		...
		for (iSqInt = 0; iSqInt < (((newDigitLen - 1) - digitShift) + 1); iSqInt += 1) {
			((((unsigned int *) (firstIndexableField(newOop)))) + digitShift)[iSqInt] = (((((unsigned int *) (firstIndexableField(anOop)))))[iSqInt]);
		}
		goto l1;
	}
	rshift = 32 - bitShift;
...
normalizeNegative(sqInt aLargeNegativeInteger)
{
	...
	if (digitLen <= sLen) {
		...
		val2 = val;
		if (digitLen > 1) {
			val2 = ((val2 << 32)) + (SQ_SWAP_4_BYTES_IF_BIGENDIAN(((*((unsigned int *) (firstIndexableField(aLargeNegativeInteger)))))));
		}
		if (val2 <= minVal) {
...
normalizePositive(sqInt aLargePositiveInteger)
{
	...
	if (digitLen <= sLen) {
		...
		if (digitLen > 1) {
			...
			val2 = (((((unsigned long long)val2)) << 32)) + (SQ_SWAP_4_BYTES_IF_BIGENDIAN(((*((unsigned int *) (firstIndexableField(aLargePositiveInteger)))))));
		}
		if (val2 <= maxVal) {
...
primDigitAdd(void)
{
	...
	if (over > 0) {
		...
		for (i = 0; i < longDigitLen; i += 1) {
			(((unsigned int *) (firstIndexableField(newSum))))[i] = ((((unsigned int *) (firstIndexableField(sum))))[i]);
		}
...
l4:;
	...
	_return_value = result;
l5:	/* end digitDivLarge:with:negative: */;
	if (!(failed())) {
		methodReturnValue(_return_value);
	}
	return null;
...
primDigitMultiplyNegative(void)
{
	...
	for (i = 0; i <= limitShort; i += 1) {
		if ((digit = SQ_SWAP_4_BYTES_IF_BIGENDIAN(((((unsigned int *) (firstIndexableField(shortInt))))[i])))) {
			k = i;
