/* Automatically generated by
	SmartSyntaxPluginCodeGenerator VMMaker.oscog-eem.1809 uuid: ea36b0b1-2bb5-4763-87fc-d5fecda34867
   from
	LargeIntegersPlugin VMMaker.oscog-eem.1809 uuid: ea36b0b1-2bb5-4763-87fc-d5fecda34867
 */
static char __buildInfo[] = "LargeIntegersPlugin VMMaker.oscog-eem.1809 uuid: ea36b0b1-2bb5-4763-87fc-d5fecda34867 " __DATE__ ;
...
static sqInt byteSizeOfLargeInt(sqInt bytesOop);
...
static sqInt createLargeIntegerNegbyteLength(sqInt neg, sqInt byteLength);
static sqInt createLargeIntegerNegdigitLength(sqInt neg, sqInt digitLength);
...
static unsigned int * pointerToFirstDigitOfLargeInt(sqInt bytesObject);
...
static unsigned int unsafeDigitOfLargeIntat(sqInt anOop, sqInt ix);
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"LargeIntegers v2.0 VMMaker.oscog-eem.1809 (i)"
#else
	"LargeIntegers v2.0 VMMaker.oscog-eem.1809 (e)"
#endif
;
...
anyBitOfLargeIntfromto(sqInt anOop, sqInt start, sqInt stopArg)
{
	...
	magnitude = anOop;
	stop = ((stopArg < (cDigitHighBitlen(pointerToFirstDigitOfLargeInt(magnitude), ((slotSizeOf(magnitude)) + 3) / 4))) ? stopArg : (cDigitHighBitlen(pointerToFirstDigitOfLargeInt(magnitude), ((slotSizeOf(magnitude)) + 3) / 4)));
	if (start > stop) {
		return 0;
	}
	...
	firstMask = (((unsigned long)0xFFFFFFFFULL)) << ((start - 1) & 0x1F);
	lastMask = ((usqInt) 0xFFFFFFFFULL) >> (0x1F - ((stop - 1) & 0x1F));
	if (firstDigitIx == lastDigitIx) {
		/* begin unsafeDigitOfLargeInt:at: */
	...
	if (((unsafeDigitOfLargeIntat(magnitude, firstDigitIx)) & firstMask) != 0) {
		return 1;
	}
	for (ix = (firstDigitIx + 1); ix < lastDigitIx; ix += 1) {
		if ((unsafeDigitOfLargeIntat(magnitude, ix)) != 0) {
			return 1;
		}
	}
	if (((unsafeDigitOfLargeIntat(magnitude, lastDigitIx)) & lastMask) != 0) {
		return 1;
	}
	return 0;
}
...
/*	Answer the number of bytes required to represent a LargeInteger.
	...
	/* LargeIntegersPlugin>>#byteSizeOfLargeInt: */
static sqInt
byteSizeOfLargeInt(sqInt bytesOop)
{
...
cDigitAddlenwithleninto(unsigned int *pWordShort, sqInt shortLen, unsigned int *pWordLong, sqInt longLen, unsigned int *pWordRes)
{
	...
	for (i = 0; i < shortLen; i += 1) {
		accum = ((accum >> 32) + (cDigitOfat(pWordShort, i))) + (cDigitOfat(pWordLong, i));
		/* begin cDigitOf:at:put: */
		pWordRes[i] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (accum & 0xFFFFFFFFULL)))));
	}
	for (i = shortLen; i < longLen; i += 1) {
		accum = (accum >> 32) + (cDigitOfat(pWordLong, i));
		/* begin cDigitOf:at:put: */
		pWordRes[i] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (accum & 0xFFFFFFFFULL)))));
	}
	return accum >> 32;
...
cDigitDivlenremlenquolen(unsigned int *pDiv, sqInt divLen, unsigned int *pRem, sqInt remLen, unsigned int *pQuo, sqInt quoLen)
{
	...
	for (k = 1; k <= ql; k += 1) {
		...
		if ((cDigitOfat(pRem, j - 1)) == dh) {
			...
			/* r2 := (rem digitAt: j - 2). */
			/* begin cDigitOf:at: */
			r1r2 = ((unsigned int) (SQ_SWAP_4_BYTES_IF_BIGENDIAN((pRem[j - 1]))));
			r1r2 = (r1r2 << 32) + (cDigitOfat(pRem, j - 2));
			t = r1r2 % dh;
		...
		for (i = 1; i <= divLen; i += 1) {
			hi = (cDigitOfat(pDiv, i - 1)) * (q >> 32);
			lo = (cDigitOfat(pDiv, i - 1)) * (q & 0xFFFFFFFFULL);
			b = ((cDigitOfat(pRem, l - 1)) - a) - (lo & 0xFFFFFFFFULL);
			/* begin cDigitOf:at:put: */
			pRem[l - 1] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (b & 0xFFFFFFFFULL)))));
		...
		if (a > 0) {
			...
			a = 0;
			for (i = 1; i <= divLen; i += 1) {
				a = ((a >> 32) + (cDigitOfat(pRem, l - 1))) + (cDigitOfat(pDiv, i - 1));
				/* begin cDigitOf:at:put: */
				pRem[l - 1] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (a & 0xFFFFFFFFULL)))));
				l += 1;
			}
		}
		/* begin cDigitOf:at:put: */
...
cDigitLshiftfromlentolen(sqInt shiftCount, unsigned int *pFrom, sqInt lenFrom, unsigned int *pTo, sqInt lenTo)
{
	...
	for (i = 0; i <= limit; i += 1) {
		...
		/* begin cDigitOf:at:put: */
		aValue = ((unsigned int) ((carry | (digit << bitShift)) & 0xFFFFFFFFULL));
		pTo[i + digitShift] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN(aValue));
...
cDigitMontgomerylentimeslenmodulolenmInvModBinto(unsigned int *pFirst, sqInt firstLen, unsigned int *pSecond, sqInt secondLen, unsigned int *pThird, sqInt thirdLen, unsigned int mInv, unsigned int *pRes)
{
	...
	for (i = 0; i <= limit1; i += 1) {
		/* begin cDigitOf:at: */
		accum3 = ((unsigned int) (SQ_SWAP_4_BYTES_IF_BIGENDIAN((pFirst[i]))));
		accum3 = (accum3 * (cDigitOfat(pSecond, 0))) + (cDigitOfat(pRes, 0));
		u = (accum3 * mInv) & 0xFFFFFFFFULL;
		accum2 = u * (cDigitOfat(pThird, 0));
		accum = (accum2 & 0xFFFFFFFFULL) + (accum3 & 0xFFFFFFFFULL);
		accum = ((accum >> 32) + (accum2 >> 32)) + (accum3 >> 32);
		for (k = 1; k <= limit2; k += 1) {
			/* begin cDigitOf:at: */
			accum3 = ((unsigned int) (SQ_SWAP_4_BYTES_IF_BIGENDIAN((pFirst[i]))));
			accum3 = (accum3 * (cDigitOfat(pSecond, k))) + (cDigitOfat(pRes, k));
			accum2 = u * (cDigitOfat(pThird, k));
			accum = (accum + (accum2 & 0xFFFFFFFFULL)) + (accum3 & 0xFFFFFFFFULL);
			/* begin cDigitOf:at:put: */
			pRes[k - 1] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (accum & 0xFFFFFFFFULL)))));
			accum = ((accum >> 32) + (accum2 >> 32)) + (accum3 >> 32);
		}
		for (k = secondLen; k <= limit3; k += 1) {
			accum2 = u * (cDigitOfat(pThird, k));
			accum = (accum + (cDigitOfat(pRes, k))) + (accum2 & 0xFFFFFFFFULL);
			/* begin cDigitOf:at:put: */
			pRes[k - 1] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (accum & 0xFFFFFFFFULL)))));
		...
		accum += lastDigit;
		/* begin cDigitOf:at:put: */
		pRes[limit3] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (accum & 0xFFFFFFFFULL)))));
		lastDigit = accum >> 32;
	}
	for (i = firstLen; i <= limit3; i += 1) {
		/* begin cDigitOf:at: */
		accum = ((unsigned int) (SQ_SWAP_4_BYTES_IF_BIGENDIAN((pRes[0]))));
		u = (accum * mInv) & 0xFFFFFFFFULL;
		accum += u * (cDigitOfat(pThird, 0));
		accum = accum >> 32;
	...
	if (!((lastDigit == 0)
		...
		for (i = 0; i <= limit3; i += 1) {
			accum = (accum + (cDigitOfat(pRes, i))) - (cDigitOfat(pThird, i));
			/* begin cDigitOf:at:put: */
			pRes[i] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (accum & 0xFFFFFFFFULL)))));
			accum = 0 - (accum >> 0x3F);
		}
	}
	return 0;
...
cDigitMultiplylenwithlenintolen(unsigned int *pWordShort, sqInt shortLen, unsigned int *pWordLong, sqInt longLen, unsigned int *pWordRes, sqInt resLen)
{
	...
	for (i = 0; i <= limitShort; i += 1) {
		if (((digit = cDigitOfat(pWordShort, i))) != 0) {
			k = i;
			...
			for (j = 0; j <= limitLong; j += 1) {
				/* begin cDigitOf:at: */
				ab = ((unsigned int) (SQ_SWAP_4_BYTES_IF_BIGENDIAN((pWordLong[j]))));
				ab = ((ab * digit) + carry) + (cDigitOfat(pWordRes, k));
				carry = ab >> 32;
				/* begin cDigitOf:at:put: */
				pWordRes[k] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (ab & 0xFFFFFFFFULL)))));
				k += 1;
			}
			if (k < resLen) {
				/* begin cDigitOf:at:put: */
...
cDigitRshiftfromlentolen(sqInt shiftCount, unsigned int *pFrom, sqInt lenFrom, unsigned int *pTo, sqInt lenTo)
{
	...
	if (bitShift == 0) {
		...
		/* Fast version for digit-aligned shifts */
		/* C indexed! */
		return cDigitCopyFromtolen(pFrom + digitShift, pTo, ((lenTo - 1)) + 1);
	}
	leftShift = 32 - bitShift;
	carry = ((usqInt) (cDigitOfat(pFrom, digitShift))) >> bitShift;
	start = digitShift + 1;
	limit = lenFrom - 1;
	for (j = start; j <= limit; j += 1) {
		...
		/* begin cDigitOf:at:put: */
		aValue = ((unsigned int) ((carry | (digit << leftShift)) & 0xFFFFFFFFULL));
		pTo[j - start] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN(aValue));
...
cDigitSublenwithleninto(unsigned int *pWordSmall, sqInt smallLen, unsigned int *pWordLarge, sqInt largeLen, unsigned int *pWordRes)
{
	...
	for (i = 0; i < smallLen; i += 1) {
		z = (z + (cDigitOfat(pWordLarge, i))) - (cDigitOfat(pWordSmall, i));
		/* begin cDigitOf:at:put: */
		pWordRes[i] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (z & 0xFFFFFFFFULL)))));
		z = 0 - (z >> 0x3F);
	}
	for (i = smallLen; i < largeLen; i += 1) {
		z += cDigitOfat(pWordLarge, i);
		/* begin cDigitOf:at:put: */
		pWordRes[i] = (SQ_SWAP_4_BYTES_IF_BIGENDIAN((((unsigned int) (z & 0xFFFFFFFFULL)))));
		z = 0 - (z >> 0x3F);
	}
...
l2:	/* end byteSizeOfCSI: */;
	...
	/* begin pointerToFirstDigitOfLargeInt: */
	...
	/* LargeIntegersPlugin>>#createLargeIntegerNeg:byteLength: */
static sqInt
createLargeIntegerNegbyteLength(sqInt neg, sqInt byteLength)
{
	return instantiateClassindexableSize((neg
		? classLargeNegativeInteger()
		: classLargePositiveInteger()), byteLength);
}

	/* LargeIntegersPlugin>>#createLargeIntegerNeg:digitLength: */
static sqInt
createLargeIntegerNegdigitLength(sqInt neg, sqInt digitLength)
{
	return instantiateClassindexableSize((neg
		? classLargeNegativeInteger()
		: classLargePositiveInteger()), digitLength * 4);
}
...
digitAddLargewith(sqInt firstInteger, sqInt secondInteger)
{
	...
	neg = (fetchClassOf(firstInteger)) == (classLargeNegativeInteger());
...
#endif /* SPURVM */
;
		cDigitCopyFromtolen(pointerToFirstDigitOfLargeInt(sum), pointerToFirstDigitOfLargeInt(newSum), longDigitLen);
...
digitCompareLargewith(sqInt firstInteger, sqInt secondInteger)
{
	...
	return integerObjectOf((cDigitComparewithlen(pointerToFirstDigitOfLargeInt(firstInteger), pointerToFirstDigitOfLargeInt(secondInteger), firstDigitLen)));
}
...
;
	...
	d = 32 - (cHighBit32(unsafeDigitOfLargeIntat(secondInteger, secondDigitLen)));
...
digitOfLargeIntat(sqInt aBytesOop, sqInt ix)
{
	...
	if (ix > (((slotSizeOf(aBytesOop)) + 3) / 4)) {
		...
		return ((unsigned int) (SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer[ix - 1]))));
	}
}
...
digitSubLargewith(sqInt firstInteger, sqInt secondInteger)
{
	...
	if (firstDigitLen == secondDigitLen) {
		while ((firstDigitLen > 1)
		 && ((unsafeDigitOfLargeIntat(firstInteger, firstDigitLen)) == (unsafeDigitOfLargeIntat(secondInteger, firstDigitLen)))) {
			firstDigitLen -= 1;
		}
...
digitLshift(sqInt anOop, sqInt shiftCount)
{
	...
	if (((highBit = cDigitHighBitlen(pointerToFirstDigitOfLargeInt(anOop), oldDigitLen))) == 0) {
...
;
	...
	if (bitShift == 0) {
		...
		/* Fast version for digit-aligned shifts */
		/* C indexed! */
		cDigitCopyFromtolen(pFrom, pTo + digitShift, ((newDigitLen - 1) - digitShift) + 1);
		goto l4;
	}
	rshift = 32 - bitShift;
	carry = 0;
...
l4:	/* end cDigitLshift:from:len:to:len: */;
	return newOop;
}
...
;
	...
	if (bitShift == 0) {
		...
		/* Fast version for digit-aligned shifts */
		/* C indexed! */
		cDigitCopyFromtolen(pFrom + digitShift, pTo, ((newDigitLen - 1)) + 1);
		goto l6;
	}
	leftShift = 32 - bitShift;
	carry = ((usqInt) (cDigitOfat(pFrom, digitShift))) >> bitShift;
...
l6:	/* end cDigitRshift:from:len:to:len: */;
	return newOop;
}
...
isNormalized(sqInt anInteger)
{
	...
	if ((unsafeByteOfLargeIntat(anInteger, len)) == 0) {
		return 0;
	}
...
(maxVal = MaxSmallInteger),
			(((usqInt) ((anInteger < 0
					? 0 - anInteger
					: anInteger))) >> (((sLen / 4) - 1) * 32)) > (((usqInt) ((maxVal < 0
					? 0 - maxVal
					: maxVal))) >> (((sLen / 4) - 1) * 32)))
		: (/* begin unsafeDigitOfLargeInt:at: */
			(cPointer = ((unsigned int *) (firstIndexableField(anInteger)))),
			(val = ((unsigned int) (SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer[(len / 4) - 1]))))),
			(sLen > 4
					? (val = (val << 32) + (unsafeDigitOfLargeIntat(anInteger, 1)))
					: 0),
...
;
	...
	copyLen = (oldDigitLen < newDigitLen
		? oldDigitLen
		: newDigitLen);
	cDigitCopyFromtolen(pointerToFirstDigitOfLargeInt(aBytesObject), pointerToFirstDigitOfLargeInt(newBytes), copyLen);
	return newBytes;
}
...
normalizeNegative(sqInt aLargeNegativeInteger)
{
	...
	while ((digitLen != 0)
	 && ((unsafeDigitOfLargeIntat(aLargeNegativeInteger, digitLen)) == 0)) {
		digitLen -= 1;
	}
	if (digitLen == 0) {
	...
	val = ((unsigned int) (SQ_SWAP_4_BYTES_IF_BIGENDIAN((cPointer[digitLen - 1]))));
	...
	if (digitLen <= sLen) {
		...
		val2 = val;
		if (digitLen > 1) {
			val2 = (val2 << 32) + (unsafeDigitOfLargeIntat(aLargeNegativeInteger, 1));
		}
		if (val2 <= minVal) {
...
normalizePositive(sqInt aLargePositiveInteger)
{
	...
	while ((digitLen != 0)
	 && ((unsafeDigitOfLargeIntat(aLargePositiveInteger, digitLen)) == 0)) {
		digitLen -= 1;
	}
	if (digitLen == 0) {
		return integerObjectOf(0);
	...
	if (digitLen <= sLen) {
		...
		val2 = val;
		if (digitLen > 1) {
			val2 = (val2 << 32) + (unsafeDigitOfLargeIntat(aLargePositiveInteger, 1));
		}
		if (val2 <= maxVal) {
...
normalize(sqInt aLargeInteger)
{
	...
	/* LargeIntegersPlugin>>#pointerToFirstDigitOfLargeInt: */
static unsigned int *
pointerToFirstDigitOfLargeInt(sqInt bytesObject)
{
	return ((unsigned int *) (firstIndexableField(bytesObject)));
}
...
;
	...
	neg = (fetchClassOf(firstLarge)) == (classLargeNegativeInteger());
	if (firstDigitLen <= secondDigitLen) {
		shortInt = firstLarge;
		shortDigitLen = firstDigitLen;
		longInt = secondLarge;
...
primDigitCompare(void)
{
	...
	if (isIntegerObject(firstInteger)) {
		...
		if (isIntegerObject(secondInteger)) {
			...
			_return_value = integerObjectOf((cDigitComparewithlen(pointerToFirstDigitOfLargeInt(firstInteger), pointerToFirstDigitOfLargeInt(secondInteger), firstDigitLen)));
...
;
	...
	d = 32 - (cHighBit32(unsafeDigitOfLargeIntat(secondAsLargeInteger, secondDigitLen)));
...
;
	...
	stObjectatput(result, 2, rem);
	_return_value = result;
l12:	/* end digitDivLarge:with:negative: */;
	if (failed()) {
		return null;
	}
	popthenPush(3, _return_value);
...
;
	...
	firstNeg = (fetchClassOf(firstLarge)) == (classLargeNegativeInteger());
	firstDigitLen = ((slotSizeOf(firstLarge)) + 3) / 4;
	secondDigitLen = ((slotSizeOf(secondLarge)) + 3) / 4;
	if (firstDigitLen == secondDigitLen) {
		while ((firstDigitLen > 1)
		 && ((unsafeDigitOfLargeIntat(firstLarge, firstDigitLen)) == (unsafeDigitOfLargeIntat(secondLarge, firstDigitLen)))) {
			firstDigitLen -= 1;
		}
		secondDigitLen = firstDigitLen;
	}
...
l15:	/* end digitMontgomery:times:modulo:mInvModB: */;
	if (failed()) {
		return null;
	}
...
/*	Argument must not be aSmallInteger! */
	...
	/* LargeIntegersPlugin>>#unsafeDigitOfLargeInt:at: */
static unsigned int
unsafeDigitOfLargeIntat(sqInt anOop, sqInt ix)
{
	unsigned int *cPointer;

	/* begin cDigitOf:at: */
	cPointer = ((unsigned int *) (firstIndexableField(anOop)));
...
void* LargeIntegers_exports[][3] = {
	...
	{(void*)_m, "primAnyBitFromTo\000\003", (void*)primAnyBitFromTo},
...
signed char primAnyBitFromToAccessorDepth = 3;
