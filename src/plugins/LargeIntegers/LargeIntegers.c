...
static sqInt digitDivLargewithnegative(sqInt firstInteger, sqInt secondInteger, sqInt neg) {
	...
	d = 8 - (cHighBit((((unsigned char *) (interpreterProxy->firstIndexableField(secondInteger))))[secondLen - 1]));
	interpreterProxy->pushRemappableOop(firstInteger);
...
static sqInt digitOfLargeIntat(sqInt aBytesOop, sqInt ix) {
	if (ix > (interpreterProxy->slotSizeOf(aBytesOop))) {
		...
		return (((unsigned char *) (interpreterProxy->firstIndexableField(aBytesOop))))[ix - 1];
	}
	return null;
}
...
static sqInt digitSubLargewith(sqInt firstInteger, sqInt secondInteger) {
	...
	secondLen = interpreterProxy->slotSizeOf(secondInteger);
	if (firstLen == secondLen) {
		while ((firstLen > 1) && ((digitOfLargeIntat(firstInteger, firstLen)) == (digitOfLargeIntat(secondInteger, firstLen)))) {
			firstLen -= 1;
		}
		secondLen = firstLen;
	}
	if ((firstLen < secondLen) || ((firstLen == secondLen) && ((digitOfLargeIntat(firstInteger, firstLen)) < (digitOfLargeIntat(secondInteger, firstLen))))) {
...
l1:	/* end digitLength: */;
	...
	if ((interpreterProxy->fetchClassOf(anInteger)) == (interpreterProxy->classLargePositiveInteger())) {
		...
		maxVal = 0x3FFFFFFF;
		return (((pointer1 = interpreterProxy->firstIndexableField(anInteger)))[sLen - 1]) > ((((usqInt) ((maxVal < 0
	? 0 - maxVal
	: maxVal))) >> ((sLen - 1) * 8)) & 0xFF);
	} else {
		...
		minVal = -1073741824;
		if ((((pointer2 = interpreterProxy->firstIndexableField(anInteger)))[sLen - 1]) < ((((usqInt) ((minVal < 0
	? 0 - minVal
	: minVal))) >> ((sLen - 1) * 8)) & 0xFF)) {
			return 0;
		} else {
			...
			for (ix = 1; ix <= sLen; ix += 1) {
				if (!((((pointer3 = interpreterProxy->firstIndexableField(anInteger)))[ix - 1]) == ((((usqInt) ((minVal < 0
	? 0 - minVal
	: minVal))) >> ((ix - 1) * 8)) & 0xFF))) {
					return 1;
				}
...
static sqInt largeIntOrIntgrowTo(sqInt oop, sqInt len) {
	...
	if ((oop & 1)) {
		...
		newBytes1 = interpreterProxy->instantiateClassindexableSize(interpreterProxy->fetchClassOf(oop), len);
		oop = interpreterProxy->popRemappableOop();
		oldLen = interpreterProxy->slotSizeOf(oop);
		if (oldLen < len) {
			copyLen = oldLen;
		} else {
			copyLen = len;
		}
		cDigitCopyFromtolen(interpreterProxy->firstIndexableField(oop), interpreterProxy->firstIndexableField(newBytes1), copyLen);
		newBytes = newBytes1;
	}
	return newBytes;
}
...
static sqInt normalizeNegative(sqInt aLargeNegativeInteger) {
	...
	if (len <= sLen) {
		...
		minVal = -1073741824;
		if ((len < sLen) || ((digitOfBytesat(aLargeNegativeInteger, sLen)) < ((((usqInt) ((minVal < 0
	? 0 - minVal
	: minVal))) >> ((sLen - 1) * 8)) & 0xFF))) {
		...
		for (i = 1; i <= sLen; i += 1) {
			...
			if (!((digitOfBytesat(aLargeNegativeInteger, i)) == ((((usqInt) ((minVal < 0
	? 0 - minVal
	: minVal))) >> ((i - 1) * 8)) & 0xFF))) {
...
static sqInt normalizePositive(sqInt aLargePositiveInteger) {
	...
	sLen = 4;
	if ((len <= sLen) && ((digitOfBytesat(aLargePositiveInteger, sLen)) <= ((((usqInt) ((0x3FFFFFFF < 0
	? 0 - 0x3FFFFFFF
	: 0x3FFFFFFF))) >> ((sLen - 1) * 8)) & 0xFF))) {
...
EXPORT(sqInt) primAnyBitFromTo(void) {
	...
	_return_value = ((anyBitOfLargeIntfromto(large, from, to))? interpreterProxy->trueObject(): interpreterProxy->falseObject());
	if (interpreterProxy->failed()) {
		return null;
	}
...
EXPORT(sqInt) primDigitBitAnd(void) {
	...
	_return_value = digitBitLogicwithopIndex(firstInteger, secondInteger, (sqInt)andOpIndex);
	if (interpreterProxy->failed()) {
		return null;
	}
	interpreterProxy->popthenPush(2, _return_value);
...
EXPORT(sqInt) primDigitBitOr(void) {
	...
	_return_value = digitBitLogicwithopIndex(firstInteger, secondInteger, (sqInt)orOpIndex);
	if (interpreterProxy->failed()) {
		return null;
	}
	interpreterProxy->popthenPush(2, _return_value);
...
EXPORT(sqInt) primDigitBitXor(void) {
	...
	_return_value = digitBitLogicwithopIndex(firstInteger, secondInteger, (sqInt)xorOpIndex);
	if (interpreterProxy->failed()) {
		return null;
	}
	interpreterProxy->popthenPush(2, _return_value);
...
EXPORT(sqInt) primGetModuleName(void) {
	...
	strncpy(interpreterProxy->firstIndexableField(strOop), moduleName, strLen);
	if (interpreterProxy->failed()) {
		return null;
	}
...
void* LargeIntegers_exports[][3] = {
	...
	{"LargeIntegers", "primMontgomeryDigitLength", (void*)primMontgomeryDigitLength},
