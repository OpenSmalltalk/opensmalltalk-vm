...
static FT_Encoding ftEncodingValueFromString(sqInt string) {
	...
	interpreterProxy->success((interpreterProxy->isBytes(string)) && ((interpreterProxy->slotSizeOf(string)) == (sizeof(FT_Encoding))));
...
EXPORT(sqInt) primitiveLibraryHandle(void) {
	...
	errorCode = 0;
	ftAllocateHandleInReceiverForPointer((void  *)library);
	if (interpreterProxy->failed()) {
		return ftParameterError();
	}
	return null;
