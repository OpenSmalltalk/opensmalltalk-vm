/* Automatically generated by
	VMPluginCodeGenerator VMMaker.oscog-dtl.2311 uuid: c13d1e0b-fb1a-453d-9786-c93d33f42e00
   from
...
#      endif /* defined(WIN32) */
	...
	l1:	/* end putStatFor:intoBuffer:targetName: */;
		...
		storePointerofObjectwithValue(8, statArray, ((sizeof(time_t)) == 4
			? signed32BitIntegerFor((statBufPointer1->st_atime))
			: signed64BitIntegerFor((statBufPointer1->st_atime))));
		storePointerofObjectwithValue(9, statArray, ((sizeof(time_t)) == 4
			? signed32BitIntegerFor((statBufPointer1->st_mtime))
			: signed64BitIntegerFor((statBufPointer1->st_mtime))));
		storePointerofObjectwithValue(10, statArray, ((sizeof(time_t)) == 4
			? signed32BitIntegerFor((statBufPointer1->st_ctime))
			: signed64BitIntegerFor((statBufPointer1->st_ctime))));
...
oopFromTimeT(time_t posixSeconds)
{
	return ((sizeof(time_t)) == 4
		? signed32BitIntegerFor(posixSeconds)
		: signed64BitIntegerFor(posixSeconds));
}
...
#    endif /* defined(WIN32) */
	...
	l13:	/* end putStatFor:intoBuffer:targetName: */;
		...
		if (attributeNumber == 10) {
			...
			resultOop = ((sizeof(time_t)) == 4
				? signed32BitIntegerFor(posixSeconds1)
				: signed64BitIntegerFor(posixSeconds1));
		}
		if (attributeNumber == 11) {
			/* begin oopFromTimeT: */
			posixSeconds2 = ((time_t) ((statBuf.st_ctime)));
			resultOop = ((sizeof(time_t)) == 4
				? signed32BitIntegerFor(posixSeconds2)
				: signed64BitIntegerFor(posixSeconds2));
		}
	}
	else {
		if (attributeNumber == 12) {
...
{
	popthenPush(1, stringFromCString("1.2.4"));
	return 0;
}
...
statArrayFortoArrayfromfileName(char *cPathName, sqInt attributeArray, struct stat *statBufPointer, sqInt fileNameOop)
{
	...
	storePointerofObjectwithValue(8, attributeArray, ((sizeof(time_t)) == 4
		? signed32BitIntegerFor((statBufPointer->st_atime))
		: signed64BitIntegerFor((statBufPointer->st_atime))));
	storePointerofObjectwithValue(9, attributeArray, ((sizeof(time_t)) == 4
		? signed32BitIntegerFor((statBufPointer->st_mtime))
		: signed64BitIntegerFor((statBufPointer->st_mtime))));
	storePointerofObjectwithValue(10, attributeArray, ((sizeof(time_t)) == 4
		? signed32BitIntegerFor((statBufPointer->st_ctime))
		: signed64BitIntegerFor((statBufPointer->st_ctime))));
