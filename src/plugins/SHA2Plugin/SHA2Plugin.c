 ...
 */
static char __buildInfo[] = "SHA2Plugin CryptographyPlugins-eem.27 uuid: 63bc0f04-a4d3-4347-84fc-20b64ea64093 " __DATE__ ;
...
static sqInt primitiveCopyDoubleWordsIntoBytesBigEndian(void);
static sqInt primitiveCopyWordsIntoBytesBigEndian(void);
static sqInt primitiveIsPluginAvailable(void);
static sqInt primitiveSHA256ProcessBufferUpdatingHash(void);
static sqInt primitiveSHA512ProcessBufferUpdatingHash(void);
...
static const char *moduleName = "SHA2Plugin CryptographyPlugins-eem.27 " INT_EXT;
...
primitiveSHA512ProcessBufferUpdatingHash(void)
{
	...
	if (VMBIGENDIAN) {
		...
		for (i1 = 0; i1 < 16; i1 += 1) {
			w[i1] = (SQ_SWAP_8_BYTES(((((unsigned long long*) buffer))[i1])));
		}
	}
