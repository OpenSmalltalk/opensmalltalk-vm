...
primitiveDecodeState(void)
{
	...
	array = ((unsigned int *) (firstIndexableField(bytesOop)));
	if (VMBIGENDIAN) {
		for (i = 0; i < 4; i += 1) {
...
primitiveInitializeState(void)
{
	...
	state = ((unsigned int *) (firstIndexableField(bytesOop)));
	state[0] = 1732584193;
	state[1] = 4023233417U;
	state[2] = 2562383102U;
	state[3] = 271733878;
...
primitiveProcessBufferWithState(void)
{
	...
	buffer = ((unsigned int *) (firstIndexableField(bufferOop)));
	state = ((unsigned int *) (firstIndexableField(stateOop)));
	a = state[0];
	b = state[1];
	c = state[2];
	d = state[3];
...
void* MD5Plugin_exports[][3] = {
	...
	{(void*)_m, "primitiveDecodeState\000\000", (void*)primitiveDecodeState},
	{(void*)_m, "primitiveInitializeState\000\000", (void*)primitiveInitializeState},
	{(void*)_m, "primitivePluginAvailable\000\377", (void*)primitivePluginAvailable},
	{(void*)_m, "primitiveProcessBufferWithState\000\000", (void*)primitiveProcessBufferWithState},
...
signed char primitiveDecodeStateAccessorDepth = 0;
signed char primitiveInitializeStateAccessorDepth = 0;
signed char primitiveProcessBufferWithStateAccessorDepth = 0;
