...
EXPORT(sqInt) primitiveHostWindowPosition(void) {
	...
	if (pos == -1) {
		...
		_return_value = interpreterProxy->makePointwithxValueyValue(((usqInt) pos) >> 16, pos & 0xFFFF);
		if (interpreterProxy->failed()) {
			return null;
		}
...
EXPORT(sqInt) primitiveHostWindowSize(void) {
	...
	if (size == -1) {
		...
		_return_value = interpreterProxy->makePointwithxValueyValue(((usqInt) size) >> 16, size & 0xFFFF);
		if (interpreterProxy->failed()) {
			return null;
		}
		interpreterProxy->popthenPush(2, _return_value);
