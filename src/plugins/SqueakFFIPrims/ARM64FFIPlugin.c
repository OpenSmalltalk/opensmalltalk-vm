/* Automatically generated by
   ...
   from
	ThreadedARM64FFIPlugin VMMaker.oscog-eem.2999 uuid: c7933314-4edf-4570-b6d8-3891b13c233d
 */
static char __buildInfo[] = "ThreadedARM64FFIPlugin VMMaker.oscog-eem.2999 uuid: c7933314-4edf-4570-b6d8-3891b13c233d " __DATE__ ;
...
static sqInt ffiPushStructureofSizetypeSpecofLengthin(void *pointer, sqInt structSize, unsigned int *argSpec, sqInt argSpecSize, CalloutState *calloutState);
...
static const char *moduleName = "ARM64FFIPlugin VMMaker.oscog-eem.2999 " INT_EXT;
...
ffiCreateIntegralResultOopofAtomicTypein(usqLong retVal, sqInt atomicType, CalloutState *calloutState)
{
	...
	if (atomicType <= FFITypeSignedInt) {
		...
		return (((atomicType & 1) != 0)
			? (/* begin signedMachineIntegerFor: */
				(BytesPerWord == 8
						? signed64BitIntegerFor(((sqIntptr_t) retVal))
						: signed32BitIntegerFor(((sqIntptr_t) retVal))))
			: (/* begin positiveMachineIntegerFor: */
				(BytesPerWord == 8
						? positive64BitIntegerFor(((usqIntptr_t) retVal))
						: positive32BitIntegerFor(((usqIntptr_t) retVal)))));
	}
	if ((((usqInt)(atomicType)) >> 1) == (((usqInt)(FFITypeSignedLongLong)) >> 1)) {
		return (((atomicType & 1) != 0)
...
/*	See IHI0055B_aapcs64.pdf sections 4.3.5 & 5.4.2 Stage C; we don't yet
	support HVA's
 */

	/* ThreadedARM64FFIPlugin>>#ffiPushStructure:ofSize:typeSpec:ofLength:in: */
static sqInt
ffiPushStructureofSizetypeSpecofLengthin(void *pointer, sqInt structSize, unsigned int *argSpec, sqInt argSpecSize, CalloutState *calloutState)
{
    sqInt availableRegisterSpace;
    sqInt roundedSize;
    sqInt stackPartSize;
	...
	if (structIsHomogenousFloatArrayOfSizetypeSpecofLength(structSize, argSpec, argSpecSize)) {
		...
		if (structSize <= availableRegisterSpace) {
			...
			/* Stage C, step C.2, all in floating-point registers (!!) */
			memcpy(((void *) ((&(((calloutState->floatRegisters))[(calloutState->floatRegisterIndex)])))), pointer, structSize);
			(calloutState->floatRegisterIndex = ((calloutState->floatRegisterIndex)) + ((((usqInt)((structSize + 7))) >> 3)));
			return 0;
		}
		availableRegisterSpace = 0;
