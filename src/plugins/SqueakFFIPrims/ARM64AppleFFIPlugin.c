/* Automatically generated by
   ...
   from
	ThreadedARM64AppleFFIPlugin VMMaker.oscog-eem.3227 uuid: 3f4b7c6c-fd6b-4555-96a7-b62d694b8ef7
 */
static char __buildInfo[] = "ThreadedARM64AppleFFIPlugin VMMaker.oscog-eem.3227 uuid: 3f4b7c6c-fd6b-4555-96a7-b62d694b8ef7 " __DATE__ ;
...
static const char *moduleName = "ARM64AppleFFIPlugin VMMaker.oscog-eem.3227 " INT_EXT;
...
#  endif
	...
	l7:	/* end ffiReturnCStringFrom: */;
		...
		longLongRetPtr1 = ((calloutState->structReturnType)
			...
			: (calloutState->limit));
		/* begin ffiReturnType: */
		specLiteral2 = (argArrayOrNil != null
			? stackValue(1)
			: literalofMethod(0, primitiveMethod()));
	...
	if (atomicType <= FFITypeSignedInt32) {
		...
		if (((atomicType & 1) != 0)) {
			...
			result = (BytesPerWord == 8
				? signed64BitIntegerFor(((sqIntptr_t) intRet))
				: signed32BitIntegerFor(((sqIntptr_t) intRet)));
		}
		else {
			/* begin positiveMachineIntegerFor: */
			result = (BytesPerWord == 8
				? positive64BitIntegerFor(((usqIntptr_t) intRet))
				: positive32BitIntegerFor(((usqIntptr_t) intRet)));
		}
		goto l10;
	}
...
#  if COGMTVM
	...
	retryCount = 0;
	while (1) {
		result = ffiCallArgArrayOrNilNumArgs(externalFunction, null, nArgs);
		if (!((result == PrimErrObjectMayMove)
		 && (((retryCount += 1)) <= (nArgs + 1)))) break;
		tenuringIncrementalGC();
	}
#  else // COGMTVM
	ffiCallArgArrayOrNilNumArgs(externalFunction, null, methodArgumentCount());
#  endif
	return;
}
...
#  if COGMTVM
	...
	while (1) {
		...
		result = ffiCallArgArrayOrNilNumArgs(externalFunction, argArray, nArgs);
		if (!((result == PrimErrObjectMayMove)
		 && (((retryCount += 1)) <= (nArgs + 1)))) break;
		tenuringIncrementalGC();
	}
#  else // COGMTVM
	...
	nArgs = slotSizeOf(argArray);
	ffiCallArgArrayOrNilNumArgs(externalFunction, argArray, nArgs);
#  endif // COGMTVM
	return;
}
