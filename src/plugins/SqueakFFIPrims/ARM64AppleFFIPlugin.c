/* Automatically generated by
	...
	(Compiler-eem.512)
   from
	ThreadedARM64AppleFFIPlugin VMMaker.oscog-eem.3477 uuid: 2de18dc7-c0eb-403e-953d-0fddeb535cfb
 */
static char __buildInfo[] = "ThreadedARM64AppleFFIPlugin VMMaker.oscog-eem.3477 uuid: 2de18dc7-c0eb-403e-953d-0fddeb535cfb " __DATE__ ;
...
static const char *moduleName = "ARM64AppleFFIPlugin VMMaker.oscog-eem.3477 " INT_EXT;
static sqInt nilObj;
...
#      endif // COGMTVM
			...
			if ((((usqInt)(atomicType)) >> 1) == (((usqInt)(FFITypeSignedInt8)) >> 1)) {
				...
				if (isExternalAddress) {
					pointer = ((void *) (longAt(valueOop + BaseHeaderSize)));
				...
				if (isAlienObj) {
					pointer = pointerForOop(/* startOfData: */
							((longAt(valueOop + BaseHeaderSize)) > 0
		...
		if (((((calloutState->ffiRetHeader)) & FFIFlagPointer) != 0)) {
			...
			if (retClass == nilObj) {
				...
				if ((((usqInt)(atomicType1)) >> 1) == (((usqInt)(FFITypeSignedChar8)) >> 1)) {
					result = (!(stringForCString(((usqInt) intRet)))
							? fetchPointerofObject(PrimErrNoMemory + 1, primitiveErrorTable())
							: stringForCString(((usqInt) intRet)));
					goto l3;
				}
