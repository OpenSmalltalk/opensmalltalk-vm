/* Automatically generated by
	...
	(Compiler-ct.535)
   from
	ThreadedARM64AppleFFIPlugin VMMaker.oscog-eem.3792 uuid: b00773bb-68a0-441a-b1b0-211aa4a67e97
 */
static char __buildInfo[] = "ThreadedARM64AppleFFIPlugin VMMaker.oscog-eem.3792 uuid: b00773bb-68a0-441a-b1b0-211aa4a67e97 " __DATE__ ;
...
typedef struct {
	...
	void *errorVariable;
	int	integerRegisterIndex;
	int	floatRegisterIndex;
	int	returnSpecSize;
	int	isHomogenousFloatArrayReturn;
...
static const char *moduleName = "ARM64AppleFFIPlugin VMMaker.oscog-eem.3792 " INT_EXT;
static sqInt nilObj;
#if !COGMTVM
			...
			case 8:  // ffiPushUnsigned64:in:
				if ((((calloutState->integerRegisterIndex)) < NumIntRegArgs)
				 && ((!(((calloutState->ffiArgHeader)) & FFIFlagVariadic)))) {
					...
					if (misAlignedBytes) {
						(calloutState->currentArg = ((calloutState->currentArg)) + (8 - misAlignedBytes));
					}
			...
			case 14: // ffiPushUnsigned16:in:
				if ((((calloutState->integerRegisterIndex)) < NumIntRegArgs)
				 && ((!(((calloutState->ffiArgHeader)) & FFIFlagVariadic)))) {
					...
					if (misAlignedBytes) {
						(calloutState->currentArg = ((calloutState->currentArg)) + (2 - misAlignedBytes));
					}
...
l4:
	...
	if ((((calloutState->ffiRetHeader)) & (FFIFlagPointer | FFIFlagStructure)) == FFIFlagStructure) {
		...
		(calloutState->returnSpecSize = byteSizeOf(argSpec));
		(calloutState->isHomogenousFloatArrayReturn = structIsHomogenousFloatArrayOfSizetypeSpecofLength(((calloutState->ffiRetHeader)) & FFIStructSizeMask, ((unsigned int *) (firstIndexableField(argSpec))), ((calloutState->returnSpecSize)) / (sizeof(unsigned int))));
	}
	err = 0;
...
l5:
	...
	if (((((usqInt)(atomicType)) >> 1) == (((usqInt)(FFITypeSingleFloat)) >> 1))
	 ...
	 && ((calloutState->isHomogenousFloatArrayReturn)))) {
 ...
 || (isCharacterAtomicType((calloutState->errorResultType)))))));
		...
		if (((calloutState->returnSpecSize)) > ((calloutState->structReturnSize))) {
		...
		if (((((calloutState->ffiRetHeader)) & FFIFlagPointer) != 0)) {
			...
			if (retClass == nilObj) {
				...
				if ((((usqInt)(atomicType1)) >> 1) == (((usqInt)(FFITypeSignedChar8)) >> 1)) {
					result = (intRet
								? ((ifNilSqInt = stringForCString(((char *) intRet))),
								(!(ifNilSqInt)
