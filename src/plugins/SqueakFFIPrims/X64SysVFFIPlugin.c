/* Automatically generated by
   ...
   from
	ThreadedX64SysVFFIPlugin VMMaker.oscog-mt.3362 uuid: b8f74e14-6430-924d-93fb-2fc1cd50ba34
 */
static char __buildInfo[] = "ThreadedX64SysVFFIPlugin VMMaker.oscog-mt.3362 uuid: b8f74e14-6430-924d-93fb-2fc1cd50ba34 " __DATE__ ;
...
#define PluginVersionInfo " VMMaker.oscog-mt.3362"
#define WordSize 8
...
static const char *moduleName = "X64SysVFFIPlugin VMMaker.oscog-mt.3362 " INT_EXT;
...
#        endif // COGMTVM
	...
	l3:	/* end ffiValidateExternalData:AtomicType: */;
			...
			if ((isExternalAddress = oopClass == (classExternalAddress()))) {
				isAlienObj = (isStringObj = 0);
			}
			else {
				if ((isStringObj = includesBehaviorThatOf(oopClass, classString()))) {
					isAlienObj = 0;
				}
				else {
					isAlienObj = includesBehaviorThatOf(oopClass, classAlien());
				}
			}
			if ((((usqInt)(atomicType2)) >> 1) == (((usqInt)(FFITypeSignedChar8)) >> 1)) {
				...
				atomicType2 = FFITypeUnsignedInt8;
			}
#      if COGMTVM
			...
			if ((!isExternalAddress)
			 ...
			 && (((!isAlienObj)
			 || ((assert(isKindOfClass(oop, classAlien())),
...
#      endif // COGMTVM
			if ((((usqInt)(atomicType2)) >> 1) == (((usqInt)(FFITypeSignedInt8)) >> 1)) {
			...
			if (atomicType2 <= FFITypeDoubleFloat) {
				if (isExternalAddress) {
					/* begin ffiPushPointer:in: */
					pointer4 = ((void *) (longAt(oop + BaseHeaderSize)));
					if (((calloutState->integerRegisterIndex)) < NumIntRegArgs) {
				...
				if (isAlienObj) {
					/* begin ffiPushPointer:in: */
					pointer5 = pointerForOop(((longAt(oop + BaseHeaderSize)) > 0
						? (oop + BaseHeaderSize) + BytesPerOop
						: longAt((oop + BaseHeaderSize) + BytesPerOop)));
					if (((calloutState->integerRegisterIndex)) < NumIntRegArgs) {
				...
				if (isWordsOrBytes(oop)) {
					/* begin ffiPushPointer:in: */
					pointer6 = firstIndexableField(oop);
					if (((calloutState->integerRegisterIndex)) < NumIntRegArgs) {
...
#    endif // COGMTVM
		requiredStackSize = (externalFunctionInstSize > ExternalFunctionStackSizeIndex
			...
			: -1);
		if (failed()) {
			result = primitiveFailFor(PrimErrBadMethod);
			goto l10;
		}
...
#        endif
	...
	l6:	/* end ffiReturnCStringFrom: */;
			...
			case FFITypeUnsignedChar16:
				result1 = characterObjectOf(intRet & 0xFFFF);
				break;

			case FFITypeUnsignedChar32:
				result1 = characterObjectOf(((unsigned int) intRet));
				break;
		...
		registerType1 = (calloutState->structReturnType);
...
#  else // COGMTVM
	...
	l18:	/* end ffiContentsOfHandle:errCode: */;
	if (failed()) {
		address2 = 0;
		goto l12;
	}
...
#  endif // COGMTVM
	...
	/* begin ffiCheckReturn:With:in: */
	if (!(argClass1 == (nilObject()))) {
		if (!(includesBehaviorThatOf(argClass1, classExternalStructure()))) {
			err1 = FFIErrorBadReturn;
			goto l15;
		}
	}
	if (!((isWords(argSpec1))
		 && ((slotSizeOf(argSpec1)) > 0))) {
		err1 = FFIErrorWrongType;
		goto l15;
	}
	(calloutState1->ffiRetSpec = argSpec1);
	(calloutState1->ffiRetHeader = fetchLong32ofObject(0, argSpec1));
	if (!(((((calloutState1->ffiRetHeader)) & FFIFlagAtomic) != 0))) {
		if (argClass1 == (nilObject())) {
			err1 = FFIErrorBadReturn;
			goto l15;
		}
	}
	if ((((calloutState1->ffiRetHeader)) & (FFIFlagPointer | FFIFlagStructure)) == FFIFlagStructure) {
		(calloutState1->structReturnSize = ((calloutState1->ffiRetHeader)) & FFIStructSizeMask);
		/* begin encodeStructReturnTypeIn: */
		registerType2 = (((calloutState1->structReturnSize)) <= (WordSize * 2)
			? registerTypeForStructSpecsOfLength(firstIndexableField((calloutState1->ffiRetSpec)), slotSizeOf((calloutState1->ffiRetSpec)))
			: 6);
		(calloutState1->structReturnType = registerType2);
	}
	err1 = 0;
	l15:	/* end ffiCheckReturn:With:in: */;
	if (err1 != 0) {
		ffiFail(err1);
		goto l20;
	}
...
#      endif
	...
	vmHandle1 = disownVM((((((calloutState1->callFlags)) & FFICallFlagThreaded) != 0)
		? DisownVMForFFICall + DisownVMForThreading
		: DisownVMForFFICall));
	...
	if (!returnStructByValue1) {
		...
		if (((((calloutState1->ffiRetHeader)) & FFIFlagPointer) != 0)) {
			/* begin ffiReturnPointer:ofType:in: */
			specLiteral2 = literalofMethod(0, primitiveMethod());
			...
			if (retClass2 == (nilObject())) {
				...
				if ((((usqInt)(atomicType11)) >> 1) == (((usqInt)(FFITypeSignedChar8)) >> 1)) {
	...
	l16:	/* end ffiReturnCStringFrom: */;
		...
		/* begin ffiCreateIntegralResultOop:ofAtomicType:in: */
		assert(!((isFloatAtomicType(atomicType2))));
		if (atomicType2 == FFITypeBool) {
		...
		if (atomicType2 <= FFITypeSignedInt32) {
			...
			if (atomicType2 <= (FFITypeSignedInt32)) {
				...
				/* # of significant bits */
				shift1 = (atomicType2 >= FFITypeUnsignedInt32
					? 32
		...
		if ((((usqInt)(atomicType2)) >> 1) == (((usqInt)(FFITypeSignedInt64)) >> 1)) {
			result2 = (((atomicType2 & 1) != 0)
				? signed64BitIntegerFor(intRet1)
				: positive64BitIntegerFor(intRet1));
			goto l11;
		}
		assert(isCharacterAtomicType(atomicType2));
		switch (atomicType2) {
		case FFITypeUnsignedChar8:
		case FFITypeSignedChar8:
		...
		case FFITypeUnsignedChar16:
			result2 = characterObjectOf(intRet1 & 0xFFFF);
			break;

		case FFITypeUnsignedChar32:
			result2 = characterObjectOf(((unsigned int) intRet1));
			break;
	...
	registerType11 = (calloutState1->structReturnType);
	switch (registerType11) {
	case 0:
	...
	ownVM(vmHandle1);
	/* begin ffiReturnStruct:ofType:in: */
	specLiteral11 = literalofMethod(0, primitiveMethod());
