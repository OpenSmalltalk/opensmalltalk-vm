/* Automatically generated by
   ...
   from
	ThreadedX64SysVFFIPlugin VMMaker.oscog-eem.2692 uuid: 63be09a4-63a7-465c-8f34-48d50ba631f8
 */
static char __buildInfo[] = "ThreadedX64SysVFFIPlugin VMMaker.oscog-eem.2692 uuid: 63be09a4-63a7-465c-8f34-48d50ba631f8 " __DATE__ ;
...
static sqInt registerTypeForStructSpecsOfLengthStartingAtByteOffsetEightbyteOffsetIsUnion(sqInt initialRegisterType, unsigned int *specs, sqInt specSize, unsigned int *indexPtr, sqInt initialByteOffset, sqInt initialEightbyteOffset, sqInt isUnion);
...
static const char *moduleName = "X64SysVFFIPlugin VMMaker.oscog-eem.2692 " INT_EXT;
...
registerTypeForStructSpecsOfLength(unsigned int *specs, sqInt specSize)
{
	...
	eightByteOffset = 0;
	byteOffset = 0;
	registerType = (((specs[index]) & FFIStructSizeMask) <= 8
		? 4
		: 0);
	while (((index += 1)) < specSize) {
		...
		if (!(spec == FFIFlagStructure)) {
			...
			/* this marks end of structure and should be ignored */
			if (spec & FFIFlagPointer) {
				fieldSize = BytesPerWord;
				alignment = fieldSize;
				isInt = 1;
...
/*	Answer with a number characterizing the register type for passing a
	struct/union of size <= 16 bytes.
	The bit at offset i of registerType is set to 1 if eight-byte at offset i
	is a int register (RAX ...)
	The bit at offset 2 indicates if there is a single eightbyte (struct size
	<= 8)
	* 2r00 for float float (XMM0 XMM1)
 ...
 */
	...
	/* ThreadedX64SysVFFIPlugin>>#registerType:ForStructSpecs:OfLength:StartingAt:ByteOffset:EightbyteOffset:IsUnion: */
static sqInt
registerTypeForStructSpecsOfLengthStartingAtByteOffsetEightbyteOffsetIsUnion(sqInt initialRegisterType, unsigned int *specs, sqInt specSize, unsigned int *indexPtr, sqInt initialByteOffset, sqInt initialEightbyteOffset, sqInt isUnion)
{
    sqInt alignment;
    sqInt atomic;
    sqInt byteOffset;
		...
		if (spec == FFIFlagStructure) {
			...
			if (spec & FFIFlagPointer) {
				...
				case FFIFlagStructure:
					...
					subIndex = indexPtr[0];
					alignment = alignmentOfStructSpecOfLengthStartingAt(specs, specSize, (&subIndex));
					recurse = (isUnionSpecOfLengthStartingAt(specs, specSize, indexPtr[0])) != isUnion;
					if (!recurse) {
						subLevel += 1;
					}
					break;
			...
			if (recurse) {
				...
				registerType = registerTypeForStructSpecsOfLengthStartingAtByteOffsetEightbyteOffsetIsUnion(registerType, specs, specSize, indexPtr, byteOffset, eightbyteOffset, !isUnion);
			}
			if (!isUnion) {
