...
/* Automatically generated by
   ...
   from
	FFIPlugin VMMaker-dtl.370 uuid: bdc970fd-9c84-4ece-8746-21060933384a
 */
static char __buildInfo[] = "FFIPlugin VMMaker-dtl.370 uuid: bdc970fd-9c84-4ece-8746-21060933384a " __DATE__ ;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"SqueakFFIPrims 14 September 2015 (i)"
#else
	"SqueakFFIPrims 14 September 2015 (e)"
#endif
;
...
static sqInt ffiCalloutToWithFlags(sqInt address, sqInt callType) {
	...
	if (atomicType1 < FFITypeSingleFloat) {
		if ((((usqInt) atomicType1) >> 1) == (((usqInt) FFITypeSignedLongLong) >> 1)) {
			...
			retOop1 = interpreterProxy->fetchPointerofObject(retVal & 0xFF, interpreterProxy->characterTable());
			goto l1;
		}
	}
...
static sqInt ffiCreateLongLongReturn(sqInt isSigned) {
	...
	for (i = 4; i <= (nBytes - 1); i += 1) {
		ptr[i] = ((((usqInt) highWord) >> ((i - 4) * 8)) & 0xFF);
	}
	ptr[3] = ((((usqInt) lowWord) >> 24) & 0xFF);
	ptr[2] = ((((usqInt) lowWord) >> 16) & 0xFF);
	ptr[1] = ((((usqInt) lowWord) >> 8) & 0xFF);
	ptr[0] = (lowWord & 0xFF);
	return largeInt;
}
