/* Automatically generated by
	VMPluginCodeGenerator VMMaker.oscog-eem.303 uuid: 99b81fe2-3c89-43bb-8a2c-2541842da7e0
   from
	ThreadedIA32FFIPlugin VMMaker.oscog-eem.303 uuid: 99b81fe2-3c89-43bb-8a2c-2541842da7e0
 */
static char __buildInfo[] = "ThreadedIA32FFIPlugin VMMaker.oscog-eem.303 uuid: 99b81fe2-3c89-43bb-8a2c-2541842da7e0 " __DATE__ ;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"SqueakFFIPrims VMMaker.oscog-eem.303 (i)"
#else
	"SqueakFFIPrims VMMaker.oscog-eem.303 (e)"
#endif
;
	...
	if (atomicType < FFITypeSingleFloat) {
		...
		if ((((usqInt) atomicType) >> 1) == (((usqInt) FFITypeSignedLongLong) >> 1)) {
			...
			if (oopClass == (classCharacter())) {
				intValue = fetchIntegerofObject(0, oop);
				goto l1;
			}
			...
			intValue = signed32BitValueOf(oop);
		...
		if (((calloutState->ffiArgHeader)) & FFIFlagAtomic) {
			...
			if (isStruct) {
				...
				if (atomicType != FFITypeVoid) {
					...
					if ((slotSizeOf(ptrType)) < 2) {
						err = FFIErrorWrongType;
						goto l3;
					}
					specOop = fetchPointerofObject(0, ptrType);
					if (isIntegerObject(specOop)) {
						err = FFIErrorWrongType;
						goto l3;
					}
					if (!(isWords(specOop))) {
		...
		if (atomicType1 < FFITypeSingleFloat) {
			...
			if ((((usqInt) atomicType1) >> 1) == (((usqInt) FFITypeSignedLongLong) >> 1)) {
				...
				if (oopClass2 == (classCharacter())) {
					intValue = fetchIntegerofObject(0, valueOop);
					goto l1;
				}
				...
				intValue = signed32BitValueOf(valueOop);
			l1:	/* end ffiIntegerValueOf: */;
			}
			if (failed()) {
				return FFIErrorCoercionFailed;
...
ffiCreateIntegralResultOopofAtomicTypein(usqLong retVal, sqInt atomicType, CalloutState *calloutState)
{
	...
	if ((((usqInt) atomicType) >> 1) == (((usqInt) FFITypeSignedLongLong) >> 1)) {
		...
		return fetchPointerofObject(retVal & 255, characterTable());
	}
}
...
ffiIntegerValueOf(sqInt oop)
{
	...
	if (oopClass == (classCharacter())) {
		return fetchIntegerofObject(0, oop);
	}
	if (oopClass == (classLargePositiveInteger())) {
...
ffiLoadCalloutAddressFrom(sqInt oop)
{
	...
	/* begin ffiContentsOfHandle:errCode: */
	if (isIntegerObject(moduleHandlePtr)) {
	...
	if (!((byteSizeOf(moduleHandlePtr)) == 4)) {
		moduleHandle1 = ffiFail(FFIErrorBadExternalLibrary);
		goto l1;
	}
...
primitiveCallout(void)
{
	...
	if (!((literalCountOf(meth)) > 0)) {
		primitiveFailFor(PrimErrBadMethod); return;
	}
...
#  else /* COGMTVM */
	...
	/* begin ffiContentsOfHandle:errCode: */
	if (isIntegerObject(addressPtr1)) {
...
primitiveCalloutWithArgs(void)
{
	...
	if (!((methodArgumentCount()) == 1)) {
		primitiveFailFor(PrimErrBadNumArgs); return;
	}
...
#  if COGMTVM
	...
	while (1) {
		...
		if (!(isArray(argArray))) {
			primitiveFailFor(PrimErrBadArgument); return;
		}
