/* Automatically generated by
   ...
   from
	ThreadedIA32FFIPlugin VMMaker.oscog-eem.222 uuid: 9255d122-7216-4700-8ead-493416ce0cf6
 */
static char __buildInfo[] = "ThreadedIA32FFIPlugin VMMaker.oscog-eem.222 uuid: 9255d122-7216-4700-8ead-493416ce0cf6 " __DATE__ ;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"SqueakFFIPrims VMMaker.oscog-eem.222 (i)"
#else
	"SqueakFFIPrims VMMaker.oscog-eem.222 (e)"
#endif
;
...
#  if COGMTVM
	...
	while (1) {
		...
		address2 = interpreterProxy->fetchPointerofObject(0, addressPtr);
	...
	l8:	/* end ffiLoadCalloutAddress: */;
		...
		requiredStackSize = (externalFunctionInstSize > ExternalFunctionStackSizeIndex
			...
			: -1);
		if (interpreterProxy->failed()) {
			result = interpreterProxy->primitiveFailFor((null == null
				? PrimErrBadMethod
				: PrimErrBadReceiver));
			goto l3;
		}
	...
	l2:	/* end ffiLogCallout: */;
		...
		if (((calloutState->ffiRetHeader)) & FFIFlagPointer) {
			...
			if (retClass1 == (interpreterProxy->nilObject())) {
				...
				if ((((usqInt) atomicType1) >> 1) == (((usqInt) FFITypeSignedChar) >> 1)) {
					...
					/* begin ffiReturnCStringFrom: */
					if ((((usqInt) intRet)) == null) {
						result1 = interpreterProxy->methodReturnValue(interpreterProxy->nilObject());
						goto l11;
					}
					cString = ((char *) (((usqInt) intRet)));
					strLen = 0;
					...
					strOop = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classString(), strLen);
					strPtr = interpreterProxy->firstIndexableField(strOop);
					for (i2 = 0; i2 <= (strLen - 1); i2 += 1) {
						strPtr[i2] = (cString[i2]);
					}
					result1 = interpreterProxy->methodReturnValue(strOop);
					goto l11;
				}
				interpreterProxy->pushRemappableOop(retType);
				oop11 = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classExternalAddress(), 4);
				ptr2 = interpreterProxy->firstIndexableField(oop11);
				ptr2[0] = (((sqInt) intRet));
				interpreterProxy->pushRemappableOop(oop11);
				retOop1 = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classExternalData(), 0);

				/* external address */

				oop11 = interpreterProxy->popRemappableOop();
				interpreterProxy->storePointerofObjectwithValue(0, retOop1, oop11);

				/* return type */

				oop11 = interpreterProxy->popRemappableOop();
				interpreterProxy->storePointerofObjectwithValue(1, retOop1, oop11);
				result1 = interpreterProxy->methodReturnValue(retOop1);
				goto l11;
			}
			interpreterProxy->pushRemappableOop(retClass1);
			classOop = (((calloutState->ffiRetHeader)) & FFIFlagStructure
				? interpreterProxy->classByteArray()
				: interpreterProxy->classExternalAddress());
			oop11 = interpreterProxy->instantiateClassindexableSize(classOop, 4);
			ptr2 = interpreterProxy->firstIndexableField(oop11);
			ptr2[0] = (((sqInt) intRet));
		...
		if (((calloutState->ffiRetHeader)) & FFIFlagStructure) {
			...
			retOop = interpreterProxy->popRemappableOop();
			interpreterProxy->storePointerofObjectwithValue(0, retOop, oop2);
			result1 = interpreterProxy->methodReturnValue(retOop);
			goto l11;
		}
		if ((atomicType == FFITypeSingleFloat)
		 || (atomicType == FFITypeDoubleFloat)) {
			result1 = interpreterProxy->methodReturnValue(interpreterProxy->floatObjectOf(floatRet));
			goto l11;
		}
		result1 = interpreterProxy->methodReturnValue(ffiCreateIntegralResultOopofAtomicTypein(intRet, atomicType, calloutState));
...
#  else /* COGMTVM */
	...
	flags1 = interpreterProxy->fetchIntegerofObject(ExternalFunctionFlagsIndex, externalFunction);
	if (interpreterProxy->failed()) {
	...
	addressPtr1 = interpreterProxy->fetchPointerofObject(0, externalFunction);
	/* begin ffiContentsOfHandle:errCode: */
	if (interpreterProxy->isIntegerObject(addressPtr1)) {
		address3 = ffiFail(FFIErrorBadAddress);
		goto l9;
	}
	if (!(interpreterProxy->isBytes(addressPtr1))) {
		address3 = ffiFail(FFIErrorBadAddress);
		goto l9;
	}
	if (!((interpreterProxy->byteSizeOf(addressPtr1)) == 4)) {
		address3 = ffiFail(FFIErrorBadAddress);
		goto l9;
	}
	address3 = interpreterProxy->fetchPointerofObject(0, addressPtr1);
l9:	/* end ffiContentsOfHandle:errCode: */;
	...
	if (address3 == 0) {
		...
		ptr1 = interpreterProxy->firstIndexableField(addressPtr1);
		ptr1[0] = address3;
	}
	address1 = address3;
l10:	/* end ffiLoadCalloutAddress: */;
	...
	argTypeArray1 = interpreterProxy->fetchPointerofObject(ExternalFunctionArgTypesIndex, externalFunction);
	if (!((interpreterProxy->isArray(argTypeArray1))
		 && ((interpreterProxy->slotSizeOf(argTypeArray1)) == (nArgs1 + 1)))) {
		ffiFail(FFIErrorBadArgs);
		goto l6;
	}
	...
	argType1 = interpreterProxy->fetchPointerofObject(0, argTypeArray1);
	argSpec1 = interpreterProxy->fetchPointerofObject(0, argType1);
	argClass1 = interpreterProxy->fetchPointerofObject(1, argType1);
	...
	for (i1 = 1; i1 <= nArgs1; i1 += 1) {
		argType1 = interpreterProxy->fetchPointerofObject(i1, argTypeArray1);
		argSpec1 = interpreterProxy->fetchPointerofObject(0, argType1);
		argClass1 = interpreterProxy->fetchPointerofObject(1, argType1);
		oop1 = (null == null
			? interpreterProxy->stackValue(nArgs1 - i1)
			: interpreterProxy->fetchPointerofObject(i1 - 1, null));
	...
	/* begin ffiLogCallout: */
	if (ffiLogEnabled) {
		functionName1 = interpreterProxy->fetchPointerofObject(externalFunctionInstSize, externalFunction);
		if (!(interpreterProxy->isBytes(functionName1))) {
			null;
			goto l5;
		}
		ffiLogCallOfLength(interpreterProxy->firstIndexableField(functionName1), interpreterProxy->byteSizeOf(functionName1));
	}
l5:	/* end ffiLogCallout: */;
	if ((requiredStackSize1 < 0)
	 && (externalFunctionInstSize > ExternalFunctionStackSizeIndex)) {
		stackSize1 = ((calloutState1->currentArg)) - ((calloutState1->argVector));
		interpreterProxy->storeIntegerofObjectwithValue(ExternalFunctionStackSizeIndex, externalFunction, stackSize1);
	}
	/* begin ffiCalloutTo:SpecOnStack:in: */
	...
	if (((calloutState1->ffiRetHeader)) & FFIFlagPointer) {
		...
		specLiteral11 = (null != null
			? interpreterProxy->stackValue(1)
			: interpreterProxy->literalofMethod(0, interpreterProxy->primitiveMethod()));
		argTypes11 = interpreterProxy->fetchPointerofObject(ExternalFunctionArgTypesIndex, specLiteral11);
		retType1 = interpreterProxy->fetchPointerofObject(0, argTypes11);
		retClass11 = interpreterProxy->fetchPointerofObject(1, retType1);
		if (retClass11 == (interpreterProxy->nilObject())) {
			...
			if ((((usqInt) atomicType11) >> 1) == (((usqInt) FFITypeSignedChar) >> 1)) {
				...
				if ((((usqInt) intRet1)) == null) {
					result2 = interpreterProxy->methodReturnValue(interpreterProxy->nilObject());
					goto l12;
				}
				...
				strOop1 = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classString(), strLen1);
				strPtr1 = interpreterProxy->firstIndexableField(strOop1);
				for (i3 = 0; i3 <= (strLen1 - 1); i3 += 1) {
					strPtr1[i3] = (cString1[i3]);
				}
				result2 = interpreterProxy->methodReturnValue(strOop1);
				goto l12;
			}
			interpreterProxy->pushRemappableOop(retType1);
			oop12 = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classExternalAddress(), 4);
			ptr3 = interpreterProxy->firstIndexableField(oop12);
			ptr3[0] = (((sqInt) intRet1));
			interpreterProxy->pushRemappableOop(oop12);
			retOop11 = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classExternalData(), 0);

			/* external address */

			oop12 = interpreterProxy->popRemappableOop();
			interpreterProxy->storePointerofObjectwithValue(0, retOop11, oop12);

			/* return type */

			oop12 = interpreterProxy->popRemappableOop();
			interpreterProxy->storePointerofObjectwithValue(1, retOop11, oop12);
			result2 = interpreterProxy->methodReturnValue(retOop11);
			goto l12;
		}
		interpreterProxy->pushRemappableOop(retClass11);
		classOop1 = (((calloutState1->ffiRetHeader)) & FFIFlagStructure
			? interpreterProxy->classByteArray()
			: interpreterProxy->classExternalAddress());
		oop12 = interpreterProxy->instantiateClassindexableSize(classOop1, 4);
		ptr3 = interpreterProxy->firstIndexableField(oop12);
		ptr3[0] = (((sqInt) intRet1));

		/* return class */

		retClass11 = interpreterProxy->popRemappableOop();
		interpreterProxy->pushRemappableOop(oop12);
		retOop11 = interpreterProxy->instantiateClassindexableSize(retClass11, 0);
	...
	if (((calloutState1->ffiRetHeader)) & FFIFlagStructure) {
		...
		specLiteral2 = (null != null
			? interpreterProxy->stackValue(1)
			: interpreterProxy->literalofMethod(0, interpreterProxy->primitiveMethod()));
		argTypes2 = interpreterProxy->fetchPointerofObject(ExternalFunctionArgTypesIndex, specLiteral2);
		ffiRetType1 = interpreterProxy->fetchPointerofObject(0, argTypes2);
		retClass2 = interpreterProxy->fetchPointerofObject(1, ffiRetType1);
		retOop2 = interpreterProxy->instantiateClassindexableSize(retClass2, 0);
		interpreterProxy->pushRemappableOop(retOop2);
		oop3 = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classByteArray(), (calloutState1->structReturnSize));
		if (returnStructInRegisters((calloutState1->structReturnSize))) {
			memcpy(interpreterProxy->firstIndexableField(oop3), (&intRet1), (calloutState1->structReturnSize));
		}
		else {
			memcpy(interpreterProxy->firstIndexableField(oop3), (calloutState1->limit), (calloutState1->structReturnSize));
		}
		retOop2 = interpreterProxy->popRemappableOop();
		interpreterProxy->storePointerofObjectwithValue(0, retOop2, oop3);
		result2 = interpreterProxy->methodReturnValue(retOop2);
		goto l12;
	}
	if ((atomicType2 == FFITypeSingleFloat)
	 || (atomicType2 == FFITypeDoubleFloat)) {
		result2 = interpreterProxy->methodReturnValue(interpreterProxy->floatObjectOf(floatRet1));
		goto l12;
	}
	result2 = interpreterProxy->methodReturnValue(ffiCreateIntegralResultOopofAtomicTypein(intRet1, atomicType2, calloutState1));
...
#  if COGMTVM
	...
	l8:	/* end ffiLoadCalloutAddress: */;
		...
		requiredStackSize = (externalFunctionInstSize > ExternalFunctionStackSizeIndex
			...
			: -1);
		if (interpreterProxy->failed()) {
			result = interpreterProxy->primitiveFailFor((argArray == null
				? PrimErrBadMethod
				: PrimErrBadReceiver));
			goto l3;
		}
...
l10:	/* end ffiLoadCalloutAddress: */;
	...
	argTypeArray1 = interpreterProxy->fetchPointerofObject(ExternalFunctionArgTypesIndex, externalFunction);
	if (!((interpreterProxy->isArray(argTypeArray1))
		 && ((interpreterProxy->slotSizeOf(argTypeArray1)) == (nArgs + 1)))) {
		ffiFail(FFIErrorBadArgs);
		goto l6;
	}
	...
	for (i1 = 1; i1 <= nArgs; i1 += 1) {
		...
		argClass1 = interpreterProxy->fetchPointerofObject(1, argType1);
		oop1 = (argArray == null
			? interpreterProxy->stackValue(nArgs - i1)
			: interpreterProxy->fetchPointerofObject(i1 - 1, argArray));
		err1 = ffiArgumentSpecClassin(oop1, argSpec1, argClass1, calloutState1);
		if (err1 != 0) {
			/* begin cleanupCalloutState: */
			while (((calloutState1->stringArgIndex)) > 0) {
