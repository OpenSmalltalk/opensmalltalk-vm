/* Automatically generated by
   ...
   from
	IA32ABIPlugin VMMaker-oscog.8 uuid: b336429e-652e-4551-8d38-a00ef67eaa61
 */
static char __buildInfo[] = "IA32ABIPlugin VMMaker-oscog.8 uuid: b336429e-652e-4551-8d38-a00ef67eaa61 " __DATE__ ;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"IA32ABI VMMaker-oscog.8 (i)"
#else
	"IA32ABI VMMaker-oscog.8 (e)"
#endif
;
...
primAlienReplace(void) {
	...
	if (interpreterProxy->includesBehaviorThatOf(interpreterProxy->fetchClassOf(array), interpreterProxy->classAlien())) {
		...
		dest = ((startOfDatawithSize(array, totalLength)) + start) - 1;
		if (totalLength == 0) {
	...
	if ((repl & 1)) {
		...
		if ((interpreterProxy->fetchClassOf(repl)) == (interpreterProxy->classLargePositiveInteger())) {
			...
			if (interpreterProxy->includesBehaviorThatOf(interpreterProxy->fetchClassOf(repl), interpreterProxy->classAlien())) {
				...
				src = ((startOfDatawithSize(repl, totalLength)) + replStart) - 1;
				if (totalLength == 0) {
...
primDoubleAt(void) {
	...
	byteOffset = (interpreterProxy->stackIntegerValue(0)) - 1;
	...
	addr = (startOfData(rcvr)) + byteOffset;
...
primDoubleAtPut(void) {
	...
	byteOffset = (interpreterProxy->stackIntegerValue(1)) - 1;
...
primReturnFromContextThrough(void) {
	...
	jmpbuf = interpreterProxy->positive32BitValueOf(interpreterProxy->stackValue(0));
	if ((interpreterProxy->failed())
	 || (jmpbuf == 0)) {
		return interpreterProxy->primitiveFailFor(PrimErrBadArgument);
	}

	/* N.B.  Once reestablishContextPriorToCallback: has run the stack for this primitive can
	 no longer be accessed, since the current context is now that prior to the callback.
	 So we must grab the address now before changing the active context. */

	context = interpreterProxy->stackValue(1);
	/* begin startOfData: */
	rcvr = interpreterProxy->stackValue(2);
	if ((longAt(rcvr + BaseHeaderSize)) > 0) {
		address = (rcvr + BaseHeaderSize) + BytesPerOop;
		goto l1;
	}
	else {
		address = longAt((rcvr + BaseHeaderSize) + BytesPerOop);
		goto l1;
	}
l1:	/* end startOfData: */;
	if (!(interpreterProxy->reestablishContextPriorToCallback(context))) {
		return interpreterProxy->primitiveFailFor(PrimErrBadArgument);
	}
	longjmp((void *)jmpbuf, address);
}
...
primStrlenFromStartIndex(void) {
	...
	ptr = ((char *) ((startOfDatawithSize(rcvr, limit)) + byteOffset));
	if (limit == 0) {
		index = strlen(ptr);
	}
	else {
...
startOfData(sqInt rcvr) {
	if ((longAt(rcvr + BaseHeaderSize)) > 0) {
		return (rcvr + BaseHeaderSize) + BytesPerOop;
	}
	else {
		return longAt((rcvr + BaseHeaderSize) + BytesPerOop);
	}
}
