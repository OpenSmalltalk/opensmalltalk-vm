/* Automatically generated by
	VMPluginCodeGenerator VMMaker.oscog-eem.2857 uuid: 4f72a34b-cd5d-4e2c-9eae-45498349bad6
   from
	IA32ABIPlugin VMMaker.oscog-eem.2857 uuid: 4f72a34b-cd5d-4e2c-9eae-45498349bad6
 */
static char __buildInfo[] = "IA32ABIPlugin VMMaker.oscog-eem.2857 uuid: 4f72a34b-cd5d-4e2c-9eae-45498349bad6 " __DATE__ ;
...
static const char *moduleName = "IA32ABI VMMaker.oscog-eem.2857 " INT_EXT;
...
primAlienCopyInto(void)
{
	...
	myLength = (BytesPerWord == 8
		? ((sqLong) (longAt(alien + BaseHeaderSize)))
		: ((int) (longAt(alien + BaseHeaderSize))));
	src = (((myLength > 0
	? (alien + BaseHeaderSize) + BytesPerOop
	: longAt((alien + BaseHeaderSize) + BytesPerOop))) + start) - 1;
	if (isKindOfClass(dest, classAlien())) {
		totalLength = (BytesPerWord == 8
			? ((sqLong) (longAt(dest + BaseHeaderSize)))
			: ((int) (longAt(dest + BaseHeaderSize))));
...
primAlienReplace(void)
{
	...
	if (isKindOfClass(array, classAlien())) {
		totalLength = (BytesPerWord == 8
			? ((sqLong) (longAt(array + BaseHeaderSize)))
			: ((int) (longAt(array + BaseHeaderSize))));
		dest = (((totalLength > 0
	...
	if (isKindOfInteger(repl)) {
		...
		if (isKindOfClass(repl, classAlien())) {
			totalLength = (BytesPerWord == 8
				? ((sqLong) (longAt(repl + BaseHeaderSize)))
				: ((int) (longAt(repl + BaseHeaderSize))));
			src = (((totalLength > 0
...
primDoubleAt(void)
{
	...
	if (((startAddr = (((BytesPerWord == 8
		? ((sqLong) (longAt(rcvr + BaseHeaderSize)))
		: ((int) (longAt(rcvr + BaseHeaderSize))))) > 0
...
void* IA32ABI_exports[][3] = {
	...
	{(void*)_m, "primMostRecentCallbackContext\000\377", (void*)primMostRecentCallbackContext},
	{(void*)_m, "primOopAt\000\000", (void*)primOopAt},
	{(void*)_m, "primOopAtPut\000\000", (void*)primOopAtPut},
...
signed char primOopAtAccessorDepth = 0;
signed char primOopAtPutAccessorDepth = 0;
