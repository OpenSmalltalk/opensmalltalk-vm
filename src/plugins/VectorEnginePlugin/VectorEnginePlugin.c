/* Automatically generated by
	SmartSyntaxPluginCodeGenerator VMMaker.oscog-eem.3006 uuid: 52ac109b-6c67-49ce-a59a-c0e0dbb20e59
   from
	VectorEnginePlugin VectorEnginePlugin-dtl.10 uuid: 22240387-b64b-4850-88c7-dc1510e70abe
 */
static char __buildInfo[] = "VectorEnginePlugin VectorEnginePlugin-dtl.10 uuid: 22240387-b64b-4850-88c7-dc1510e70abe " __DATE__ ;
...
static const char *moduleName = "VectorEnginePlugin VectorEnginePlugin-dtl.10 " INT_EXT;
...
/*	Blends fill color over background. Target translucency computed
	correctly.
...
/*	Blends fill color over background.
	...
	OT:
	Target ignored on input and set to opaque on output.
	For blending, alphas
	are in [0 .. 1.0] and R, G, B color components are in [0 .. 255]
 */
...
/*	Blends stroke color and fill color over background.
	...
	Target translucency
	computed correctly.
	For blending, alphas are in [0 .. 1.0] and R, G, B
	color components are in [0 .. 255]
 */
...
/*	Inside the shape: Do an appropriate (anti aliased) gradient between stoke
	...
	background.
	WP: Whole pixel anti aliasing.
	OT: Target ignored on input
	and set to opaque on output.
	For blending, alphas are in [0 .. 1.0] and
...
/*	
	...
	Semantic version numbering in
	a single SmallInteger:
	Version numbers 1 to 99 are backwards compatible.
	When incompatibility is needed. jump straight to 101.
	Version numbers 101
	to 199 are backwards compatible. When incompatibility is needed. jump
	straight to 201.
	And so.
	Note: this is Api version, not package version.
	If no Api change, Api version doesn't change, regardless of changes in the
...
pluginApiVersion(void)
{
	...
	_return_value = integerObjectOf(5);
	if (!(failed())) {
...
/*	slight optimization possible when we know width = 0:
		- we know
	candidateAlpha is never 127
		- we know distanceToEdge is always >
	erodedHalfWidth. 
 */
...
/*	The Contour of the current morph is the pixel area affected, but without
	holes.
	For each y line where a pixel is affected, record the leftmost and
	rightmost affected pixels.
	Note: Only includes pen trajectory, but not
	stroke width.
 */
...
void* VectorEnginePlugin_exports[][3] = {
	...
	{(void*)_m, "primResetContour", (void*)primResetContour},
	...
	{(void*)_m, "primUpdateContourLastLine", (void*)primUpdateContourLastLine},
