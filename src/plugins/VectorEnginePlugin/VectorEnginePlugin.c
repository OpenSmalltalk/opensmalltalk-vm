/* Automatically generated by
	...
	(Compiler-eem.513)
   from
	VectorEnginePlugin VectorEnginePlugin-eem.20 uuid: 4769e745-c41c-4364-b448-0278664b7681
 */
static char __buildInfo[] = "VectorEnginePlugin VectorEnginePlugin-eem.20 uuid: 4769e745-c41c-4364-b448-0278664b7681 " __DATE__ ;
...
static const char *moduleName = "VectorEnginePlugin VectorEnginePlugin-eem.20 " INT_EXT;
	...
	for (displayY = t; displayY <= b; displayY += 1) {
		...
		for (displayX = l; displayX <= r; displayX += 1) {
			...
			if ((antiAliasAlphasWord != 0)
			 ...
			 || (isGreenInside
			 || (isBlueInside)))) {
				/* begin blendFillOnlyAt:redIsInside:greenIsInside:blueIsInside:antiAliasAlphasWord: */
				...
				if (currentClipsSubmorphs) {
					morphIdWord = morphIds[pixelIndex];
					clippingAntiAliasBits = morphIdWord & 0x7F;
					antiAliasGreenAlphaBitsShifted = (((usqInt)(antiAliasGreenAlphaBits)) >> 8);
...
primBlendFillOnlyWPOT(void)
{
	...
	if (fillA == 1.0) {
		resultRBits = (((sqInt)((usqInt)((((sqInt)(fillR + 0.5)))) << 16)));
		resultGBits = (((sqInt)((usqInt)((((sqInt)(fillG + 0.5)))) << 8)));
		resultBBits = ((sqInt)(fillB + 0.5));
...
primBlendStrokeAndFillWPOT(void)
{
	...
	float antiAliasAlpha;
	uint8_t antiAliasAlphaBits;
	float antiAliasAlphaFloat;
	float antiAliasUnAlpha;
	unsigned *aWordArray;
	sqInt b;
	float clippingAntiAlias;
	uint32_t clippingAntiAliasBits;
	uint32_t clippingAntiAliasBits1;
	float clippingAntiAliasFloat;
	sqInt displayX;
	sqInt displayY;
	uint8_t edgesThisPixel;
	uint8_t edgesUpToThisPixel;
	...
	float resultB;
	uint32_t resultBBits;
	uint32_t resultBBits1;
	float resultBFloat;
	float resultG;
	uint32_t resultGBits;
	uint32_t resultGBits1;
	float resultGFloat;
	float resultR;
	uint32_t resultRBits;
	uint32_t resultRBits1;
	float resultRFloat;
	sqInt t;
	uint32_t targetWord;
	uint32_t targetWord1;
	float unAlpha;
	float unAlphaFloat;
			...
			if (edgesUpToThisPixel) {
				if ((optimizedOpaqueTargetWord != 0)
				 ...
				 || (((morphIds[pixelIndex]) & 0x7F) == 0x7F)))) {
					...
					/* begin blendStrokeAndFillInsideWPOTAt:antiAliasAlphaByte: */
					antiAliasAlphaFloat = antiAliasAlphaBits * 0.007874;
					antiAliasUnAlpha = 1.0 - antiAliasAlphaFloat;

					/* We are inside the shape */
					alphaFloat = (antiAliasAlphaFloat * strokeA) + (antiAliasUnAlpha * fillA);
					foreRFloat = (antiAliasAlphaFloat * strokeR) + (antiAliasUnAlpha * fillR);
					foreGFloat = (antiAliasAlphaFloat * strokeG) + (antiAliasUnAlpha * fillG);
					foreBFloat = (antiAliasAlphaFloat * strokeB) + (antiAliasUnAlpha * fillB);
					if (currentClipsSubmorphs) {
						clippingAntiAliasBits1 = 0x7F;
					}
					else {
						if (clipCurrentMorph) {
							morphIdWord1 = morphIds[pixelIndex];
							clippingAntiAliasBits1 = morphIdWord1 & 0x7F;

							/* 1.0/127.0 */
							clippingAntiAliasFloat = clippingAntiAliasBits1 * 0.007874;
							alphaFloat = alphaFloat * clippingAntiAliasFloat;
						}
						else {
							clippingAntiAliasBits1 = 0;
						}
					}
					...
					/* Don't clip us, but do clip submorphs
					   We are inside the shape */
					if (!(alphaFloat == 0.0)) {
						targetWord1 = targetBits[pixelIndex];
						resultRBits1 = targetWord1 & 0xFF0000;
						resultGBits1 = targetWord1 & 0xFF00;
						resultBBits1 = targetWord1 & 0xFF;
						unAlphaFloat = 1.0 - alphaFloat;
						resultRFloat = (alphaFloat * foreRFloat) + (unAlphaFloat * ((((usqInt)(resultRBits1)) >> 16)));
						resultRBits1 = (((sqInt)((usqInt)((((sqInt)(resultRFloat + 0.5)))) << 16)));
						resultGFloat = (alphaFloat * foreGFloat) + (unAlphaFloat * ((((usqInt)(resultGBits1)) >> 8)));
						resultGBits1 = (((sqInt)((usqInt)((((sqInt)(resultGFloat + 0.5)))) << 8)));
						resultBFloat = (alphaFloat * foreBFloat) + (unAlphaFloat * resultBBits1);
						resultBBits1 = ((sqInt)(resultBFloat + 0.5));
						targetWord1 = ((0xFF000000U | resultRBits1) | resultGBits1) | resultBBits1;
						targetBits[pixelIndex] = targetWord1;
						morphIdWord1 = ((((usqInt)(currentMorphId) << 8))) + clippingAntiAliasBits1;
						morphIds[pixelIndex] = morphIdWord1;
					}
				}
			}
			else {
				if (antiAliasAlphaBits) {
					alphaMaskWP[pixelIndex] = 0;
					if (antiAliasAlphaBits) {
						/* begin blendStrokeAndFillOutsideWPOTAt:antiAliasAlphaByte: */
						antiAliasAlpha = antiAliasAlphaBits * 0.007874;
						...
						if (!(alpha == 0.0)) {
							targetWord = targetBits[pixelIndex];
							resultRBits1 = targetWord & 0xFF0000;
							resultGBits1 = targetWord & 0xFF00;
							resultBBits1 = targetWord & 0xFF;
							unAlpha = 1.0 - alpha;
							resultR = (alpha * foreR) + (unAlpha * ((((usqInt)(resultRBits1)) >> 16)));
							resultRBits1 = (((sqInt)((usqInt)((((sqInt)(resultR + 0.5)))) << 16)));
							resultG = (alpha * foreG) + (unAlpha * ((((usqInt)(resultGBits1)) >> 8)));
							resultGBits1 = (((sqInt)((usqInt)((((sqInt)(resultG + 0.5)))) << 8)));
							resultB = (alpha * foreB) + (unAlpha * resultBBits1);
							resultBBits1 = ((sqInt)(resultB + 0.5));
							targetWord = ((0xFF000000U | resultRBits1) | resultGBits1) | resultBBits1;
							targetBits[pixelIndex] = targetWord;
							morphIdWord = ((((usqInt)(currentMorphId) << 8))) + clippingAntiAliasBits;
							morphIds[pixelIndex] = morphIdWord;
						}
	...
	for (displayY = t; displayY <= b; displayY += 1) {
		...
		for (displayX = l; displayX <= r; displayX += 1) {
			...
			if (antiAliasAlphasWord) {
				...
				/* begin blendStrokeOnlyAt:antiAliasAlphasWord: */
				antiAliasRedAlphaBits = antiAliasAlphasWord & 0x7F0000;
				antiAliasGreenAlphaBits = antiAliasAlphasWord & 0x7F00;
				antiAliasBlueAlphaBits = antiAliasAlphasWord & 0x7F;
				antiAliasRedAlpha = antiAliasRedAlphaBits * (1.0 / (8.323072e6));
			...
			if ((optimizedOpaqueTargetWord != 0)
			 ...
			 || (((morphIds[pixelIndex]) & 0x7F) == 0x7F)))) {
				...
				if (antiAliasAlphaBits) {
					...
					/* begin blendStrokeOnlyWPOTAt:antiAliasAlphaByte: */
					antiAliasAlpha = antiAliasAlphaBits * 0.007874;
					alpha = antiAliasAlpha * strokeA;
					if (currentClipsSubmorphs) {
						morphIdWord = morphIds[pixelIndex];
					...
					if (!(alpha == 0.0)) {
						...
						resultBBits1 = targetWord & 0xFF;
						unAlpha = 1.0 - alpha;
						resultR = (alpha * strokeR) + (unAlpha * ((((usqInt)(resultRBits1)) >> 16)));
						resultRBits1 = (((sqInt)((usqInt)((((sqInt)(resultR + 0.5)))) << 16)));
						resultG = (alpha * strokeG) + (unAlpha * ((((usqInt)(resultGBits1)) >> 8)));
						resultGBits1 = (((sqInt)((usqInt)((((sqInt)(resultG + 0.5)))) << 8)));
						resultB = (alpha * strokeB) + (unAlpha * resultBBits1);
						resultBBits1 = ((sqInt)(resultB + 0.5));
						targetWord = ((0xFF000000U | resultRBits1) | resultGBits1) | resultBBits1;
						targetBits[pixelIndex] = targetWord;
						morphIdWord = ((((usqInt)(currentMorphId) << 8))) + clippingAntiAliasBits;
...
primDisplayString(void)
{
	sqInt aBoolean;
	float advanceWidth;
	unsigned *alphaWordArray;
	char *aString;
	sqInt charIndex;
	sqInt charIndexSqInt;
	float *contourData;
	...
	edgeWordArray = firstIndexableField(stackValue(1));
	alphaWordArray = firstIndexableField(stackValue(0));

	/* begin displayStringLoop:displayIf:wholePixel:contourIndexAccessor:from:to:atx:y:scalex:y:contourData:edgeCounts:alphaMask: */
	edgeCounts = edgeWordArray;
	alphaMask = alphaWordArray;
	txA11 = txA11 * sx;
	txA12 = txA12 * sy;
	txA21 = txA21 * sx;
	txA22 = txA22 * sy;
	nextGlyphX = destX / sx;
	nextGlyphY = destY / sy;
	for (charIndexSqInt = (startIndex - 1); charIndexSqInt < stopIndex; charIndexSqInt += 1) {
		...
		for (idx = 1; idx <= numContours; idx += 1) {
			...
			ttX = (contourData[i + 1]) + nextGlyphX;
			ttY = (contourData[i + 2]) + nextGlyphY;
			i += 3;
...
primDisplayStringWP(void)
{
	...
	/* begin displayStringLoop:displayIf:wholePixel:contourIndexAccessor:from:to:atx:y:scalex:y:contourData:edgeCounts:alphaMask: */
	edgeCountsWP = edgeWordArray;
	alphaMaskWP = alphaWordArray;
	txA11 = txA11 * sx;
	txA12 = txA12 * sy;
	txA21 = txA21 * sx;
	txA22 = txA22 * sy;
...
primDisplayUtf32WP(void)
{
	...
	contourDataIndexes = firstIndexableField(stackValue(3));
	aBoolean = booleanValueOf(stackValue(2));
	edgeByteArray = firstIndexableField(stackValue(1));
	alphaByteArray = firstIndexableField(stackValue(0));

	/* begin displayStringLoop:displayIf:wholePixel:contourIndexAccessor:from:to:atx:y:scalex:y:contourData:edgeCounts:alphaMask: */
	edgeCountsWP = edgeByteArray;
	alphaMaskWP = alphaByteArray;
	txA11 = txA11 * sx;
	txA12 = txA12 * sy;
	txA21 = txA21 * sx;
	txA22 = txA22 * sy;
...
primDisplayUtf8(void)
{
	...
	nextGlyphX = destX / sx;
	nextGlyphY = destY / sy;
	for (charIndex = (byteStartIndex - 1); charIndex < byteStopIndex; charIndex += 1) {
		byte = aByteArray[charIndex];
...
pvt_cubicBezierFromXytoXycontrol1Xycontrol2Xy(float xFrom, float yFrom, float xTo, float yTo, float xControl1, float yControl1, float xControl2, float yControl2)
{
	...
	dx = fabsf(txControl1 - txFrom);
	dx2 = fabsf(txTo - txControl2);
	dx3 = fabsf(txControl2 - txControl1);
	dy = fabsf(tyControl1 - tyFrom);
	dy2 = fabsf(tyTo - tyControl2);
	dy3 = fabsf(tyControl2 - tyControl1);
