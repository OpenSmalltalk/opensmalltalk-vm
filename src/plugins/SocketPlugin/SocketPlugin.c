/* Smalltalk from Squeak4.5 with VMMaker 4.13.11 translated as C source on 25 March 2015 9:00:21 pm */
/* Automatically generated by
	SmartSyntaxPluginCodeGenerator VMMaker-dtl.358 uuid: 6d8d1641-9d58-41f7-99f5-22478bc10ae2
   from
	SocketPlugin VMMaker-dtl.358 uuid: 6d8d1641-9d58-41f7-99f5-22478bc10ae2
 */
static char __buildInfo[] = "SocketPlugin VMMaker-dtl.358 uuid: 6d8d1641-9d58-41f7-99f5-22478bc10ae2 " __DATE__ ;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"SocketPlugin 25 March 2015 (i)"
#else
	"SocketPlugin 25 March 2015 (e)"
#endif
;
...
EXPORT(sqInt) primitiveHasSocketAccess(void) {
	...
	hasAccess = (sHSAfn == 0) || ( ((int (*) (void)) sHSAfn)());
...
EXPORT(sqInt) primitiveSocketBindTo(void) {
	...
	/* begin socketValueOf: */
	interpreterProxy->success((interpreterProxy->isBytes(socket)) && ((interpreterProxy->byteSizeOf(socket)) == (sizeof(SQSocket))));
...
EXPORT(sqInt) primitiveSocketGetOptions(void) {
	...
	results = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classArray(), 2);
	interpreterProxy->storePointerofObjectwithValue(0, results, interpreterProxy->integerObjectOf(errorCode));
	interpreterProxy->storePointerofObjectwithValue(1, results, interpreterProxy->integerObjectOf(returnedValue));
	if (interpreterProxy->failed()) {
		return null;
	}
...
EXPORT(sqInt) primitiveSocketReceiveUDPDataBufCount(void) {
	...
	if (!(interpreterProxy->failed())) {
		...
		results = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classArray(), 4);
		interpreterProxy->storePointerofObjectwithValue(0, results, interpreterProxy->integerObjectOf((bytesReceived / elementSize)));
		interpreterProxy->storePointerofObjectwithValue(1, results, interpreterProxy->popRemappableOop());
		interpreterProxy->storePointerofObjectwithValue(2, results, interpreterProxy->integerObjectOf(port));
		interpreterProxy->storePointerofObjectwithValue(3, results, (moreFlag
...
EXPORT(sqInt) primitiveSocketAccept(void) {
	...
	/* begin socketValueOf: */
	interpreterProxy->success((interpreterProxy->isBytes(sockHandle)) && ((interpreterProxy->byteSizeOf(sockHandle)) == (sizeof(SQSocket))));
	serverSocket = (interpreterProxy->failed()
		? null
		: ((SocketPtr) (interpreterProxy->firstIndexableField(sockHandle))));
	if (!(interpreterProxy->failed())) {
		socketOop = interpreterProxy->instantiateClassindexableSize(interpreterProxy->classByteArray(), sizeof(SQSocket));
		/* begin socketValueOf: */
		interpreterProxy->success((interpreterProxy->isBytes(socketOop)) && ((interpreterProxy->byteSizeOf(socketOop)) == (sizeof(SQSocket))));
		s = (interpreterProxy->failed()
			? null
