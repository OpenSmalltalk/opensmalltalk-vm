/* Automatically generated by
   ...
   from
	SocketPlugin VMMaker.oscog-eem.3417 uuid: 6b0ab3f7-46c0-4e4c-a800-57a4d13ffef0
 */
static char __buildInfo[] = "SocketPlugin VMMaker.oscog-eem.3417 uuid: 6b0ab3f7-46c0-4e4c-a800-57a4d13ffef0 " __DATE__ ;
...
static const char *moduleName = "SocketPlugin VMMaker.oscog-eem.3417 " INT_EXT;
...
primitiveSocketAccept(void)
{
	...
	if ((isBytes(sockHandle))
	 && ((byteSizeOf(sockHandle)) == (sizeof(SQSocket)))) {
		serverSocket = firstIndexableField(sockHandle);
	}
	else {
		primitiveFailFor(PrimErrBadArgument);
		serverSocket = null;
	...
	if (!(failed())) {
		...
		if ((isBytes(socketOop))
		 && ((byteSizeOf(socketOop)) == (sizeof(SQSocket)))) {
			s = firstIndexableField(socketOop);
		}
		else {
			primitiveFailFor(PrimErrBadArgument);
			s = null;
