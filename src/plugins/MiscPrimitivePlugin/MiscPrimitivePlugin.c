/* Automatically generated by
	VMPluginCodeGenerator VMMaker.oscog-eem.2359 uuid: b3273e3e-dd9d-4819-a928-7034e1cf412c
   from
	MiscPrimitivePlugin VMMaker.oscog-eem.2359 uuid: b3273e3e-dd9d-4819-a928-7034e1cf412c
 */
static char __buildInfo[] = "MiscPrimitivePlugin VMMaker.oscog-eem.2359 uuid: b3273e3e-dd9d-4819-a928-7034e1cf412c " __DATE__ ;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"MiscPrimitivePlugin VMMaker.oscog-eem.2359 (i)"
#else
	"MiscPrimitivePlugin VMMaker.oscog-eem.2359 (e)"
#endif
;
...
primitiveCompareString(void)
{
	...
	for (i = 0; i < (((len1 < len2) ? len1 : len2)); i += 1) {
		...
		c2 = order[string2[i]];
		if (!(c1 == c2)) {
			return methodReturnValue(integerObjectOf((c1 < c2
				? 1
				: 3)));
		}
	}
	methodReturnValue(integerObjectOf((len1 == len2
		? 2
		: (len1 < len2
				? 1
				: 3))));
...
primitiveCompressToByteArray(void)
{
	...
	methodReturnValue(integerObjectOf(i));
	return 0;
}
...
primitiveFindFirstInString(void)
{
	...
	if ((sizeOfSTArrayFromCPrimitive(inclusionMap)) != 256) {
		return methodReturnValue(integerObjectOf(0));
	}
	i = start - 1;
	...
	methodReturnValue(integerObjectOf((i >= stringSize
		? 0
		: i + 1)));
	return 0;
}
	...
	if (((keySize = sizeOfSTArrayFromCPrimitive(key))) > 0) {
		...
		for (startIndex = (((start < 1) ? 1 : start)), startIndexLimiT = ((sizeOfSTArrayFromCPrimitive(body)) - keySize); startIndex <= startIndexLimiT; startIndex += 1) {
			...
			while ((matchTable[body[(startIndex + index) - 1]]) == (matchTable[key[index]])) {
				if (index == keySize) {
					return methodReturnValue(integerObjectOf(startIndex));
				}
				index += 1;
			}
		}
...
primitiveIndexOfAsciiInString(void)
{
	...
	for (pos = (start - 1); pos < stringSize; pos += 1) {
		if ((aString[pos]) == anInteger) {
			return methodReturnValue(integerObjectOf(pos + 1));
		}
	}
	return methodReturnValue(integerObjectOf(0));
}
...
primitiveStringHash(void)
{
	...
	methodReturnValue(integerObjectOf(hash));
	return 0;
}
