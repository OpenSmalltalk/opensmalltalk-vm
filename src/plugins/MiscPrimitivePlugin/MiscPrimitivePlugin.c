/* Automatically generated by
   ...
   from
	MiscPrimitivePlugin VMMaker.oscog-eem.3453 uuid: 6917573f-1029-4681-87bc-5f02f287102b
 */
static char __buildInfo[] = "MiscPrimitivePlugin VMMaker.oscog-eem.3453 uuid: 6917573f-1029-4681-87bc-5f02f287102b " __DATE__ ;
...
static const char *moduleName = "MiscPrimitivePlugin VMMaker.oscog-eem.3453 " INT_EXT;
...
primitiveCompareString(void)
{
	...
	order = ((unsigned char *) (firstIndexableField(orderOop)));
	if ((sizeOfSTArrayFromCPrimitive(order)) < 0x100) {
		return primitiveFailFor(PrimErrBadArgument);
	}
	string1 = ((unsigned char *) (firstIndexableField(string1Oop)));
	string2 = ((unsigned char *) (firstIndexableField(string2Oop)));
...
primitiveCompressToByteArray(void)
{
	...
	ba = ((unsigned char *) (firstIndexableField(baOop)));
	...
	for (j1 = 0; j1 <= 3; j1 += 1) {
		ba[(1) + j1] = ((((usqInt)(size)) >> ((3 - j1) * 8)) & 0xFF);
	}
	i = 5;
	...
	while (k < size) {
		...
		if (j > k) {
			if (eqBytes) {
				...
				/* begin encodeBytesOf:in:at: */
				for (j2 = 0; j2 <= 3; j2 += 1) {
					ba[(i + 1) + j2] = ((((usqInt)(((((j - k) + 1) * 4) + 1))) >> ((3 - j2) * 8)) & 0xFF);
				}
				i = (i + 1) + 4;
	l2:	/* end encodeInt:in:at: */;
				...
				/* begin encodeBytesOf:in:at: */
				for (j3 = 0; j3 <= 3; j3 += 1) {
					ba[(i + 1) + j3] = ((((usqInt)(((((j - k) + 1) * 4) + 2))) >> ((3 - j3) * 8)) & 0xFF);
				}
				i = (i + 1) + 4;
	l3:	/* end encodeInt:in:at: */;
				/* begin encodeBytesOf:in:at: */
				for (j4 = 0; j4 <= 3; j4 += 1) {
					ba[i + j4] = ((((usqInt)(word)) >> ((3 - j4) * 8)) & 0xFF);
				}
				i += 4;
			}
			k = j + 1;
			...
			if (eqBytes) {
				...
				/* begin encodeBytesOf:in:at: */
				for (j5 = 0; j5 <= 3; j5 += 1) {
					ba[(i + 1) + j5] = ((((usqInt)((5))) >> ((3 - j5) * 8)) & 0xFF);
				}
				i = (i + 1) + 4;
	...
	l6:	/* end encodeInt:in:at: */;
				for (m = k; m < j; m += 1) {
					/* begin encodeBytesOf:in:at: */
					for (j7 = 0; j7 <= 3; j7 += 1) {
						ba[i + j7] = ((((usqInt)((bm[m]))) >> ((3 - j7) * 8)) & 0xFF);
					}
					i += 4;
				}
				k = j;
...
primitiveConvert8BitSigned(void)
{
	...
	aByteArray = ((unsigned char *) (firstIndexableField(byteArrayOop)));
...
primitiveFindFirstInString(void)
{
	...
	inclusionMap = ((unsigned char *) (firstIndexableField(inclusionMapOop)));
	if ((sizeOfSTArrayFromCPrimitive(inclusionMap)) != 0x100) {
		return methodReturnInteger(0);
	}
	aString = ((unsigned char *) (firstIndexableField(aStringOop)));
...
primitiveFindSubstring(void)
{
	...
	matchTable = ((unsigned char *) (firstIndexableField(matchTableOop)));
	if ((sizeOfSTArrayFromCPrimitive(matchTable)) < 0x100) {
		return primitiveFailFor(PrimErrBadArgument);
	}
	key = ((unsigned char *) (firstIndexableField(keyOop)));
	if (((keySize = sizeOfSTArrayFromCPrimitive(key))) > 0) {
		/* adjust for zero relative indexes */
		keySize -= 1;
		/* adjust for zero relative indexes */
		start = (((start - 1) < 0) ? 0 : (start - 1));
		body = ((unsigned char *) (firstIndexableField(bodyOop)));
...
primitiveIndexOfAsciiInString(void)
{
	...
	aString = ((unsigned char *) (firstIndexableField(stringOop)));
	stringSize = sizeOfSTArrayFromCPrimitive(aString);
...
primitiveTranslateStringWithTable(void)
{
	...
	table = ((unsigned char *) (firstIndexableField(tableOop)));
