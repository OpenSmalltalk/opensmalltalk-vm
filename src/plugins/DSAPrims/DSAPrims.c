/* Automatically generated by
   ...
   from
	DSAPlugin CryptographyPlugins-eem.30 uuid: 53c9140e-9395-49f1-acb1-539c6fb29813
 */
static char __buildInfo[] = "DSAPlugin CryptographyPlugins-eem.30 uuid: 53c9140e-9395-49f1-acb1-539c6fb29813 " __DATE__ ;
	...
	l1:	/* end subtractDivisorMultipliedByDigit:digitShift: */;
			if (qTooBig) {
				...
				for (i1 = 1; i1 <= divisorDigitCount; i1 += 1) {
					sum = ((dsaRemainder[rIndex1]) + (dsaDivisor[i1])) + carry;
					dsaRemainder[rIndex1] = (sum & 0xFF);
					carry = (((usqInt)(sum)) >> 8);
					rIndex1 += 1;
				}
				sum = (dsaRemainder[rIndex1]) + carry;
				dsaRemainder[rIndex1] = (sum & 0xFF);
				q -= 1;
			}
		}
...
primitiveBigMultiply(void)
{
	...
	prodPtr = ((unsigned char *) (firstIndexableField(prod)));
	f2Ptr = ((unsigned char *) (firstIndexableField(f2)));
	f1Ptr = ((unsigned char *) (firstIndexableField(f1)));
	for (i = 0; i < f1Len; i += 1) {
		if (((digit = f1Ptr[i])) != 0) {
			carry = 0;
...
primitiveExpandBlock(void)
{
	...
	wordPtr = ((unsigned int *) (firstIndexableField(expanded)));
	bytePtr = ((unsigned char *) (firstIndexableField(buf)));
	src = 0;
	for (i = 0; i <= 15; i += 1) {
...
primitiveHashBlock(void)
{
	...
	statePtr = ((unsigned int *) (firstIndexableField(state)));
	bufPtr = ((unsigned int *) (firstIndexableField(buf)));
	a = statePtr[0];
	b = statePtr[1];
	c = statePtr[2];
	d = statePtr[3];
...
primitiveHighestNonZeroDigitIndex(void)
{
	...
	bigIntPtr = ((unsigned char *) (firstIndexableField(arg)));
	i = stSizeOf(arg);
	while ((i > 0)
	 && ((bigIntPtr[(i -= 1)]) == 0)) {
	}
...
void* DSAPrims_exports[][3] = {
	...
	{(void*)_m, "primitiveBigMultiply\000\001\001", (void*)primitiveBigMultiply},
	{(void*)_m, "primitiveExpandBlock\000\001\001", (void*)primitiveExpandBlock},
	{(void*)_m, "primitiveHashBlock\000\001\001", (void*)primitiveHashBlock},
	{(void*)_m, "primitiveHasSecureHashPrimitive\000\377\001", (void*)primitiveHasSecureHashPrimitive},
	{(void*)_m, "primitiveHighestNonZeroDigitIndex\000\001\001", (void*)primitiveHighestNonZeroDigitIndex},
