...
dupToStdErr(sqInt anSQFileDataStructure)
{
 ...
 && ((interpreterProxy->byteSizeOf(anSQFileDataStructure)) == (sizeof(SQFile))))
...
fileDescriptorFrom(sqInt aSQFileByteArray)
{
 ...
 && ((interpreterProxy->byteSizeOf(aSQFileByteArray)) == (sizeof(SQFile))))
	...
	if ((sandboxSecurity()) == 1) {
		...
		if (((pid = vfork())) == 0) {
			...
			if (envVecBuffer == (interpreterProxy->nilObject())) {
				...
				idx1 = 0;
				while (idx1 < envCount) {
					ptr[idx1] = (envPtr + (interpreterProxy->integerValueOf(envOffsetPtr[idx1])));
					idx1 += 1;
				}
				env = ((char **) envPtr);
			}
			...
			idx2 = 0;
			while (idx2 < argCount) {
				ptr1[idx2] = (argsPtr + (interpreterProxy->integerValueOf(argOffsetPtr[idx2])));
				idx2 += 1;
			}
...
getStdHandle(sqInt n)
{
	...
	memcpy(interpreterProxy->firstIndexableField(fileOop), &fileRecords[n], sizeof(SQFile));
...
primitiveArgumentAt(void)
{
	...
	if ((index > argCnt) || (index < 1)) {
		interpreterProxy->popthenPush(2, interpreterProxy->nilObject());
	}
	else {
		sPtr = argVec[index - 1];
		...
		s = newString;
		interpreterProxy->popthenPush(2, s);
	}
}
...
primitiveCanReceiveSignals(sqInt anIntegerPid)
{
	...
	if (interpreterProxy->isIntegerObject(interpreterProxy->stackValue(0))) {
...
primitiveFileProtectionMask(void)
{
	...
	if (mode == 0) {
		...
		interpreterProxy->stObjectatput(result, 4, interpreterProxy->integerObjectOf(mode & 07));
		interpreterProxy->stObjectatput(result, 3, interpreterProxy->integerObjectOf((mode & 070) >> 3));
		interpreterProxy->stObjectatput(result, 2, interpreterProxy->integerObjectOf((mode & 0700) >> 6));
		interpreterProxy->stObjectatput(result, 1, interpreterProxy->integerObjectOf((mode & 07000) >> 9));
		interpreterProxy->pop(2);
...
primitiveFileStat(void)
{
	...
	if (mode == 0) {
		mode = statBuf->st_mode;
		interpreterProxy->stObjectatput(mask, 4, interpreterProxy->integerObjectOf(mode & 07));
		interpreterProxy->stObjectatput(mask, 3, interpreterProxy->integerObjectOf((mode & 070) >> 3));
		interpreterProxy->stObjectatput(mask, 2, interpreterProxy->integerObjectOf((mode & 0700) >> 6));
		interpreterProxy->stObjectatput(mask, 1, interpreterProxy->integerObjectOf((mode & 07000) >> 9));
		interpreterProxy->stObjectatput(result, 3, mask);
		interpreterProxy->stObjectatput(result, 2, interpreterProxy->integerObjectOf(statBuf->st_gid));
		interpreterProxy->stObjectatput(result, 1, interpreterProxy->integerObjectOf(statBuf->st_uid));
		interpreterProxy->pop(2);
		interpreterProxy->push(result);
	}
	else {
...
primitiveForwardSignalToSemaphore(void)
{
	...
	if (index == (interpreterProxy->nilObject())) {
		...
		if (interpreterProxy->isIntegerObject(index)) {
...
primitiveGetSession(void)
{
	...
	interpreterProxy->popthenPush(1, sessionOop);
}
...
primitiveKillOnExit(void)
{
	...
	while (count <= pidCount) {
		pid = interpreterProxy->integerValueOf(*pidPointer++);
		*p++ = pid;
		count += 1;
	}
...
primitiveTestLockableFileRegion(void)
{
 ...
 && ((interpreterProxy->getThisSessionID()) == (sessionIdentifierFromSqFile(interpreterProxy->arrayValueOf(sqFileOop)))))
	...
	if (result == -1) {
		...
		interpreterProxy->stObjectatput(resultArray, 2, interpreterProxy->integerObjectOf(lockStruct.l_pid));
		interpreterProxy->stObjectatput(resultArray, 3, interpreterProxy->integerObjectOf(lockStruct.l_type));
		interpreterProxy->stObjectatput(resultArray, 4, interpreterProxy->integerObjectOf(lockStruct.l_whence));
		interpreterProxy->stObjectatput(resultArray, 5, interpreterProxy->integerObjectOf(lockStruct.l_start));
		interpreterProxy->stObjectatput(resultArray, 6, interpreterProxy->integerObjectOf(lockStruct.l_len));
