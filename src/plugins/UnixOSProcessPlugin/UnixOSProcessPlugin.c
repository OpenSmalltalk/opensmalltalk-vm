 ...
 */
static char __buildInfo[] = "UnixOSProcessPlugin VMConstruction-Plugins-OSProcessPlugin.oscog-eem.82 uuid: 1510486b-819a-4643-921a-1e39553cc4a8 " __DATE__ ;
...
static const char *moduleName = "UnixOSProcessPlugin VMConstruction-Plugins-OSProcessPlugin.oscog-eem.82 " INT_EXT;
...
#    if defined(SA_DISABLE)
			SA_DISABLE
#    else
			/* e.g. Mac OS documents SA_DISABLE but defines SS_DISABLE */ SS_DISABLE
#    endif
			)) != 0)))) {
		return 1;
	}
	...
	ifNilSqInt = (sigstack.ss_sp = malloc((sigstack.ss_size = SigStackSize)));
	if (!ifNilSqInt) {
