 ...
 */
static char __buildInfo[] = "RomePlugin Rome-Plugin-dtl.40 uuid: fc8fcc8b-edca-42ec-90db-24190e3e33fd " __DATE__ ;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"RomePlugin dtl.40 27 March 2016 (i)"
#else
	"RomePlugin dtl.40 27 March 2016 (e)"
#endif
;
...
static sqInt addColorinto(sqInt attrArrayOop, PangoAttrList *pangoAttrList) {
	...
	alpha = ((usqInt) (c && 0xFF000000U)) >> 24;
	r = ((usqInt) (c & 0xFF0000)) >> 16;
	g = ((usqInt) (c & 0xFF00)) >> 8;
	b = c & 0xFF;
	if (!(r == 0)) {
		r = r * 257;
...
static void addColorStopTooffsetrgbalpha(cairo_pattern_t*pattern, sqInt intOffset, sqInt rgb, sqInt alpha) {
	...
	r = (((usqInt) rgb) >> 20) & 0x3FF;
	g = (((usqInt) rgb) >> 10) & 0x3FF;
	b = (((usqInt) rgb) >> 0) & 0x3FF;
	if (alpha == 0xFF) {
...
EXPORT(sqInt) initialiseModule(void) {
	...
	defaultFontDescription = (PangoFontDescription  *)pango_font_description_from_string("Times New Roman 10");
...
static sqInt leadingCharOf(unsigned int value) {
	return ((usqInt) (value & 0x3FC00000) >> 22);
}
...
EXPORT(sqInt) primitiveLanguageAttributes(void) {
	...
	ws = string[0];
	currentTag = ((usqInt) (ws & 0x3FC00000) >> 22);
	leadingChar = -1;
	currentStart = 0;
	/* begin utf8CountFor: */
	v = ws & 0x1FFFFF;
...
l1:	/* end utf8CountFor: */;
	...
	for (i = 1; i <= (stringSize - 1); i += 1) {
		ws = string[i];
		leadingChar = ((usqInt) (ws & 0x3FC00000) >> 22);
...
l1:	/* end utf8CountFor: */;
	...
	for (i = 2; i <= nBytes; i += 1) {
		shift += 6;
		utf8String[index] = (((((shift < 0) ? ((usqInt) val >> -shift) : ((usqInt) val << shift))) & 0x3F) + 128);
		index += 1;
	}
