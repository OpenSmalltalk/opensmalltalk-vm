...
static sqInt cbColorComponentFrom(sqInt oop) {
	return (colorComponentfrom((int  *)cbComponent, oop)) && (colorComponentBlocksfrom((int  **)cbBlocks, oop));
}
...
static sqInt crColorComponentFrom(sqInt oop) {
	return (colorComponentfrom((int  *)crComponent, oop)) && (colorComponentBlocksfrom((int  **)crBlocks, oop));
}
...
EXPORT(sqInt) primitiveColorConvertGrayscaleMCU(void) {
	...
	if (!((colorComponentfrom((int  *)yComponent, arrayOop)) && (colorComponentBlocksfrom((int  **)yBlocks, arrayOop)))) {
		return interpreterProxy->primitiveFail();
	}
	...
	for (i = 0; i <= (jpegBitsSize - 1); i += 1) {
		...
		y = ((y < 1) ? 1 : y);
		jpegBits[i] = (((0xFF000000U + (y << 16)) + (y << 8)) + y);
	}
...
EXPORT(sqInt) primitiveDecodeMCU(void) {
	...
	if (!(colorComponentfrom((int  *)yComponent, oop))) {
		return interpreterProxy->primitiveFail();
	}
	...
	decodeBlockIntocomponent(anArray, (int  *)yComponent);
	if (interpreterProxy->failed()) {
		return null;
	}
...
static sqInt yColorComponentFrom(sqInt oop) {
	return (colorComponentfrom((int  *)yComponent, oop)) && (colorComponentBlocksfrom((int  **)yBlocks, oop));
}
