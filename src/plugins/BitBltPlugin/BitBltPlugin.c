/* Automatically generated by
   ...
   from
	BitBltSimulation VMMaker.oscog-eem.3453 uuid: 6917573f-1029-4681-87bc-5f02f287102b
 */
static char __buildInfo[] = "BitBltSimulation VMMaker.oscog-eem.3453 uuid: 6917573f-1029-4681-87bc-5f02f287102b " __DATE__ ;
...
static const char *moduleName = "BitBltPlugin VMMaker.oscog-eem.3453 " INT_EXT;
		...
		while (((deltaX -= 1)) != 0) {
			...
			if (srcAlpha == 0xFF) {
				...
				if (!(srcAlpha == 0)) {
					...
					sourceWord = ((unsigned int) ((((((usqInt)((dither8Lookup[addThreshold1 + (((((usqInt)(sourceWord)) >> 16)) & 0xFF)])) << 10))) + ((((usqInt)((dither8Lookup[addThreshold1 + (((((usqInt)(sourceWord)) >> 8)) & 0xFF)])) << 5)))) + (dither8Lookup[addThreshold1 + (sourceWord & 0xFF)])));
					if (sourceWord == 0) {
						sourceWord = 1U << srcShift;
					}
					else {
	...
	if (numArgs >= 2) {
		...
		if (numArgs == 4) {
			...
			if (isBytes(gammaLookupTableOop)) {
				gammaLookupTable = ((unsigned char *) (firstIndexableField(gammaLookupTableOop)));
			}
			ungammaLookupTableOop = stackValue(0);
			if (isBytes(ungammaLookupTableOop)) {
				ungammaLookupTable = ((unsigned char *) (firstIndexableField(ungammaLookupTableOop)));
			}
		}
		return 1;
	}
 ...
 && (skew == 0))));
	...
	if (combinationRule == 3) {
		for (i = 1; i <= bbH; i += 1) {
			...
			if (nWords > 1) {
				destMask = mask2;
				thisWord = (((((skew < 0) ? (((usqInt)(skewMask)) >> (-skew)) : (((usqInt)(skewMask) << skew)))) & mask2) == 0
					? 0
					: (						/* begin srcLongAt: */
							assert((((usqInt)sourceIndex)) < endOfSource),
...
loadBitBltFromwarping(sqInt bbObj, sqInt aBool)
{
	...
	if (	/* begin noSourceCombinationRule */
		(combinationRule == 0)
	 || ((combinationRule == 5)
	 || ((combinationRule == 10)
	 || (combinationRule == 15)))) {
	...
	l4:	/* end loadBitBltSourceForm */;
		...
		if (isWords(cmOop)) {
			...
			cmLookupTable = ((unsigned int *) (firstIndexableField(cmOop)));
		}
		else {
			...
			cmShiftTable = ((int *) (firstIndexableField(mapOop)));
	l5:	/* end loadColorMapShiftOrMaskFrom:type: */;
			...
			cmMaskTable = ((unsigned int *) (firstIndexableField(mapOop1)));
	l6:	/* end loadColorMapShiftOrMaskFrom:type: */;
			...
			if (oop == (nilObject())) {
				...
				cmLookupTable = ((unsigned int *) (firstIndexableField(oop)));
			}
...
primitiveDisplayString(void)
{
	...
	left = destX;
	sourcePtr = ((char *) (firstIndexableField(sourceString)));
...
warpLoop(void)
{
	...
	l11:	/* end fetchIntOrFloat:ofObject: */;
	/* begin deltaFrom:to:nSteps: */
	if (words > pBy) {
			...
			if (smoothingCount == 1) {
				...
				if (mapperFlags == (ColorMapPresent | ColorMapIndexedPart)) {
					...
					do {
						...
						sourcePix = ((unsigned int) sourcePix1);
	l16:	/* end pickWarpPixelAtX:y: */;
					...
					do {
						...
						sourcePix = ((unsigned int) sourcePix2);
	l18:	/* end pickWarpPixelAtX:y: */;
						/* begin mapPixel:flags: */
						pv = sourcePix;
						if ((mapperFlags & ColorMapPresent) != 0) {
							if ((mapperFlags & ColorMapFixedPart) != 0) {
