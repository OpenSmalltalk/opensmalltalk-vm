/* Automatically generated by
   ...
   from
	BitBltSimulation VMMaker.oscog-eem.1833 uuid: 04364d67-49f1-4615-87e7-942001d37706
 */
static char __buildInfo[] = "BitBltSimulation VMMaker.oscog-eem.1833 uuid: 04364d67-49f1-4615-87e7-942001d37706 " __DATE__ ;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"BitBltPlugin VMMaker.oscog-eem.1833 (i)"
#else
	"BitBltPlugin VMMaker.oscog-eem.1833 (e)"
#endif
;
		...
		while (((deltaX -= 1)) != 0) {
			...
			if (srcAlpha == 0xFF) {
				...
				if (!(srcAlpha == 0)) {
					...
					/* Mix colors */
					destWord = ((((usqInt) (destWord & 0x7C00) << 9)) | (((usqInt) (destWord & 0x3E0) << 6))) | ((((usqInt) (destWord & 0x1F) << 3)) | 0xFF000000ULL);
		...
		while (((deltaX -= 1)) != 0) {
			...
			if (destMSB) {
				if (srcShift == 0) {
					...
					srcShift -= 8;
					dstMask = (((usqInt) dstMask) >> 8) | 0xFF000000ULL;
				}
			}
			else {
				if (srcShift == 24) {
		...
		do {
			...
			if (mapperFlags == (ColorMapPresent | ColorMapIndexedPart)) {
				...
				do {
					...
					if (!((((srcShift1 += srcShiftInc)) & 0xFFFFFFE0ULL) == 0)) {
						if (sourceMSB) {
							srcShift1 += 32;
						}
						else {
...
fixAlphawith(unsigned int sourceWord, unsigned int destinationWord)
{
	...
	if (!((destinationWord & 0xFF000000ULL) == 0)) {
		return destinationWord;
	}
	return destinationWord | (sourceWord & 0xFF000000ULL);
}
...
isIdentityMapwith(int *shifts, unsigned int *masks)
{
	...
	if (((shifts[RedIndex]) == 0)
	 ...
	 && ((masks[AlphaIndex]) == 0xFF000000ULL)))))))) {
		return 1;
	}
	return 0;
}
...
partitionedRgbComponentAlphadestnBitsnPartitions(unsigned int sourceWord, unsigned int destWord, sqInt nBits, sqInt nParts)
{
	...
	for (i = 1; i <= nParts; i += 1) {
		...
		if (!(nBits == 32)) {
			if (nBits == 16) {
				p1 = ((((p1 & 0x1F) << 3) | ((p1 & 0x3E0) << 6)) | ((p1 & 0x7C00) << 9)) | 0xFF000000ULL;
				p2 = ((((p2 & 0x1F) << 3) | ((p2 & 0x3E0) << 6)) | ((p2 & 0x7C00) << 9)) | 0xFF000000ULL;
			}
			else {
				p1 = (rgbMapfromto(p1, nBits, 32)) | 0xFF000000ULL;
				p2 = (rgbMapfromto(p2, nBits, 32)) | 0xFF000000ULL;
			}
		}
...
pickSourcePixelsflagssrcMaskdestMasksrcShiftIncdstShiftInc(sqInt nPixels, sqInt mapperFlags, sqInt srcMask, sqInt dstMask, sqInt srcShiftInc, sqInt dstShiftInc)
{
	...
	if (mapperFlags == (ColorMapPresent | ColorMapIndexedPart)) {
		...
		do {
			...
			if (!((((srcShift += srcShiftInc)) & 0xFFFFFFE0ULL) == 0)) {
				if (sourceMSB) {
					srcShift += 32;
				}
				else {
...
primitivePixelValueAt(void)
{
	...
	mask = ((usqInt) 0xFFFFFFFFULL) >> (32 - depth);
...
rgbAddwith(unsigned int sourceWord, unsigned int destinationWord)
{
	...
	if (destDepth < 16) {
		...
		carryOverflowMask = (0xFFFFFFFFULL / componentMask) << (destDepth - 1);
	...
	if (destDepth == 16) {
		...
		componentMask = 0xFF;
		carryOverflowMask = 2155905152ULL;
	...
	for (i = 1; i <= destPPW; i += 1) {
		...
		if (!(destDepth == 32)) {
			if (destDepth == 16) {
				...
				p1 = (rgbMapfromto(p1, destDepth, 32)) | 0xFF000000ULL;
				p2 = (rgbMapfromto(p2, destDepth, 32)) | 0xFF000000ULL;
			}
		}
		v = rgbComponentAlpha32with(p1, p2);
		if (!(destDepth == 32)) {
...
l14:	/* end deltaFrom:to:nSteps: */;
	...
	if ((methodArgumentCount()) == 2) {
		...
		if (sourceMapOop == (nilObject())) {
			...
			if ((slotSizeOf(sourceMapOop)) < (1L << sourceDepth)) {
