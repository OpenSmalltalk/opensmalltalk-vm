...
extern sqInt stackMutableObjectValue(sqInt offset);
#else
...
primitiveGetRendererColorMasks(void)
{
	...
	array = stackMutableObjectValue(0);
	handle = stackIntegerValue(1);
	if (!(failed())) {
		if ((isArray(array))
		 && (((slotSizeOf(array)) == 4)
		 && (b3dxGetRendererColorMasks(handle, masks)))) {
			/* positive32BitIntegerFor: can no longer fail or GC */
			for (i = 0; i <= 3; i += 1) {
				storePointerofObjectwithValue(i, array, positive32BitIntegerFor(masks[i]));
			}
			return pop(2);
		}
	}
...
#if VM_PROXY_MAJOR > 1 || (VM_PROXY_MAJOR == 1 && VM_PROXY_MINOR >= 17)
		stackMutableObjectValue = interpreterProxy->stackMutableObjectValue;
#else
#if !defined(stackMutableObjectValue)
		stackMutableObjectValue = 0;
#endif
#endif
...
stackPrimitiveVertexArrayofSize(sqInt index, sqInt nItems)
{
	...
	oopSize = 0;
	if ((oop = stackObjectValue(index))) {
		if ((isWords(oop))
		 && ((((oopSize = slotSizeOf(oop))) >= (nItems * 16))
		 && ((oopSize % 16) == 0))) {
			return firstIndexableField(oop);
		}
	}
