/* Automatically generated by
	...
	(Compiler-ct.535)
   from
	BalloonEnginePlugin VMMaker.oscog-eem.3784 uuid: ad2305b0-44cb-456b-8b07-7a1a9825bc2b
 */
static char __buildInfo[] = "BalloonEnginePlugin VMMaker.oscog-eem.3784 uuid: ad2305b0-44cb-456b-8b07-7a1a9825bc2b " __DATE__ ;
...
static const char *moduleName = "B2DPlugin VMMaker.oscog-eem.3784 " INT_EXT;
...
l1:;
	...
	if (x < (objBuffer[fill + GFOriginX])) {
		...
		if ((workBuffer[GWAALevel]) == 1) {
			...
			dsSqInt = (*((int *) deltaST));
			dtSqInt = (((int *) deltaST))[1];
			rampIndex = accurateLengthOfwith(dsSqInt / 0x10000, dtSqInt / 0x10000);
			rampValue = (((int *) ramp))[rampIndex];
			...
			while (xSqInt < x1SqInt) {
				/* Try to copy the current value more than just once */
				while ((xSqInt < x1SqInt)
				 && ((((dsSqInt / 0x10000) * (dsSqInt / 0x10000)) + ((dtSqInt / 0x10000) * (dtSqInt / 0x10000))) >= length2SqInt)) {
				...
				/* Step to next ramp value */
				nextLength = ((dsSqInt / 0x10000) * (dsSqInt / 0x10000)) + ((dtSqInt / 0x10000) * (dtSqInt / 0x10000));
	...
	if (x < x1) {
		...
		if ((workBuffer[GWAALevel]) == 1) {
			...
			lastLength = ((dsSqInt / 0x10000) * (dsSqInt / 0x10000)) + ((dtSqInt / 0x10000) * (dtSqInt / 0x10000));
			xSqInt = x;
			x1SqInt = x1;
			while ((xSqInt < x1SqInt)
			 && (lastLength < length2Int)) {
				...
				while ((xSqInt < x1SqInt)
				 && ((((dsSqInt / 0x10000) * (dsSqInt / 0x10000)) + ((dtSqInt / 0x10000) * (dtSqInt / 0x10000))) <= nextLength)) {
					spanBuffer[xSqInt] = rampValue;
					xSqInt += 1;
					dsSqInt += dsX;
					dtSqInt += dtX;
...
loadRenderingState(void)
{
	...
	fillOop = stackMutableObjectValue(0);
	edgeOop = stackMutableObjectValue(1);
	if (failed()) {
...
primitiveGetClipRect(void)
{
	...
	if ((methodArgumentCount()) == 1) {
		if (!((failureCode = quickLoadEngineFrom(stackValue(1))))) {
			rectOop = stackMutableObjectValue(0);
			if (failed()) {
				failureCode = primitiveFailureCode();
			}
			else {
				if ((isPointers(rectOop))
				 && ((slotSizeOf(rectOop)) >= 2)) {
					...
					return methodReturnValue(rectOop);
				}
				failureCode = PrimErrBadArgument;
			}
		}
	}
	else {
		failureCode = PrimErrBadNumArgs;
	}
	return primitiveFailFor(failureCode);
}
...
primitiveNextActiveEdgeEntry(void)
{
	...
	edgeOop = stackMutableObjectValue(0);
	if (failed()) {
		return primitiveFail();
	}
	hasEdge = 0;
