/* Automatically generated by
   ...
   from
	BalloonEnginePlugin VMMaker.oscog-eem.2040 uuid: 5c1c9278-c66a-4508-8b7c-fb1b0c8b3e46
 */
static char __buildInfo[] = "BalloonEnginePlugin VMMaker.oscog-eem.2040 uuid: 5c1c9278-c66a-4508-8b7c-fb1b0c8b3e46 " __DATE__ ;
...
#ifdef SQUEAK_BUILTIN_PLUGIN
	"B2DPlugin VMMaker.oscog-eem.2040 (i)"
#else
	"B2DPlugin VMMaker.oscog-eem.2040 (e)"
#endif
;
	...
	while (x < lastPixel) {
		...
		if ((xp >= 0)
		 ...
		 && (yp < bmHeight)))) {
			...
			fillValue = uncheckedTransformColor(value1);
		l26:	/* end bitmapValue:bits:atX:y: */;
			fillValue = ((usqInt) (fillValue & cMask)) >> cShift;
			idx = ((usqInt) x) >> baseShift;
			spanBuffer[idx] = ((spanBuffer[idx]) + fillValue);
		}
	...
	while (x < rightX) {
		...
		if ((xp >= 0)
		 ...
		 && (yp < bmHeight)))) {
			...
			fillValue = uncheckedTransformColor(value2);
		l28:	/* end bitmapValue:bits:atX:y: */;
			fillValue = ((usqInt) (fillValue & cMask)) >> cShift;
			idx = ((usqInt) x) >> baseShift;
			spanBuffer[idx] = ((spanBuffer[idx]) + fillValue);
		}
...
findNextExternalFillFromAET(void)
{
	...
	l9:	/* end quickRemoveInvalidFillsAt: */;
	...
	l19:	/* end topRightX */;
		...
		if (((workBuffer[GWSize]) - (workBuffer[GWBufferTop])) == 0) {
			...
			/* begin stackFillValue: */
			someIntegerValue3 = workBuffer[(workBuffer[GWBufferTop]) + (((workBuffer[GWSize]) - (workBuffer[GWBufferTop])) - 3)];
			goto l20;
		}
	l20:	/* end topFill */;
		fill1 = someIntegerValue3;
		if (!(fill1 == 0)) {
...
hideFilldepth(sqInt fillIndex, sqInt depth)
{
	...
	value = workBuffer[(workBuffer[GWBufferTop]) + (((workBuffer[GWSize]) - (workBuffer[GWBufferTop])) - 3)];
	...
	value1 = workBuffer[(workBuffer[GWBufferTop]) + ((((workBuffer[GWSize]) - (workBuffer[GWBufferTop])) - 3) + 1)];
	...
	value2 = workBuffer[(workBuffer[GWBufferTop]) + ((((workBuffer[GWSize]) - (workBuffer[GWBufferTop])) - 3) + 2)];
...
loadArrayPolygonnPointsfilllineWidthlineFill(sqInt points, sqInt nPoints, sqInt fillIndex, sqInt lineWidth, sqInt lineFill)
{
	...
	for (i = 1; i < nPoints; i += 1) {
		...
		if (2 > 0) {
			...
			point = ((int *) (((int *) (workBuffer + GWPoint1))));
		...
		if (2 > 1) {
			/* begin transformPoint: */
			point1 = ((int *) (((int *) (workBuffer + GWPoint2))));
			if ((workBuffer[GWHasEdgeTransform]) != 0) {
		...
		if (2 > 2) {
			/* begin transformPoint: */
			point2 = ((int *) (((int *) (workBuffer + GWPoint3))));
			if ((workBuffer[GWHasEdgeTransform]) != 0) {
		...
		if (2 > 3) {
			/* begin transformPoint: */
			point3 = ((int *) (((int *) (workBuffer + GWPoint4))));
			if ((workBuffer[GWHasEdgeTransform]) != 0) {
...
loadBeziersegmentleftFillrightFilloffset(sqInt bezier, sqInt index, sqInt leftFillIndex, sqInt rightFillIndex, sqInt yOffset)
{
	...
	if ((workBuffer[(workBuffer[GWBufferTop]) + ((((workBuffer[GWSize]) - (workBuffer[GWBufferTop])) - index) + 5)]) >= (workBuffer[(workBuffer[GWBufferTop]) + ((((workBuffer[GWSize]) - (workBuffer[GWBufferTop])) - index) + 1)])) {
		...
		value = workBuffer[(workBuffer[GWBufferTop]) + ((((workBuffer[GWSize]) - (workBuffer[GWBufferTop])) - index))];
		...
		value2 = workBuffer[(workBuffer[GWBufferTop]) + ((((workBuffer[GWSize]) - (workBuffer[GWBufferTop])) - index) + 2)];
		...
		value4 = workBuffer[(workBuffer[GWBufferTop]) + ((((workBuffer[GWSize]) - (workBuffer[GWBufferTop])) - index) + 4)];
		...
		/* begin edgeXValueOf:put: */
		value6 = workBuffer[(workBuffer[GWBufferTop]) + ((((workBuffer[GWSize]) - (workBuffer[GWBufferTop])) - index) + 4)];
		...
		/* begin bezierViaXOf:put: */
		value8 = workBuffer[(workBuffer[GWBufferTop]) + ((((workBuffer[GWSize]) - (workBuffer[GWBufferTop])) - index) + 2)];
		...
		/* begin bezierEndXOf:put: */
		value10 = workBuffer[(workBuffer[GWBufferTop]) + ((((workBuffer[GWSize]) - (workBuffer[GWBufferTop])) - index))];
...
loadCompressedSegmentfromshortleftFillrightFilllineWidthlineColor(sqInt segmentIndex, sqInt points, sqInt pointsShort, sqInt leftFill, sqInt rightFill, sqInt lineWidth, sqInt lineFill)
{
	...
	if (3 > 0) {
		/* begin transformPoint: */
		point4 = ((int *) (((int *) (workBuffer + GWPoint1))));
		if ((workBuffer[GWHasEdgeTransform]) != 0) {
	...
	if (3 > 1) {
		/* begin transformPoint: */
		point5 = ((int *) (((int *) (workBuffer + GWPoint2))));
		if ((workBuffer[GWHasEdgeTransform]) != 0) {
	...
	if (3 > 2) {
		/* begin transformPoint: */
		point6 = ((int *) (((int *) (workBuffer + GWPoint3))));
		if ((workBuffer[GWHasEdgeTransform]) != 0) {
	...
	if (3 > 3) {
		/* begin transformPoint: */
		point7 = ((int *) (((int *) (workBuffer + GWPoint4))));
		if ((workBuffer[GWHasEdgeTransform]) != 0) {
...
loadCompressedShapesegmentsleftFillsrightFillslineWidthslineFillsfillIndexListpointShort(int *points, sqInt nSegments, int *leftFills, int *rightFills, int *lineWidths, int *lineFills, int *fillIndexList, sqInt pointsShort)
{
	...
	l7:	/* end loadCompressedSegment:from:short:leftFill:rightFill:lineWidth:lineColor: */;
		if (engineStopped) {
			return null;
		}
	}
...
primitiveAddBitmapFill(void)
{
	...
	tileFlag1 = (tileFlag
		? 1
		: 0);
	point1 = ((int *) (((int *) (workBuffer + GWPoint1))));
	point2 = ((int *) (((int *) (workBuffer + GWPoint2))));
	point3 = ((int *) (((int *) (workBuffer + GWPoint3))));
	if (cmOop == (nilObject())) {
		cmSize = 0;
		cmBits = null;
	}
...
l6:	/* end allocateBitmapFill:colormap: */;
	...
	loadFillOrientationfromalongnormalwidthheight(bmFill, point1, point2, point3, bmWidth, bmHeight);
	fill = bmFill;
l8:	/* end loadBitmapFill:colormap:tile:from:along:normal:xIndex: */;
	if (engineStopped) {
...
primitiveAddPolygon(void)
{
	...
	l2:	/* end loadArrayPolygon:nPoints:fill:lineWidth:lineFill: */;
	}
	else {
		...
		for (i1 = 1; i1 < nPoints; i1 += 1) {
			...
			(((int *) (workBuffer + GWPoint2)))[0] = x11;
			(((int *) (workBuffer + GWPoint2)))[1] = y11;
			/* begin transformPoints: */
			if (2 > 0) {
				/* begin transformPoint: */
				point4 = ((int *) (((int *) (workBuffer + GWPoint1))));
	...
	l11:	/* end loadPolygon:nPoints:fill:lineWidth:lineFill:pointsShort: */;
	}
	if (engineStopped) {
		return primitiveFailFor(GEFEngineStopped);
	}
