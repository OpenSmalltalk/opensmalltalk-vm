/* Smalltalk from Squeak4.5 with VMMaker 4.13.8 translated as C source on 25 November 2014 8:00:02 pm */
/* Automatically generated by
	CCodeGeneratorGlobalStructure VMMaker-dtl.355 uuid: 9e6e08b4-e8f0-44b0-803c-de1559fbcb27
   from
	Interpreter VMMaker-dtl.355 uuid: 9e6e08b4-e8f0-44b0-803c-de1559fbcb27
 */
static char __buildInfo[] = "Interpreter VMMaker-dtl.355 uuid: 9e6e08b4-e8f0-44b0-803c-de1559fbcb27 " __DATE__ ;
...
#define InterpreterSourceVersion "4.13.8"
...
register struct foo * foo = &fum;
	...
	foo->stackPointer = (foo->activeContext + (BASE_HEADER_SIZE)) + (((TempFrameStart + tmp) - 1) * (BYTES_PER_WORD));
...
l1:	/* end fetchClassOf: */;
	...
	if (!foo->primFailCode) {
		...
		for (i = 1; i <= foo->argumentCount; i += 1) {
			...
			if ((((usqInt) foo->activeContext)) < (((usqInt) foo->youngStart))) {
				possibleRootStoreIntovalue(foo->activeContext, valuePointer);
			}
		...
		/* begin storePointer:ofObject:withValue: */
		if ((((usqInt) foo->activeContext)) < (((usqInt) foo->youngStart))) {
			possibleRootStoreIntovalue(foo->activeContext, foo->messageSelector);
		}
...
register struct foo * foo = &fum;
	...
	/* begin storePointer:ofObject:withValue: */
	if ((((usqInt) activeProc)) < (((usqInt) foo->youngStart))) {
		possibleRootStoreIntovalue(activeProc, foo->activeContext);
	}
...
register struct foo * foo = &fum;
	...
	if (foo->compilerInitialized) {
		...
		if ((((usqInt) oldProc)) < (((usqInt) foo->youngStart))) {
			possibleRootStoreIntovalue(oldProc, foo->activeContext);
		}
