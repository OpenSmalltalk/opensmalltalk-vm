...
#pragma export on
EXPORT(sqInt) primitiveStartOfMemory(void);
#pragma export off
...
sqInt extraVMMemory;
const char *interpreterVersion = "Squeak4.5 of 31 March 2014 [latest update: #13718]";
...
register struct foo * foo = &fum;
	...
	/* begin pop:thenPush: */
	oop = positive64BitIntegerFor((sqLong)memory);
	longAtput((sp = foo->stackPointer - ((1 - 1) * (BYTES_PER_WORD))), oop);
	foo->stackPointer = sp;
	return null;
}
...
void* vm_exports[][3] = {
	...
	{"", "primitiveStartOfMemory", (void*)primitiveStartOfMemory},
