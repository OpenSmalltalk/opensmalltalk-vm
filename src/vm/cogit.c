...
genDoubleComparisoninvert(AbstractInstruction *(*jumpOpcodeGenerator)(void *), sqInt invertComparison) {
	...
	if (invertComparison) {
		...
		jumpFalse = jumpOpcodeGenerator(0);
		annotateobjRef(gMoveCwR(falseObject(), ReceiverResultReg), trueObject());
		flag("currently caller pushes result");
		gRetN(BytesPerWord * 2);
		jmpTarget(jumpFalse, annotateobjRef(gMoveCwR(trueObject(), ReceiverResultReg), falseObject()));
		gRetN(BytesPerWord * 2);
	}
	else {
