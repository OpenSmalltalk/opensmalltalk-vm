/* Automatically generated by
   ...
   from
	SimpleStackBasedCogit VMMaker-oscog.16 uuid: f8b8c00f-be77-4358-a537-ac5dd4e27747
 */
static char __buildInfo[] = "SimpleStackBasedCogit VMMaker-oscog.16 uuid: f8b8c00f-be77-4358-a537-ac5dd4e27747 " __DATE__ ;
...
static AbstractInstruction * gAddRdRd(sqInt dpreg1, sqInt dpreg2);
...
static AbstractInstruction * gAndCwR(sqInt wordConstant, sqInt reg);
...
static sqInt checkIsRealMapEntrymcpcforMcpc(sqInt annotation, sqInt mcpc, sqInt targetMcpc);
...
static AbstractInstruction * gDivRdRd(sqInt dpreg1, sqInt dpreg2);
...
static AbstractInstruction * gFill16(sqInt value);
...
static sqInt findMatchingBcpcSizePC(char annotation, char *mcpc, sqInt bcpc, sqInt numBytes, void *targetMcpc);
...
static sqInt genTrampolineForcalledargargarg(void *aRoutine, char *aString, sqInt regOrConst0, sqInt regOrConst1, sqInt regOrConst2);
...
static AbstractInstruction * gJumpBelowOrEqual(void *jumpTarget);
static AbstractInstruction * gJumpBelow(void *jumpTarget);
static AbstractInstruction * gJumpCarry(void *jumpTarget);
...
static AbstractInstruction * gJumpLongNonZero(void *jumpTarget);
...
static AbstractInstruction * gJumpNoCarry(void *jumpTarget);
static AbstractInstruction * gJumpNonNegative(void *jumpTarget);
...
static AbstractInstruction * gLogicalShiftRightRR(sqInt reg1, sqInt reg2);
...
static AbstractInstruction * gMoveRdRd(sqInt dpreg1, sqInt dpreg2);
...
static AbstractInstruction * gMulRdRd(sqInt dpreg1, sqInt dpreg2);
...
static void markAndTraceCogMethodWithYoungReferents(CogMethod *cogMethod);
...
static AbstractInstruction * gOrCwR(sqInt wordConstant, sqInt reg);
...
static sqInt remapCacheTag(sqInt cacheTag);
...
static void * roundUpAddress(void *address);
...
static AbstractInstruction * gSubRdRd(sqInt dpreg1, sqInt dpreg2);
static AbstractInstruction * gSubRR(sqInt reg1, sqInt reg2);
static sqInt safemod(sqInt signedInteger, sqInt positiveModulus);
...
static int cStackAlignment = STACK_ALIGN_BYTES;
...
static AbstractInstruction *
gAddRdRd(sqInt dpreg1, sqInt dpreg2) {
	return genoperandoperand(AddRdRd, dpreg1, dpreg2);
}
...
static AbstractInstruction *
gAndCwR(sqInt wordConstant, sqInt reg) {
	return genoperandoperand(AndCwR, wordConstant, reg);
}
...
bytecodePCForstartBcpcin(sqInt mcpc, sqInt startbcpc, CogBlockMethod *cogMethod) {
 ...
 || ((((usqInt) (byteAt(mapStart))) >> AnnotationShift) == IsDisplacementX2N)));
	...
	return mapAtmethodObjstartmcpcbcpcperformUntilarg(mapStart, (homeMethod->methodObject), startmcpc, startbcpc, findMatchingBcpcSizePC, ((void *) mcpc));
}
...
/*	For the mapEntry matching targetMcpc answer if its annotation is real or
	merely a displacement. Since the enumerator mapFor:performUntil:arg: only
	invokes its argument with real entries this is simply a check for
	targetMcpc existing at all. */

static sqInt
checkIsRealMapEntrymcpcforMcpc(sqInt annotation, sqInt mcpc, sqInt targetMcpc) {
	return mcpc == targetMcpc;
}
...
cogExtendPICCaseNMethodtag(CogMethod *cPIC, sqInt caseNMethodOrNil, sqInt caseNTag) {
	...
	compilationBreakpoint(((cPIC->selector)) + BaseHeaderSize, lengthOf((cPIC->selector)));
...
cogOpenPICSelectornumArgs(sqInt selector, sqInt numArgs) {
	...
	compilationBreakpoint(selector + BaseHeaderSize, lengthOf(selector));
...
cogselector(sqInt aMethodObj, sqInt aSelectorOop) {
	...
	compilationBreakpoint(aSelectorOop + BaseHeaderSize, lengthOf(aSelectorOop));
...
compileClosedPICPrototype(void) {
	...
	for (h = 1; h <= (numPICCases - 1); h += 1) {
		gCmpCwR(3133021973U + h, TempReg);
		gMoveCwR(195929424 + h, ClassReg);
		gJumpLongZero(((void *) (3202131424U + h)));
		if (h == 1) {
			endCPICCase1 = gLabel();
		}
	}
...
compileGetErrorCode(void) {
	...
	primErrorTableSize = ((sqInt) (lastPointerOf(primErrorTable)) >> 2);
...
static AbstractInstruction *
gDivRdRd(sqInt dpreg1, sqInt dpreg2) {
	return genoperandoperand(DivRdRd, dpreg1, dpreg2);
}
...
enterCogCodePopReceiver(void) {
	...
	assertValidMachineCodeFrame(stackValue(1));
...
enterCogCodePopReceiverAndClassRegs(void) {
	...
	assertValidMachineCodeFrame(stackValue(2));
...
gFill16(sqInt value) {
	...
	return genoperand(Fill16, value);
}
...
static sqInt
findMatchingBcpcSizePC(char annotation, char *mcpc, sqInt bcpc, sqInt numBytes, void *targetMcpc) {
...
static sqInt
genDoubleArithmetic(sqInt arithmeticOperator) {
...
genDoubleComparison(AbstractInstruction *(*jumpOpcodeGenerator)(void *)) {
	...
	jumpTrue = jumpOpcodeGenerator(0);
...
genPrimitiveFloatEqual(void) {
	return genDoubleComparison(gJumpFPEqual);
}

static sqInt
genPrimitiveFloatGreaterOrEqual(void) {
	return genDoubleComparison(gJumpFPGreaterOrEqual);
}

static sqInt
genPrimitiveFloatGreaterThan(void) {
	return genDoubleComparison(gJumpFPGreater);
}

static sqInt
genPrimitiveFloatLessOrEqual(void) {
	return genDoubleComparison(gJumpFPLessOrEqual);
}

static sqInt
genPrimitiveFloatLessThan(void) {
	return genDoubleComparison(gJumpFPLess);
}
...
genPrimitiveFloatNotEqual(void) {
	return genDoubleComparison(gJumpFPNotEqual);
}
...
genTrampolineForcalledargargarg(void *aRoutine, char *aString, sqInt regOrConst0, sqInt regOrConst1, sqInt regOrConst2) {
	return genTrampolineForcalledcallJumpBarnumArgsargargargargsaveRegsresultRegappendOpcodes(aRoutine, aString, 1, 3, regOrConst0, regOrConst1, regOrConst2, null, 0, null, 0);
}
...
static AbstractInstruction *
gJumpBelowOrEqual(void *jumpTarget) {
	return genoperand(JumpBelowOrEqual, ((sqInt)jumpTarget));
}
...
static AbstractInstruction *
gJumpCarry(void *jumpTarget) {
	return genoperand(JumpCarry, ((sqInt)jumpTarget));
}
...
gJumpFPLessOrEqual(void *jumpTarget) {
	return genJumpFPLessOrEqual(backEnd, jumpTarget);
}
...
gJumpFPLess(void *jumpTarget) {
	return genJumpFPLess(backEnd, jumpTarget);
}
...
static AbstractInstruction *
gJumpLongNonZero(void *jumpTarget) {
	return genoperand(JumpLongNonZero, ((sqInt)jumpTarget));
}
...
static AbstractInstruction *
gJumpNoCarry(void *jumpTarget) {
	return genoperand(JumpNoCarry, ((sqInt)jumpTarget));
}

static AbstractInstruction *
gJumpNonNegative(void *jumpTarget) {
	return genoperand(JumpNonNegative, ((sqInt)jumpTarget));
}
...
static AbstractInstruction *
gLogicalShiftRightRR(sqInt reg1, sqInt reg2) {
	return genoperandoperand(LogicalShiftRightRR, reg1, reg2);
}
...
static AbstractInstruction *
gMoveRdRd(sqInt dpreg1, sqInt dpreg2) {
	return genoperandoperand(MoveRdRd, dpreg1, dpreg2);
}
...
static AbstractInstruction *
gMulRdRd(sqInt dpreg1, sqInt dpreg2) {
	return genoperandoperand(MulRdRd, dpreg1, dpreg2);
}
...
static void
markAndTraceCogMethodWithYoungReferents(CogMethod *cogMethod) {
	assert((cogMethod->cmRefersToYoung));
	if (isYoung((cogMethod->methodObject))) {
...
static AbstractInstruction *
gOrCwR(sqInt wordConstant, sqInt reg) {
	return genoperandoperand(OrCwR, wordConstant, reg);
}
...
static sqInt
remapCacheTag(sqInt cacheTag) {
	return (couldBeObject(cacheTag)
		? remap(cacheTag)
		: cacheTag);
}
...
static void *
roundUpAddress(void *address) {
	return ((void *) (((((unsigned long) address)) + 7) & -8));
}
...
static AbstractInstruction *
gSubRdRd(sqInt dpreg1, sqInt dpreg2) {
	return genoperandoperand(SubRdRd, dpreg1, dpreg2);
}
...
static sqInt
safemod(sqInt signedInteger, sqInt positiveModulus) {
    sqInt remainder;

	return (((remainder = signedInteger % positiveModulus)) < 0
...
/*	self assert: ((coInterpreter isIntegerObject: anOop)
	or: [coInterpreter addressCouldBeObj: anOop]). */
