...
static sqInt
implementsNewMethod(void)
{
	return 0;
}

static sqInt
implementsNewWithArg(void)
{
	return 0;
}
...
{
	return (((((usqInt)address)) >= methodZoneBase) && ((((usqInt)address)) <= mzFreeStart));
}
...
isSendReturnPC(sqInt retpc)
{
	...
	return (((target >= firstSend) && (target <= lastSend)))
	 || (((target >= methodZoneBase) && (target <= mzFreeStart)));
}
...
mapObjectReferencesInGeneratedRuntime(void)
{
	...
	for (i = 0; i <= (runtimeObjectRefIndex - 1); i += 1) {
		...
		literal = literalBeforeFollowingAddress(backEnd, ((sqInt)mcpc));
		mappedLiteral = remapObj(literal);
...
static void
mapObjectReferencesInMachineCodeForScavenge(void)
{
...
static sqInt
objectRepresentationImplementsNewMethod(sqInt primIndex)
{
	return implementsNewMethod();
}

static sqInt
objectRepresentationImplementsNewWithArg(sqInt primIndex)
{
	return implementsNewWithArg();
}

static sqInt
objectRepresentationImplementsNew(sqInt primIndex)
{
	return implementsNew();
}
...
remapIfObjectRefpchasYoung(sqInt annotation, char *mcpc, sqInt hasYoungPtr)
{
	...
	if (annotation == IsSendCall) {
		cacheTag = inlineCacheTagAt(backEnd, ((sqInt)mcpc));
		if (couldBeObject(cacheTag)) {
			mappedCacheTag = remapObj(cacheTag);
