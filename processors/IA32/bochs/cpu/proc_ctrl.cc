...
{
  if (!real_mode() && CPL!=0) {
    BX_DEBUG(("HLT: %s privilege check failed, CPL=%d, generate #GP(0)",
...
{
  if (!real_mode() && CPL!=0) {
    BX_ERROR(("CLTS: privilege check failed, generate #GP(0)"));
    exception(BX_GP_EXCEPTION, 0, 0);
  }
...
#if BX_CPU_LEVEL >= 4
  if (!real_mode() && CPL!=0) {
    BX_ERROR(("INVD: privilege check failed, generate #GP(0)"));
    exception(BX_GP_EXCEPTION, 0, 0);
  }
...
#if BX_CPU_LEVEL >= 4
  if (!real_mode() && CPL!=0) {
    BX_ERROR(("WBINVD: privilege check failed, generate #GP(0)"));
    exception(BX_GP_EXCEPTION, 0, 0);
  }
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::SYSRET(bxInstruction_c *i)
{
  ...
  if(!protected_mode() || CPL != 0) {
    BX_ERROR(("SYSRET: privilege check failed, generate #GP(0)"));
    exception(BX_GP_EXCEPTION, 0, 0);
  }
