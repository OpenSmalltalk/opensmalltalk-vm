/* Automatically generated by
	CCodeGeneratorGlobalStructure VMMaker.oscog-eem.2975 uuid: 1fe889e7-a3ea-4e7f-bbe8-cc1bd6e97100
   from
	ImageLeakChecker VMMaker.oscog-eem.2975 uuid: 1fe889e7-a3ea-4e7f-bbe8-cc1bd6e97100
 */
static char __buildInfo[] = "ImageLeakChecker VMMaker.oscog-eem.2975 uuid: 1fe889e7-a3ea-4e7f-bbe8-cc1bd6e97100 " __DATE__ ;
...
const char *interpreterVersion = "Open Smalltalk ImageChecker VM [ImageLeakChecker VMMaker.oscog-eem.2975]";
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	longAtput(startAddress, ((startAddress + 8 /* allocationUnit */) == objOop
		? 0
		: 1));
...
/*	Object parsing.
	...
	slots to 1, and this
	is ambiguous with a slimbridge. The resolution is that slimbridges are
	used only in new space.
 */
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		if (isEnumerableObject(obj1)) {
			if ((isBytesNonImm(obj1))
			 && (((lengthOfformat(obj1, formatOf(obj1))) >= cssz)
			 && ((strncmp(aCString, pointerForOop(obj1 + BaseHeaderSize), cssz)) == 0))) {
				printHex(obj1);
				/* begin space */
				printChar(' ');
...
{
	return (bytesInBody(objOop)) >= (0x100);
}
...
{
	return checkForLeaks & GCModeBecome;
}
...
{
	return checkForLeaks & GCModeFull;
}
...
{
	return checkForLeaks & GCModeIncremental;
}
...
{
	return checkForLeaks & GCModeNewSpace;
}
...
/*	useful for VM debugging; use export: not api, so it will be accessible on
	win32 and won't be written to cointerp.h
 */
...
validFreeTreeChunkparent(sqInt chunk, sqInt parent)
{
	...
	if (((bytesInBody(chunk)) / 8 /* allocationUnit */) < 32 /* numFreeLists */) {
		return "too small";
	}
...
/*	use export: not api, so it won't be written to cointerp.h */
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	primitiveDoMixedArithmetic = (headerFlags & 0x100) == 0;
}
