/* Automatically generated by
	CCodeGeneratorGlobalStructure VMMaker.oscog-eem.2582 uuid: 0ebe26fd-934b-4a0e-8d53-fd9f30a3246b
   from
	ImageLeakChecker VMMaker.oscog-eem.2582 uuid: 0ebe26fd-934b-4a0e-8d53-fd9f30a3246b
 */
static char __buildInfo[] = "ImageLeakChecker VMMaker.oscog-eem.2582 uuid: 0ebe26fd-934b-4a0e-8d53-fd9f30a3246b " __DATE__ ;
...
static sqInt checkAllocFiller(void);
static void cr(void);
...
static void NoDbgRegParms initSpaceForAllocationChecklimit(SpurNewSpaceSpace *aNewSpace, usqInt limit);
...
const char *interpreterVersion = "Croquet Closure ImageChecker VM [ImageLeakChecker VMMaker.oscog-eem.2582]";
...
#define oopisLessThan(anOop,otherOop) ((usqInt)(anOop) < (usqInt)(otherOop))
	...
	/* ImageLeakChecker>>#checkAllocFiller */
static sqInt
checkAllocFiller(void)
{
	return 0;
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if ((argc != (argi + 1))
	 || (((ifh = sqImageFileOpen(argv[argi], "rb"))) == 0)) {
		printf("Usage: %s [%s] imageFileName\n", argv[0], (PharoVM
			? "--" "verbose"
			: "-" "verbose"));
		return 1;
	}
...
runLeakCheckerForResult(sqInt gcModes)
{
	...
	return checkHeapIntegrityclassIndicesShouldBeValid(1, 1);
}
...
addressAfter(sqInt objOop)
{
	...
	return (objOop + BaseHeaderSize) + (((usqInt) (numSlots + (numSlots & 1)) << 2));
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if ((initialIndex < 32 /* numFreeLists */)
	 && ((1U << initialIndex) <= GIV(freeListsMask))) {
		if (GIV(freeListsMask) & (1U << initialIndex)) {
		...
		index = initialIndex;
		while ((((index += initialIndex)) < 32 /* numFreeLists */)
		 && ((1U << index) <= GIV(freeListsMask))) {
			if (GIV(freeListsMask) & (1U << index)) {
 ...
 */
	...
	/* Spur32BitMemoryManager>>#bytesInObject: */
static usqInt NoDbgRegParms
...
/*	Reduce the number of indexable fields in objOop, a pointer object, to
	...
	/* Spur32BitMemoryManager>>#shorten:toIndexableSize: */
sqInt
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	assert(oopisLessThanOrEqualTo((addressAfter(objOop)) + delta, GIV(endOfMemory)));
	if (delta == 0) {
		return 0;
	}
		...
		while (oopisLessThan(obj, GIV(freeOldSpaceStart))) {
			...
			if (classIndex >= (isForwardedObjectClassIndexPun())) {
				...
				if (classIndex == (isFreeObjectClassIndexPun())) {
					...
					if (field != 0) {
						storePointerNoAssertofFreeChunkwithValue(0 /* freeChunkNextIndex */, obj, swizzleObj(field));
					}
					chunkBytes = bytesInObject(obj);
					if (chunkBytes >= (32 /* numFreeLists */ * 8 /* allocationUnit */)) {
						for (index = 2 /* freeChunkParentIndex */, indexLimiT = 4 /* freeChunkLargerIndex */; index <= indexLimiT; index += 1) {
							...
							if (field != 0) {
								storePointerNoAssertofFreeChunkwithValue(index, obj, swizzleObj(field));
							}
						}
					}
				}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if ((initialIndex < 32 /* numFreeLists */)
	 && ((1U << initialIndex) <= GIV(freeListsMask))) {
		...
		index = initialIndex;
		while ((((index += index)) < 32 /* numFreeLists */)
		 && ((1U << index) <= GIV(freeListsMask))) {
			if (GIV(freeListsMask) & (1U << index)) {
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	for (i = 0, iLimiT = (32 /* numFreeLists */ - 1); i <= iLimiT; i += 1) {
		if (((GIV(freeLists)[i]) != 0)
		 && (((1U << i) & GIV(freeListsMask)) == 0)) {
			return 0;
		}
	}
	return 1;
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	objOop = objectStartingAt(((pastSpace()).start));
	...
	objOop = objectStartingAt(((eden()).start));
	while (oopisLessThan(objOop, GIV(freeStart))) {
		...
		objOop = ((((usqInt) followingWord1) >> (numSlotsHalfShift())) == (numSlotsMask())
	...
	while (1) {
		...
		objOop1 = ((((usqInt) followingWord2) >> (numSlotsHalfShift())) == (numSlotsMask())
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	prevPrevObj = (prevObj = null);
	assert((((pastSpace()).start)) < (((eden()).start)));
	objOop1 = objectStartingAt(((pastSpace()).start));
	limit = GIV(pastSpaceStart);
	...
	objOop1 = objectStartingAt(((eden()).start));
	...
	while (1) {
		...
		prevObj1 = objOop11;
		/* begin objectAfter:limit: */
		followingWordAddress2 = addressAfter(objOop11);
		if (oopisGreaterThanOrEqualTo(followingWordAddress2, GIV(endOfMemory))) {
		...
		flag("endianness");
		followingWord2 = longAt(followingWordAddress2 + 4);
		objOop11 = ((((usqInt) followingWord2) >> (numSlotsHalfShift())) == (numSlotsMask())
			? followingWordAddress2 + BaseHeaderSize
			: followingWordAddress2);
	l3:	/* end objectAfter:limit: */;
	}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		assert((long64At(objOop11)) != 0);
		if (isEnumerableObject(objOop11)) {
			if ((isBytesNonImm(objOop11))
			 && (((lengthOfformat(objOop11, formatOf(objOop11))) >= cssz)
			 && ((strncmp(aCString, pointerForOop(objOop11 + BaseHeaderSize), cssz)) == 0))) {
				printHex(objOop11);
				/* begin space */
				printChar(' ');
				printNum(lengthOfformat(objOop11, formatOf(objOop11)));
				/* begin space */
				printChar(' ');
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	initSpaceForAllocationChecklimit((&(eden())), GIV(scavengeThreshold));
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	/* SpurMemoryManager>>#initSpaceForAllocationCheck:limit: */
static void NoDbgRegParms
initSpaceForAllocationChecklimit(SpurNewSpaceSpace *aNewSpace, usqInt limit)
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	if (!(GIV(memory) == null)) {
			}
}
...
{
	return ((characterCode >= 0) && (characterCode <= ((1U << 30) - 1)));
}
...
{
	return (bytesInObject(objOop)) >= (32 /* numFreeLists */ * 8 /* allocationUnit */);
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		assert((long64At(objOop11)) != 0);
		if (isEnumerableObject(objOop11)) {
			if ((isPointersNonImm(objOop11))
			 || (isCompiledMethod(objOop11))) {
				if (isCompiledMethod(objOop11)) {
					i = (literalCountOf(objOop11)) + LiteralStart;
				}
				else {
					if ((classIndexOf(objOop11)) == ClassMethodContextCompactIndex) {
						i = CtxtTempFrameStart + (fetchStackPointerOf(objOop11));
					}
					else {
						i = numSlotsOf(objOop11);
					}
				}
				while (((i -= 1)) >= 0) {
					if (anOop == (fetchPointerofObject(i, objOop11))) {
						printHex(objOop11);
						print(" @ ");
						printNum(i);
						/* begin cr */
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		flag("endianness");
		followingWord2 = longAt(followingWordAddress2 + 4);
		objOop2 = ((((usqInt) followingWord2) >> (numSlotsHalfShift())) == (numSlotsMask())
			? followingWordAddress2 + BaseHeaderSize
			: followingWordAddress2);
	l3:	/* end objectAfter:limit: */;
	}
...
{
	return fetchLong32ofObject(fieldIndex, oop);
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		if (isEnumerableObject(objOop11)) {
			if (((classIndexOf(objOop11)) == ClassMethodContextCompactIndex)
			 && (aMethodObj == (fetchPointerofObject(MethodIndex, objOop11)))) {
				printHex(objOop11);
				/* begin space */
				printChar(' ');
				printOopShort(objOop11);
				print(" pc ");
				printHex(fetchPointerofObject(InstructionPointerIndex, objOop11));
				/* begin cr */
				if (GIV(verbose)) {
					printf("\n");
				}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	prevPrevObj = (prevObj = null);
	assert((((pastSpace()).start)) < (((eden()).start)));
	objOop2 = objectStartingAt(((pastSpace()).start));
	limit = GIV(pastSpaceStart);
	...
	objOop2 = objectStartingAt(((eden()).start));
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if ((numBytes >= (32 /* numFreeLists */ * 8 /* allocationUnit */))
	 && (printAsTreeNode)) {
		print(" ^ ");
		/* begin printHexPtrnp: */
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	for (i = 0, iLimiT = (32 /* numFreeLists */ - 1); i <= iLimiT; i += 1) {
		...
		if (((i + 1) % (((usqInt) 32) >> 2 /* logBytesPerOop */)) == 0) {
			/* begin cr */
			if (GIV(verbose)) {
				printf("\n");
			}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		if (isEnumerableObject(objOop11)) {
			if ((isCompiledMethod(objOop11))
			 && ((maybeSelectorOfMethod(objOop11)) == anOop)) {
				printHex(objOop11);
				/* begin space */
				printChar(' ');
				printOopShort(objOop11);
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		if (isEnumerableObject(objOop11)) {
			if (isCompiledMethod(objOop11)) {
				i = ((literalCountOf(objOop11)) + LiteralStart) - 1;
				while (((i -= 1)) >= 0) {
					if (anOop == (fetchPointerofObject(i, objOop11))) {
						printHex(objOop11);
						print(" @ ");
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	if (objStack == GIV(nilObj)) {
		...
		printObjStackPagemyIndexpageType(objStack, fetchPointerofObject(ObjStackMyx, objStack), ObjStackMyx);
	}
}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		if (isEnumerableObject(objOop11)) {
			/* begin numPointerSlotsOf: */
			fmt = formatOf(objOop11);
			if (fmt <= 5 /* lastPointerFormat */) {
				if ((fmt == (indexablePointersFormat()))
				 && ((classIndexOf(objOop11)) == ClassMethodContextCompactIndex)) {

					/* contexts end at the stack pointer */
					/* begin fetchStackPointerOf: */
					sp = fetchPointerofObject(StackPointerIndex, objOop11);
					if (!((sp & 1))) {
						contextSize = 0;
						goto l12;
					}
	...
	l12:	/* end fetchStackPointerOf: */;
			...
			header = methodHeaderOf(objOop11);
			/* begin literalCountOfMethodHeader: */
			assert((header & 1));
			numLiterals = ((header >> 1)) & AlternateHeaderNumLiteralsMask;
			i = numLiterals + LiteralStart;
	l11:	/* end numPointerSlotsOf: */;
			while (((i -= 1)) >= 0) {
				if (anOop == (fetchPointerofMaybeForwardedObject(i, objOop11))) {
					printHex(objOop11);
					print(" @ ");
					printNum(i);
					/* begin space */
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	for (i = 2; i < GIV(numClassTablePages); i += 1) {
		if ((fetchPointerofObject(i, GIV(hiddenRootsObj))) == GIV(nilObj)) {
			GIV(numClassTablePages) = i;
			GIV(classTableIndex) = ((sqInt)((usqInt)(((((GIV(numClassTablePages) - 1) < 1) ? 1 : (GIV(numClassTablePages) - 1)))) << (classTableMajorIndexShift())));
			return;
		}
	}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	totalFreeBytes = 0;
	for (i = 1, iLimiT = (32 /* numFreeLists */ - 1); i <= iLimiT; i += 1) {
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	min = 2;
	for (i = min, iLimiT = (32 /* numFreeLists */ - 1); i <= iLimiT; i += 1) {
...
validFreeTreeChunkparent(sqInt chunk, sqInt parent)
{
	...
	if (((bytesInObject(chunk)) / 8 /* allocationUnit */) < 32 /* numFreeLists */) {
		return "too small";
	}
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	printNum(6521 /* imageFormatVersion */);
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		assert((long64At(objOop11)) != 0);
		if (isEnumerableObject(objOop11)) {
			if ((classIndexOf(objOop11)) == ClassFloatCompactIndex) {
...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		if (isEnumerableObject(objOop11)) {
			if ((isCompiledMethod(objOop11))
			 && ((maybeSelectorOfMethod(objOop11)) == selector)) {

				/* try and print the key of the method class association (the name of the implementing class) */
				methodClassAssociation = methodClassAssociationOf(objOop11);
				printHexnp(objOop11);
				/* begin space */
				printChar(' ');
				/* begin printOopShortInner: */
...
{
	return (imageVersion == 6521 /* imageFormatVersion */)
	 || (0);
}
...
((methodHeader >> 1)) & AlternateHeaderNumLiteralsMask)) + LiteralStart) * BytesPerOop;
				...
				reverseBytesFromto(wordAddr, objOop11 + (sizeBitsOf(objOop11)));
			}
			if (fmt == (firstLongFormat())) {

				/* Bitmap, Float etc */
				if (swapFloatWords
				 && ((classIndexOf(objOop11)) == ClassFloatCompactIndex)) {
					temp = longAt(objOop11 + BaseHeaderSize);
					longAtput(objOop11 + BaseHeaderSize, longAt((objOop11 + BaseHeaderSize) + 4));
					longAtput((objOop11 + BaseHeaderSize) + 4, temp);
				}
				else {
									}
			}
