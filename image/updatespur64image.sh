...
if [ ! -f $BASE64.image ]; then
	...
	echo $VM $BASE64.image -doit NukePreferenceWizardMorph.st
	$VM $BASE64.image -doit NukePreferenceWizardMorph.st
fi

echo $VM $BASE64.image -doit UpdateSqueakTrunkImage.st
$VM $BASE64.image -doit UpdateSqueakTrunkImage.st
